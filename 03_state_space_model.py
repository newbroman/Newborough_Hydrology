...
The rainfall lag is controlled by HEADLINE_LAG in config.py. The lag
diagnostic (03_04) tests lags 0–3 for each cluster. Physical basis:
monthly dipwell readings taken at the end of month t (or start of month
t+1) reflect recharge from month t's rainfall. With the corrected
bucketing in Script 01 (field-convention: day ≤ 15 → previous month),
the well index is correctly labelled and HEADLINE_LAG = 0 pairs each
month's water level with the same month's rainfall.
...
# cluster's centroid fit prefers lag>0 (R^2 ~ 0.6-0.7 vs 0.3-0.5 at lag-0,
...
def fit_ssm(h_series: pd.Series, climate: pd.DataFrame,
    ...
    lag      : integer month lag applied to rainfall. Default from config.
...
def lag_diagnostic(centroids: dict[int, pd.Series],
    ...
    Read this table alongside the bootstrap CIs: a lag>0 fit with a much
