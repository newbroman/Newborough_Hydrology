...
### D-133 — The pipeline records the raw inputs it consumed, and a gate fails when a committed output no longer matches the committed input

*2026-09-05*

`run_analysis.py` (2.10.0) records the SHA-256 + byte length of each `REQUIRED_DATA` raw input into `outputs/pipeline_manifest.json` under `input_provenance`, on a from-step-1 full run; `--manifest-only` and a `--from N>1` resume carry the prior run's hashes forward unchanged, so a registry rewrite neither loses nor falsifies the record. `tools/input_provenance_lint.py` (1.0.0) recomputes the hashes and FAILS when a raw input has changed since the run that produced the outputs; it gates in `check_all` (1.7.0) beside `output_lag`. Content hash, not mtime, because a checkout rewrites mtimes; the three authoritative `REQUIRED_DATA` CSVs, extensible; a dedicated tool, not an `output_lag` axis. Until a full run records the hashes the manifest carries no key and the gate skips cleanly.

**Revisit if** a raw input outside `REQUIRED_DATA` (a Script 09/10/24 documented exception) becomes load-bearing enough to certify, or the recording moves from the manifest to a dedicated provenance artefact.

### D-134 — δ₀ is a family of fitted values; unqualified "δ₀" denotes the forest-free linear-capped fit and the four other specifications each carry a standing label

*2026-09-05*

Unqualified **δ₀** always denotes the **forest-free linear-capped** fit (`Headline_fit_delta_0` = −31.35 mm yr⁻¹ in `25_report_numbers.csv`; source row `forest_free` / `linear_capped` in `25_01_panel_fit_parameters.csv`). The four other legitimate values each carry a standing qualifier wherever quoted: **full-network δ₀** (−31.72, source `full`), **canopy-controlled δ₀** (−32.40, `Canopy_controlled_delta_0`, source `full_canopy`), **C3-only δ₀** (−29.06, source `c3_only`), and **spring δ₀** (−33.34, source `forest_free_mam`). δ₀ remains the d=0 amplitude and is NOT the coastal headline (D-047): the headline coastal figure is the reference-distance rate `Headline_coastal_rate_at_ref` = −26.19 mm yr⁻¹ at 150 m. A canonical reference note in the Methods Supplement coastal chapter (§S.15) fixes the five labels, their CSV source rows, and the D-047 caveat.

**Revisit if** a sixth specification's δ₀ enters the corpus, or a downstream consumer needs δ₀ machine-readable per-spec beyond the existing CSV `source` column.

### D-135 — Published PDFs are refreshed — text fields and document indexes — at build time, so a published PDF is always internally consistent with its ODT

*2026-09-05*

The per-document export refreshes through UNO before it writes, in the sequence `export_master_pdf.py` already used: load with `UpdateDocMode` FULL_UPDATE, `TextFields.refresh()`, `.update()` every `DocumentIndex`, `TextFields.refresh()` again, `storeToURL(writer_pdf_Export)`. The shared plumbing lives in `tools/uno_pdf.py` (1.0.0); `tools/export_odt_pdf.py` (1.0.0) is the flat-ODT batch entry (one soffice session for the whole document map); `export_master_pdf.py` (1.3.0) is refactored onto `uno_pdf.py` with its `report.pdf` output unchanged (`update_links=True` is the master-only step). `build_pdfs.sh` finds a UNO-capable interpreter and, when python3-uno is not importable, WARNS and falls back to `--convert-to` (unrefreshed) rather than hard-failing; the fallback is counted in the run summary, never silent.

**Revisit if** LibreOffice's headless convert gains a reliable update-on-export, or python3-uno stops being available on the publishing machine (then the fallback becomes the norm and the refresh guarantee is lost).

### D-136 — Corpus-wide symbol disambiguation (W20): d, α and L collisions resolved through the register

*2026-09-06*

The rename is driven by the symbol register (`tools/symbol_register.csv`), applied by `symbol_apply` for prose occurrences and by targeted `odt_edit` for the equation-form occurrences its guards intentionally skip (a bare d before `)`, an L before `²`, and the qualified lag forms d(t−1) / d_{t−1}), so prose and equations move together. - **d** — distance keeps the bare glyph (sense `d_coast`). Depth below ground, in all three of its uses (P_flood input, the depth-dependent PET term d = max(−h,0), and the per-grid-cell viewer depth), collapses to one displaced sense `d_depth → d_w`; the three inventory senses were one physical quantity and register rule #2 forbids three senses sharing a replacement. The lag forms become d_w(t−1) / d_{w,t−1}. "Number of days" in the Thornthwaite PET K-correction K = (N/12)·(d/30) stays bare d (sense `d_days`, canonical): a standard established formula in a disjoint domain, no reader confusion, and the two formula instances live in embedded MathML objects — renaming them would make a standard formula non-standard for no disambiguation gain. report9's equilibrium-level equation, which wrote the datum as a lowercase d ("… /β₃ − d (d = 3.7 m)"), is corrected to z₀, the corpus datum symbol (D-datum precedent). - **α** — significance α (= 0.05) keeps the glyph (`alpha_sig`). The generic OLS intercept of the SECONDARY regressions (well-to-well correlation CEH34~CEH9, the scrape covariate model, the van-Willegen-style MSL model, and the "(α, β)" fit- parameter mention) becomes `alpha_ols_generic → α₀` — distinct from the SSM Model-B intercept, which is and stays α_B. The MS colour-map "visually distinguishable at α = 0.62" was a matplotlib fill-opacity, not a significance level, and is reworded to "a fill opacity of 0.62" so no third sense of α survives. - **L** — the coastal reach is L_cg and the HAC truncation lag keeps bare L; the aquifer/flow-path length in the decadal timescale L²Sy/(Kb) is the existing register sense `L_boussinesq → L_B` (context extended to catch the L²Sy occurrences). This supersedes the W20 addendum's proposed L_aq, which was written without knowledge of the already-committed L_B. - **MCA** — the abbreviation expands as "Multi-Cluster Affinity" (7 corpus uses; and semantically correct — MCA_Count is the number of clusters a well correlates with at r > 0.9, i.e. affinity to *multiple* clusters). The single Methods-Supplement stray "Mean Cluster Affinity" was the error and is corrected. This reverses the W20 addendum H.4, which had the direction backwards. Applied across nine documents (report8, report9, report10, report11, the Methods Supplement → v1_9_112, Supplementary Material → v1_28, Paper 1 → v1_38, and both Web-Tools documents), 49 edits, verified: symbol_check register faults 0, the applied senses clear, distance-d integrity confirmed unchanged, every edited ODT opens in headless LibreOffice.

**Revisit if** a new bare sense of d, α or L enters the corpus and is left unregistered (symbol_check ambiguous is the tripwire); or the Thornthwaite K-correction is ever written in a passage where days-d and distance-d co-occur, at which point d_days would need its own subscript after all.

### D-137 — Notation is a reader-facing report chapter (ch16) plus a complete Methods-Supplement appendix (W20)

*2026-09-06*

Two tables, one instrument. The **report** gains a new final chapter, **ch16 "Notation"** (a chapter, not an appendix, per Martin 2026-09-06), after Data Availability (ch15), linked in `report.odm`; it carries the **reader-facing subset** — every symbol/abbreviation/statistic that occurs in the report's own chapters (the R+A rows) — and points to the complete set. The **Methods Supplement** gains **Appendix C "Notation and abbreviations"** under S.24, carrying the **complete** set (all rows, including supplement-only quantities). Symbol forms are the as-implemented register forms after D-136 (z₀, λ, κ, m_P, L_cg, L_B, c_far, d, d_w, α, α_B, α₀, ρ_h, α_W, φ, φ_F, η, η², D, τ, t½, t_R, …); the significance constant is `DETECTABILITY_ALPHA`; depth-coupling values are κ from `Best_Kappa`; MCA expands "Multi-Cluster Affinity".

**Revisit if** the report's chapter numbering changes (ch16's `../report16.odt` link and the "Notation" chapter position move with it); or a new reader-facing symbol enters the report chapters and is absent from ch16 (the R+A rule then adds it to both tables).

### D-138 — M13's coastal-correction sensitivity is stated as a rate-consistency cross-check, not a lever-arm step

*2026-09-06*

A rate-consistency paragraph added to report10 §5.5.1, immediately after the "two implementations differ" paragraph. It states the paired drift term absorbs −17.0 mm yr⁻¹ against the network gradient's predicted differential of −10.7 mm yr⁻¹ for the 553 m vs 955 m (402 m) Impact-to-Forest contrast — the 1.59 ± 0.44 amplitude already named in the section — consistent within the per-well scatter about the gradient (≈0.7 residual SD) and acting against the felling signal. **Rate form, chosen by Martin over step form, 2026-09-06.**

**Revisit if** the paired-design drift donor changes, or the free `s_coast` ceases to be indistinguishable from one at the Forest controls (the D-111 trigger) — either would force the "within noise / acts against the felling signal" wording to be re-derived.

### D-139 — Paper 2's coastal-confound framing and the retired tau are reconciled to the settled corpus (W32)

*2026-09-06*

Correct now, minimally (v17 -> v18), Martin 2026-09-06. 5.2.1 rewritten to the D-111/D-138 framing: the coastal-drift term absorbs the differential at 1.59 +/- 0.44 times the network amplitude (p = 0.18 vs one, p < 0.001 vs zero), so +0.113 m is measured net of the gradient, and constraining the term to one unit -- which the record does not require -- lowers the step to +0.077 m (p = 0.002), reported as a sensitivity; the network gradient is not applied per-well (wells withheld, under a fifth of variance, per-well departures exceeding the differential). 5.4 *lower bound* -> *measured net ... rather than bounded by it*. Separately, the retired tau = Sy/beta3 "SSM drainage timescale" at 6 -> the recession time 1/beta3 (the corpus's time-like quantity; tau is the storage-drainage index, not a timescale).

**Revisit if** the W31 Paper 2 methods supplement or the W135 full edit reframes the coastal sensitivity (this prose is derived from report10 5.5.1/5.11 and moves with them); or the paired-design drift donor changes (the D-111 trigger).

### D-140 — Ranwell (1959) historical water-table sites are georeferenced as tier-D Script 43, two routes, positions provisional (W95)

*2026-09-06*

A registered **tier-D** pipeline step, Script 43 (Martin: pipeline step, not a one-off). Two independent registration routes; **geometric is the headline, B validates** (Martin). Route A: grid-north-up similarity from Penlon Lake (Features.kml "Llyn Rhos Ddu", the sole hard OSGB point), scale from the 1/4 km scale bar, rotation assumed grid-north-up. Route B: DEM height vs Ranwell's tabulated OD heights, independent of A; per-site A-vs-B residual + a coast2006 shore misfit are the gate. Nearest modern dipwell per site; a modern well counts as having a 1950s analogue within RANWELL_ANALOGUE_RADIUS_M = 100 m. Copyright: the JSTOR scan stays out of both repos (D-081 shape); committed inputs are DERIVED DATA (digitised pixels), plus the output vector and citation.

**Revisit if** precise digitisation replaces the label proxy (sites 2/3/7 added, all positions refined - re-run and the outputs finalise); or the Route-B / shore diagnostics come back large, meaning the grid-north-up assumption or the scale is wrong and rotation must be fitted (e.g. from a shore-line match) rather than assumed.

### D-145 — Ranwell's 1951-53 readings, recovered from his printed figures, are admissible data; the SSM is tested against them out of sample and the slack water table is compared with 1951-53 at basin level with a four-term error (Script 44)

*2026-09-08*

(1) the readings are **digitised from the figures** and committed as raw data (`data/ranwell_1951_53_water_levels.csv`, 146 readings; `ranwell_1951_53_monthly_ranges.csv`, 84 ranges; `ranwell_1950_53_parc_mawr_rain.csv`, 48 months; method, accuracy and the independent Fig. 4/Fig. 7 cross-check in `data/RANWELL_PROVENANCE.md`) - factual data recovered from a published figure, the figure itself staying uncommitted (D-081). (2) A registered **tier-A** step, **Script 44**, after Script 43, asks three questions in decreasing order of what the data bear. **Q1, the hindcast:** each printed site is paired with every modern well sharing its DEM basin (Script 43 Route T) within RANWELL_PAIR_MAX_M; each well's committed Model A coefficients are driven by RAF Valley climate from 1930 (`simulate_ssm`, as Script 39) and set against Ranwell's monthly-mean reading **after removing the mean offset** and **capping the model at the ground** (a flooded slack reads at the surface: the observation is censored there); the nearest paired well is the headline and every pairing is written. **Q2, the level:** Ranwell's site mean (Fig. 4 readings where they exist, else Fig. 7 mid-ranges; the two agree to 0.05 m) against the modern mean water-table surface interpolated to the site (IDW, RANWELL_IDW_*), minus the climate-only expectation from the Q1 hindcast; per-site error = quadrature of the IDW leave-one-out RMSE, the surface gradient times the Route H positional uncertainty, the reading-mean/mid-range sampling difference, and the Route H datum-offset MAD; sites combine by inverse variance with chi2/dof reported; a second combination over the sites whose modern surface is constrained (LOO <= RANWELL_LOO_MAX_M). **Q3, the rate:** delta over the interval between record midpoints, per site and combined, `resolved` = |delta| > 2 sigma. A raw-input exception is recorded here as D-051 did for Script 39; the step skips cleanly without the CSVs.

**Revisit if** Ranwell's field books or a metric survey of his pipes surface (replaces the digitisation, adds nine sites); the Bangor theses (Jennings 1990; Mills 2006) yield a measured series between 1953 and 1989; Script 40 measures a shoreline change at the AS slack that explains Site 8 (the one site more than a metre below Ranwell, on an unconstrained modern surface); or a modern well is installed at a Ranwell position, which converts Q2 at that site from an interpolation to a measurement.

### D-155 — Pipeline outputs are produced by the publishing machine, never through the bridge, and no script silences its warnings

*2026-09-11*

**the publishing machine is the pipeline's environment. Outputs committed to the repo are produced there, by `run_analysis.py`, and not through the bridge.** A bridge run is a diagnostic: it may read, inspect, lint and prove a script parses, but an artefact it writes is not a pipeline artefact and must not be committed as one. **And no script silences its warnings** — the blanket `warnings.filterwarnings('ignore')` is removed from all nine that carried it (10a, 10d, 10e, 10h, 10i, 12, 29, 30, 41), restoring Python's default of one report per unique warning per location.

**Revisit if** the bridge and the publishing machine are brought to a pinned, verified-identical environment, in which case a bridge run could produce committable artefacts and the distinction collapses; or a specific warning proves both unavoidable and uninformative, in which case it is silenced by message and module at the point it is raised, with the reason beside it.

### D-156 — Script 41's region masks are rasterised per constellation group, through that group's own fitted transform and at the frame's own size

*2026-09-11*

**masks are rasterised per constellation group**, through that group's own fitted transform, at the frame's own pixel size, cached on `(group, width, height)` (Script 41 2.10.0, Martin 2026-09-11). The 2.9.0 frame-size guard — withhold a mismatched frame with a stated reason rather than crash — is kept as a defence and can no longer fire.

**Revisit if** two frames in the SAME constellation group are found to need different transforms (the group is then not the right unit — group by fitted transform, not by constellation); or a region's pixel count varies so widely between groups that `CANOPY_MIN_REGION_PX` admits a frame in one group and refuses the same region in another for reasons of viewpoint rather than visibility (the threshold would then need to be a fraction of the region's projected area, not an absolute count); or the index is ever redefined as anything that consumes an absolute pixel count.

### D-157 — The read-modify-write stores are read with a correctly-rounded float parser, and a round-trip gate keeps them that way

*2026-09-11*

**every read and write of a read-modify-write store goes through `utils/store_io`** (`read_store`, which parses with `float_precision="round_trip"`, and `write_store`), and `tools/store_roundtrip_lint.py` gates it: for each store in `tools/roundtrip_stores.csv`, a read followed by a write must be **byte-identical** to the file on disk. Routed: `site_observations.py` 1.5.0 (3 reads, 2 writes) and `pipeline_params.py` 1.12.0 (11 reads, 6 writes).

**Revisit if** a store in `roundtrip_stores.csv` fails the gate because a legitimately new column does not round-trip — a date or a formatted string, not a float (then the registry needs a per-store column exclusion, not a relaxed gate); or pandas' default parser becomes correctly rounded, which `--selftest` will announce rather than hide (the gate still stays — the property is worth asserting whoever provides it); or a third store appears that is written row-at-a-time and is NOT registered here, which is the failure mode the registry itself cannot detect.

### D-158 — `write_initial_site_observations()` fills gaps in the site-observations store; it does not reset it

*2026-09-11*

**no. `write_initial_site_observations()` now fills gaps and resets nothing** (Martin, 2026-09-11; `site_observations.py` 1.6.0). A missing file is created with default rows as before. An existing file keeps **every** row's `value`, `source`, `updated` and `run_id` exactly as found, and gains a default row only for a registered observation it lacks. Each fill is logged by name with its producer and the note that the producer has not run yet.

**Revisit if** a producer legitimately needs its row reset before it runs — then it resets its OWN row and says so, rather than Script 01 resetting everyone's; or the store gains a column that is neither registry-owned nor producer-owned (the two-owner split is what makes "leave it alone" decidable, and a third kind of column breaks it); or a gap fill is found masking a producer that silently stopped running, in which case the fill should fail the pass rather than log it.

### D-159 — W94 becomes a measurement, not a validation: the warren only, the waterline read as a contour, and flood extent scored per slack rather than by area

*2026-09-11*

**no, and the new aim supersedes the old one.** W94 becomes a **measurement**: the frames supply water-surface elevations at ungauged slacks across the warren, and those become observations the network does not otherwise hold. Five rulings, all Martin's, 2026-09-11: 1. **Scope is the warren — the forest is excluded.** Site boundary 861.6 ha, forest union 722.1 ha of which 214.1 ha falls inside the boundary, leaving a **warren of 647.5 ha**, three quarters of the site. The mask is **canopy BY DATE**, not the static `in_forest` flag. 2. **The admission gate is a minimum patch size, not `CANOPY_MAX_GSD_M`.** That constant stays at 2.0 for Script 41. 3. **Replant closure dates come from Script 41's committed restock index**, not from assertion. 4. **Shadow is handled by basin closure** — a candidate patch whose DEM basin does not close is shadow — together with the outline-spread test and the across-frame repeatability of a directional shadow. 5. **Scoring is binary per slack per date, with area secondary and always carrying its sensitivity.**

**Revisit if** the two May negatives show flood patches, which falsifies the method outright and is not fixable by tuning; or the positives resolve only in basins that already hold a flooded well, which would make this a restatement of the dipwells rather than an independent measurement; or the repeat-floor drift test (a slack flooding on two dates must return the same floor from the 2023 DEM) shows drift large enough that pre-2023 frames cannot use it; or a LiDAR capture contemporaneous with the early frames becomes available, which would remove the vintage assumption entirely.

### D-154 — The coastal-retreat fit is not detectably distorted by offshore shelter, and the test that says so could only have found a large effect

*2026-09-11*

**no change to the Script 25 specification.** The fit stands as published — forest-free linear-capped, δ₀ = −31.35 mm/yr, L_cg = 894 m, c = −0.10 mm/yr — and the shelter hypothesis is recorded as tested and not supported, with its detection limit stated rather than implied.

**Revisit if** the bodies are confirmed emergent or substantially shallower than assumed, which would raise the expected shelter above the detection limit; or the monitoring network gains alongshore extent west of the present coverage; or a wave model gives a predicted shelter field to test against rather than a binary sector.

### D-153 — KML is read through `utils/kml_io.py`, never `gpd.read_file`, and the reason is that GDAL driver availability is a property of the machine

*2026-09-11*

**every KML read in the pipeline goes through `src/utils/kml_io.py` `read_kml()`.** No script calls `gpd.read_file` on a `.kml`, with or without `driver=`, and no script registers a driver for itself. `read_kml` tries **KML**, then **LIBKML**, then a **pure-XML route** — ElementTree with pyproj and shapely — that uses no GDAL driver at all, reports which route succeeded, and raises only if all three fail, naming each attempt. Script 41 is migrated at **2.7.0**; Scripts 37, 37b, 13 and 20 still read directly and are owed the same change.

**Revisit if** the pure-XML route is found to mis-parse a KML construction the GDAL drivers handle correctly (nested MultiGeometry, or a file with its own CRS declaration); or geopandas removes the fiona engine entirely, in which case routes 1 and 2 can be dropped and only the XML route kept.

### D-152 — The drawdown-field construction belongs in Spatial Analysis, not in the SSM chapter, and λ is defined once

*2026-09-10*

**§3.4.5 moves wholesale to §3.8.2**, ahead of the Scenario Framework, which becomes §3.8.3. §3.8.1 keeps its number. **λ is defined once**, in the moved §3.8.2, with its constants (K = 6.0 m day⁻¹, b = 5 m, Sy = 0.308, β₃ = 0.0569 month⁻¹ from C3) and its uncertainty range; §3.8.1's net-state paragraph now points there instead of re-typing them. §3.4.6 becomes §3.4.5 and keeps the t½ / t_R / τ definitions and the diagnostic synthesis.

**Revisit if** the §3.4.6 split is taken up, which would move the half-life map to §3.8 and retitle what remains; or a further spatial field is added to §3.4, which would say the boundary this entry draws is not holding.

### D-151 — Each versioned document keeps ten versions — its newest, its oldest, and the eight most recent of the rest; the Methods Supplement resets to v2_0_0

*2026-09-10*

**Ten versions per family**, resolved numerically, not lexicographically: the **newest**, the **oldest**, and the eight most recent of the remainder. `tools/prune_versions.py` applies it — a plan by default, removal only under `--apply`, and it refuses to run at all unless `odt_media verify` is green first. The Methods Supplement is renamed **`v1_9_123` → `v2_0_0`**; the three-field shape is kept and only this document resets (Martin, 2026-09-10). 73 files and 23.5 MB removed on the first pass; `docs/` 242 → 219 MB.

**Revisit if** a family needs a version restored from beyond the window, which would say ten is too few; or a second document resets its major version, at which point the reset should be a stated convention rather than a one-off.

### D-150 — A handover entry may be as long as its session's work warrants; the 40-line per-entry cap becomes advisory and Tier 0's total prose budget is the binding constraint

*2026-09-09*

**No.** `NOTE_MAX_LINES` becomes `NOTE_LONG_ENTRY`, an **advisory** threshold: `--check` prints an entry over 40 lines and does not fail it. Martin's ruling: *"long days get long entries."*

**Revisit if** Tier 0 prose approaches 600 lines and the newest entries are the reason — then the constraint has arrived at the level where it belongs and something leaves Tier 0, per the bootstrap's standing rule; or entries begin carrying reasoning that belongs in a changelog, which is a content problem a line cap would not have fixed either.

### D-149 — Superseded versions of picture-heavy documents keep their images in a shared content-addressed store rather than inside the zip; the current version of every document is never touched, and byte-identical rehydration is a gate

*2026-09-09*

**Outside the document, not inside it.** Superseded versions are stripped to ~112 KB with their images moved to `docs/media_store/<md5><ext>`, content-addressed, plus a committed `.media.json` manifest. **The CURRENT version of every document is never stripped**, so `build_pdfs.sh`, `export_lag`, `doc_version_sync`, `refresh_mirrors` and every editing path go on seeing a whole file and need no change. `tools/odt_media.py verify` rebuilds every stripped document in memory and compares it with the md5 recorded at strip time; it is a gate in `check_all`.

**Revisit if** a LibreOffice release resolves relative `xlink:href` links (the test in the spec is four lines and settles it); or `verify` ever fails, which means an archived version can no longer be reconstituted and the design has given up the only thing it promised; or a document family appears whose archives cannot be rebuilt byte-for-byte — `Supplementary_Material_v1_6.odt` is already one, written with per-entry data descriptors that Python's zip writer will not emit, and it is left whole by design rather than stripped on a weaker guarantee.

### D-148 — CEH13 and CEH14 are excluded from Script 32's mapped differential trends, on their own drainage-coefficient grounds and not by inheriting the MSL5 exclusion; the 2026-06-27 blanket-include is withdrawn

*2026-09-09*

Excluded from the mapped trends. Script 32 1.5.0 filters on its own `config.DIFF_EXCLUDED_WELLS = ("ceh13", "ceh14")`, applied in `per_well_trends()`. Both wells REMAIN in the site-mean reference panel. The caption was right and the code had drifted, so no document edit follows: report9's 72 and 75 become correct as written, and the significance counts are unchanged at 8 and 13 because neither well was individually significant in either window.

**Revisit if** a refit moves either well's drainage coefficient into the range the rest of the network occupies, at which point the disqualification lapses on its own terms; or the figure is re-specified around something other than differential movement within the recession geometry, which is what makes a barely-draining well incomparable here; or the same question is settled differently for Script 33 under W146, in which case the two analyses should be made to agree or the difference argued explicitly.

### D-147 — Coastal-gradient uncertainties (δ₀, the 150 m rate, L_cg) are quoted on the well basis, estimated by refitting with each well withheld in turn; the fitted row-basis standard errors are nominal precision and are not quoted as uncertainties

*2026-09-09*

The uncertainty quoted for δ₀, for the reference-distance rate and for L_cg is the well-basis standard error — √((n−1)/n · Σ(θᵢ − θ̄)²) over the n = 61 refits with each well withheld in turn — and the 95 % intervals quoted are ±1.96 × that error (`25_report_numbers.csv`: `delta0_well_basis_se_mm_yr`, `delta_ref_well_basis_se_mm_yr`, `L_well_basis_se_m`, each with `_ci_lo`/`_ci_hi`). The fitted standard errors remain in `25_01_panel_fit_parameters.csv` as the model's nominal precision, are labelled "fitted" wherever a document compares them (the MAM-refit diagnostic), and are not quoted as uncertainties. Other specifications (full network, canopy-controlled, C3-only, the exponential form) are quoted as point estimates without ±: the LOO is not extended to them, and the comparison across specifications is of point estimates. No point estimate, the specification comparison, the transect corroboration or the cluster partition changes.

**Revisit if** a well-clustered analytical covariance is implemented in `fit_panel` (it then replaces the refit estimate and the two should agree); or the panel gains enough near-shore wells that the refit and fitted errors converge; or the LOO is extended to the other specifications, at which point their ± may return on the same basis.

### D-146 — `MSL5_EXCLUDED_WELLS` is scoped to the MSL5 analysis because MSL5 is an open-dune metric that has not been run for the forest; it is not a network-wide well exclusion

*2026-09-09*

Scoped. `MSL5_EXCLUDED_WELLS` excludes CEH13 and CEH14 from the MSL5 analysis (Script 26's cluster trajectory, latest-per-well and IDW map; Script 20's MSL5 surface) and from nothing else. Clustering, levels, the BACI, the C4 centroid fit (D-005) and the observational amplification metric (D-028) keep both wells. Other scripts may reuse the SET as a named sensitivity, as 30 and 34 do, but must not inherit the EXCLUSION.

**Revisit if** an MSL5 target is defined for wells under canopy (the analysis is then run for the forest and the exclusion needs its own ecological justification there); or a script starts consuming the set as a blacklist outside the MSL5 analysis, which is the misuse this entry exists to prevent.

### D-141 — The +113 mm clearfell step is a front-loaded recovery that is not sustained under a permanently grazed-clear canopy; the decay is drift-robust; the Edge null is a developing-1998-replant-canopy confound (W96)

*2026-09-06*

Martin 2026-09-06. (i) +113 mm stays the D-061 record-mean headline (the 10a_12 guard reproduces it to 1e-9; era-split is a decomposition, not a refit). (ii) The Impact step is presented as FRONT-LOADED and NOT SUSTAINED even though the canopy never returns -- mechanism, both already in the report's physics and neither requiring regeneration: head-dependent drainage (beta3) exporting the recharge surplus toward a lower equilibrium (the scrape relief-drainage relaxation, 5.4.1), plus the site-wide beta1 decline eroding the winter-recharge advantage. The late-era step is reported as not distinguishable from zero. (iii) The Edge null is attributed to the developing 1998 replant canopy at CEH30/CEH16 (with the in-block Forest controls CEH32/CEH33), areas that -- unlike the grazed 2017 compartment -- do regenerate.

**Revisit if** further monitoring resolves the late-era Impact step away from zero (either sign); or the in-block-vs-outside control contrast becomes significant under the fixed_s25 or free_per_era treatments (would make the confound first-order and require the Forest-control baseline itself to be corrected); or the grazing regime on the 2017 compartment changes.

### D-142 — Figure sources are read from the caption Source: marker, not a positional registry; figure_table_sources.csv retired for figures

*2026-09-07*

The figure's own caption marker is the source of record. `figure_map.py` (`caption_source`, v1.1.0) parses it and resolves it on disk, and `figure_map.csv` carries it as a `source` column. `build_figure_ledger.py` (v2.2.0), `ref_audit.py` (v1.3.0) and `reembed_figures.py` (v1.1.0) all read figure sources from there. The 116 Figure rows in `figure_table_sources.csv` are removed; the file stays as the TABLE source-of-record (71 Table rows) that `table_source_lint`/`table_configs` depend on.

**Revisit if** a report figure caption is authored without a resolvable `Source:` marker (the ledger flags it and `ref_audit` stops covering it -- add the marker to the caption); or a non-master document (paper, Supplement, summary) needs its figures tracked here (its captions carry no such marker, so a dedicated route would be needed).

### D-143 — The project's own records are a gate: one entry point, a generated HANDOFF, a rolling 40-line HANDOVER_NOTE, and rules that carry a lifecycle

*2026-09-07*

(1) One entry point: the project box, CLAUDE.md and readme all reduce "start here" to two lines -- read `working/HANDOVER_BOOTSTRAP.md`; run `tools/session_handover.py` -- and CLAUDE.md carries traps and non-negotiables only, no counts or state. (2) The free-form `HANDOVER_cowork_*.md` format is retired. A session ends with `session_handover.py --write` (the generated state), a dated entry of at most 40 lines at the top of the rolling `working/updates/HANDOVER_NOTE.md` (the narrative), and `--check` green. The project store holds a signpost, not copies. (3) `session_handover.py --check` is a `check_all` gate ("records"): DECISION_INDEX count equals the log; the newest HANDOFF is newer than the newest substantive commit on either repo; the note has an entry for the newest commit day, entries are newest-first and within length; every script commit in the last 3 days has a `CHANGELOG_delta` naming it (a 2-day lead allowed); Tier-0 prose within the D-080 budget. (4) Rules have the lifecycle decisions have: a rule that can be checked becomes a lint; a rule that cannot is written as a pointer to where the fact lives, never as the fact; a superseded rule is retired by a dated line in the ledgers README's "Rule retirements" block. First retirement: the NUMBER_LEDGER-row rule.

**Revisit if** the records gate reads red for a reason that is not a missing record (then fix the detector, per its `--selftest`, not the rule); the 40-line cap forces a session to omit something the next session then re-derives (raise the cap, with the D-080 budget re-measured); or a second person joins the project, at which point the day-granularity note check needs a per-session rather than per-day key.

### D-144 — One document family is live at a time; every other document is numbers-only and accepts a prose edit only with a recorded reason; the Methods Supplement follows the pipeline by refusal

*2026-09-07*

(1) Phases, not a static table. Phase 1 (now): the report (report6–16 and `report.odm`) is the live family; the Methods Supplement is live only until its Script 43 chapter lands, then frozen by the changelog that lands it; Paper 1, Paper 1 SI, Paper 2, the Supplementary Material, the academic and public summaries and the web-tools documents are numbers-only. Phase 2 (Martin declares the report settled, one D-entry): the report freezes; Paper 1, its SI and Paper 2 go live and refresh from the settled report; summaries and web-tools get one refresh pass. Phase 3 (submitted): everything numbers-only. (2) "Frozen" means prose: tables regenerate through `table_gen`, numbers move through `repoint_refs` and `doc_version_sync`, symbols through `symbol_apply`, and the claims register and `cite_check` keep watching. A sentence in a frozen document changes only through `odt_edit` with `REASON` set to the D-number or changelog id that requires it, and every such write is logged (`working/doc_tier_log.csv`). If a gate is red the sentence changes now, with its reason; if no gate is red it waits for the next phase's refresh pass. (3) The register is `tools/doc_tier.csv`; `doc_tier_lint` gates it in `check_all`; the refusal lives in `odt_edit._write` (and `edit_entries`, `insert_figure`), the one place a byte reaches an ODT. (4) The Methods Supplement is never frozen against method changes: a new or changed step must change its MS chapter, and — once the Script 43 chapter exists — `run_analysis.py` refuses to register a step without a SCRIPT_LEDGER row and an MS chapter mapping, Script 40 grandfathered by name (W116).
...
### D-165 — The DEM bias is measured from DGPS-surveyed wells only, on a pinned vp2 basis — and the dry controls cannot adjudicate a uniform shift

*2026-09-13*

**only DGPS-surveyed ground contributes a pair (`PHASE9_BIAS_GROUND_SOURCES`), the waterline basis is pinned to vp2 (`PHASE9_WATERLINE_BASIS`), and the level frame stays `01_wells_clean.csv`.** `PHASE9_DEM_BIAS_M` = **0.302 m**, from 29 pairs over two dates at MAD 0.083, read back from `W94_11_waterline_vs_well.csv`. 1. **The circularity.** The measurement is median(DEM along the waterline) − (ground + h). Where ground is itself a DEM read at the well, the raster's bias appears on both sides and largely cancels. The two groups separate cleanly: **DGPS 29 pairs at +0.302, MAD 0.083; LiDAR 6 pairs at −0.038, MAD 0.153** — scattered about zero, which is what a cancelled measurement looks like. Excluding them raises the constant from 0.282 to 0.302 and **tightens the scatter by a quarter**. It is the scraped-well exclusion's own logic, already in the code: a well whose ground has CHANGED since the LiDAR cannot measure the LiDAR, and neither can a well whose ground IS the LiDAR. 2. **The basis, which was being decided by accident.** The docstring and the code took the tile polygons wherever they existed, while the committed constant was the vp2 number — so the next re-run would have moved the bias from +0.282 to +0.220, and every slack floor with it, with nothing recording the change. On DGPS pairs the bases differ by **3.2 cm** (+0.302 vp2, +0.270 tiles), more than any other choice here. Three vp2 dates beat one tile date until more tile dates exist; switching is now an edit. 3. **The frame: tested and rejected.** `01_wells_all.csv` adds **two pairs**, both LiDAR-ground, and widens the MAD from 0.111 to 0.141. The reason so few arrive is structural — a pair needs a well at or above ground ON A READ DATE, and the short-record wells D-164 recovered span none of the four. `wells_all` is right for phase 13 and wrong here: **the basis follows what the analysis does with it**, which is D-164's lesson pointing the other way.

**Revisit if** further tile-resolution flood reads are digitised, at which point the basis should be re-decided on more than one date and the 3.2 cm gap between bases explained rather than averaged over; or any well currently carrying a LiDAR ground elevation is DGPS-surveyed, which would add real pairs where the excluded ones were circular; or the slack floors are recalibrated, which D-164 already names as the condition that can invalidate the extent series and which would supersede this constant's role rather than its value.

### D-166 — Readings collected after the pipeline's inputs are frozen reach the flood work through a separate file, never through the pipeline's raw CSVs

*2026-09-13*

**into `data/flood_calibration_levels.csv` and `data/flood_calibration_climate.csv`, read by `tools/warren_flood_prep.py` alone through `_merge_calibration()`, and never into `Newborough_Cleaned_For_Model.csv` or `RAF_Valley_Climate.csv`** (Martin, 2026-09-13: *"I didn't want the March 2026 data added to the general pipeline. I just wanted it for the calibration of the flooding maps, that's why I wanted it in a separate csv to the one the rest of the project uses"*). No numbered script reads these files, no provenance hash covers them, nothing downstream recomputes when they change, and the merge **never overwrites a month the pipeline already has**, so it can only add.

**Revisit if** the pipeline's raw inputs are refreshed to a date at or beyond the calibration file's latest reading, at which point those rows are redundant and should be dropped rather than left to shadow the pipeline; or a numbered script is ever proposed to read these files, which would make them pipeline inputs in everything but name and requires this entry to be defeated first; or the merge is ever changed to overwrite rather than only add, which removes the property that makes it safe.

### D-167 — The dry frames are a calibration of the CLASSIFIER's false positives — but a FORCED read of one does not measure them

*2026-09-14*

(Martin, 2026-09-14) the wet/dry frame pairs were supplied **so that dark features which look like water — bushes, scrub, shadow — could be digitised on the dry frames as FALSE WET FEATURES** and removed. They are a calibration of the classifier's false-positive field. They are **not** a zero-water gate on a hydrological model; `DRY_CONTROL_MIN_RATIO` applied them that way, and that use was invented in this tool and never asked for. **That half of this entry stands.**

**Revisit if** a false-wet mask is built by applying a wet frame's calibrated threshold to a dry frame, at which point the measurement this entry withdraws can be made properly; or the gate itself is changed, which would alter what `--force-read` means everywhere it has been used (2017-03-24 is the other forced read in the corpus, and its status should be reconsidered at the same time).

### D-168 — W94 is retired: at this imagery's resolution and in these bands, water in a slack and slack vegetation are not separable

*2026-09-14*

(Martin, 2026-09-14) **no, and W94 is retired.** *"almost every polygon is a slack ... while the recent slacks are close we can't really discriminate."*

**Revisit if** imagery carrying a near-infrared band reaches this site at a usable resolution — water is unmistakable in NIR and the whole failure above dissolves, because the two confused surfaces then differ in the band being measured; or a field survey of wet extent on a dated frame provides ground truth at polygon level rather than at 50 point wells; or the question changes from AREA to ORDER, which the z index above can already answer.

### D-169 — Flooded extent from imagery is readmitted as a VETTED product: nadir captures on a random control net, a relative darkness read, and a person's tick on every body

*2026-09-15*

(Martin, 2026-09-15: *"its the rule"*) **Three things.** 1. **`data/geo/flood_extent_2021-03-24_vetted.kml` is the accepted extent for 2021-03-24** (1,646 bodies, 106.47 ha) and is `TRUTH_KML`. It is a VETTED product: read by the tool, accepted, edited or rejected body by body by Martin in Google Earth on the imagery, with the decisions kept in `flood_extent_2021-03-24_review.kml`. The superseded `flood_truth_2021-03-24.kml` (62.57 ha) stays as the record of what phases 22–24 were first scored against. 2. **The automatic read of `warren_flood_prep.py` phase 26 is the method for the series:** captures top-down with Terrain OFF at ~2.1 km eye altitude with `georef_random.kml` drawn on; registration on that net with no prior (80–120 pins per frame, ~0.3 px); luminance read onto a 1.5 m ground grid; the threshold **`NADIR_DARK_BELOW_MEDIAN` = 27 below the frame's own median** inside `warren.kml`; bodies **≥ `NADIR_MIN_BODY_M2` = 25**. Pin-tip offsets are a property of the capture and are recorded per file in `data/geo/nadir_captures.csv`. A date's extent may be quoted from the automatic read WITH the dry-date floor stated beside it, or from a vetted read without. 3. **The union of the dry-date reads is the slack-floor INVENTORY, not a mask.** It is never subtracted from a wet read.

**Revisit if** a vetted read of a second wet date disagrees with the automatic read by more than the ~3 % measured here, which would mean the size rule does not generalise; or the dry-date floor on a further dry capture exceeds the 1.6–4.9 % band by a factor of two, which would mean the relative threshold is not stable across renderings; or NIR imagery arrives, at which point the vet becomes unnecessary and D-168's Revisit-if is met directly.

### D-170 — The Sentinel-2 series is an ORDINAL wetness index, not an area; the hindcast is scored for area against the vetted extents and for rank and timing against Sentinel

*2026-09-15*

(Martin, 2026-09-15: "lets amend phase 27") **Four things.** 1. **`data/geo/flood_extent_2020-03-30_vetted.kml` is the accepted extent for 2020-03-30** — 141 bodies, 17.64 ha, on the 86 % of the warren west of the Google Earth mosaic seam (the tile east of it is 11/9/2019 and is excluded, `seam_x_px` in `nadir_captures.csv`). Martin: what he kept is "surface water and damp ground"; a stricter vet would be smaller. 2. **The Sentinel-2 darkness rule is not an area measurement.** At the fixed ratio it reads 43 ha on the floor for 2020-03-25 against 17 ha vetted (IoU 0.34, recall 0.92, precision 0.35; best ratio on that date 0.71 against 0.86 on 2021); the vetted water lies inside the dark class but is a third of it. The floor mask moves this by three points and SWIR by none: damp slack floor and shallow-flooded floor are alike in every Sentinel band at 10–20 m. The 2021 coincidence (108–115 ha read, 100 vetted, IoU 0.71–0.73) is the flooded end, where the whole wet floor is under water. 3. **Sentinel-2 is kept as an ordinal winter-wetness index and a timing check.** Floor-only (phase 29 mask), haze-gated (tile cloud < 25 %, ≥ 95 % clear), the series ranks winter scenes against the median well level at **rho +0.71** (n 36); the flood winters of 2021 and 2026 sit at the top. `wet_floor_pct_masked` is the column; it is never converted to hectares, and it is never quoted as a flooded area. 4. **Phase 27 is amended.** Area is scored against the two vetted extents — 2021-03-24 at the wet end, 2020-03-30 in the middle, each inside its covered area and each stated as one point. Rank and timing are scored against the Sentinel index across the winter scenes since 2016. The wells remain the monthly judge. The "dry-winter baseline" becomes a baseline of the index, not of hectares. Hindcast outputs are `W94_27_*`; overlays (phase 28) are `W94_28_*` — the spec's `W94_28/29` names are renumbered because `W94_29` is the floor mask.

**Revisit if** a third vetted extent at a wetness between the two agrees with the fixed Sentinel rule to within the 2021 IoU (~0.7), which would reopen the hectare calibration; or imagery at ≤ 2 m with a NIR band arrives for a winter date; or the hindcast's rank correlation with the Sentinel index falls below the wells' own (+0.71), which would mean the spread construction, not the index, is at fault.

### D-171 — `01_climate.csv` is the climate record; `03_regional_averages.csv` is a well-indexed frame that also carries climate, and its join is now outer

*2026-09-16*

(Martin, 2026-09-16, asked whether to fix at source, in the flood tools, or both: **"Both"**.) **Two changes and one rule.** 1. **`export_regional_averages()` joins the climate OUTER** (Script 03 ≥ 1.14.0). The frame is indexed on the cluster CENTROIDS, which exist only for months with a dipwell reading, so a left join dropped the climate of every unmeasured month. The file gains two rows whose cluster columns are NaN — which is what "no observed head that month" should look like. 2. **`_pflood_climate()` reads `INT_CLIMATE` (`01_climate.csv`)** for the actual monthly series (`warren_flood_prep` ≥ 1.40.0). Script 01 writes that file complete and it needs no well to exist. **Its climatology is clipped to the span of the well record** (`01_wells_all.csv`), because `01_climate.csv` begins in 1930 and averaging all of it would silently widen the climatology from ~21 years to ~96 and move phase 18's Mode C constants — which is not what this entry is for. 3. **The rule: `03_regional_averages.csv` is a WELL-INDEXED frame that also carries climate, and must not be read as a climate record.** Anything needing a complete monthly climate series reads `01_climate.csv`.

**Revisit if** the cluster centroids are ever exported separately from the climate, at which point the outer join has nothing to do and item 1 is moot; or Script 01's climate output changes its units or column names, which is what the conversion in `_pflood_climate` depends on; or a month arrives with a dipwell reading and no climate, the mirror image of this fault, which the outer join now admits and no gate yet refuses.

### D-172 — The dry end of phase 27's area gate has two sides: flooding nothing is a failure, not a pass

*2026-09-16*

(Martin, 2026-09-16, offered a symmetric band, a recall floor, or leaving it: **the symmetric band**.) **The dry end gains the missing lower bound**, `HINDCAST_AREA_FAIL_DRY_LOW` = 0.5, the same multiplier the wet end already uses. A mode that floods below half the dry vet on its own covered floor is REJECTED. **On the run of 2026-09-16 this rejects Mode C** (0.000 ha against a bar of 8.490 ha) and leaves Mode R passing (32.974 ha).

**Revisit if** a third vetted extent arrives at an intermediate wetness (D-170's own Revisit-if), which would let the band be set from three points rather than two; or the spread construction is replaced, at which point the band is being applied to a different object.

### D-173 — The two recording breaks are September 2011 and January 2017, they hold 46 % of all interpolation, and the published coefficients do not depend on them

*2026-09-16*

(Martin, 2026-09-16: *"run the exclude_interpolated refit"*, then *"write the D entry"*.) **Four things, three settled and one deferred.** 1. **The breaks are 2011-09 and 2017-01.** In the raw sheet the dated column for each round EXISTS and is EMPTY — `01/10/2011` and `01/02/2017`, zero wells in each — and Script 01 then INTERPOLATED the whole network: **53 and 74 wells** against a background of one to four elsewhere. Martin's count of two was right; this entry records which two. 2. **The canonical cluster coefficient table STANDS, unchanged.** The `exclude_interpolated=True` sensitivity that Methods Supplement §S.3 describes and states it does not run has now been run (`tools/interpolated_sensitivity.py`). Masking the two breaks and nothing else moves every coefficient by **under about 2 % of its own value** — β₁ +0.25 to +0.82 %, β₂ −0.38 to −2.11 %, β₃ +0.89 to +2.06 % — with the β₂ and β₃ rank orders unchanged and every sign preserved. **No published number is corrected by this entry.** 3. **The sensitivity is now a tool, not a paragraph.** It is kept as `tools/interpolated_sensitivity.py`, and it refuses to report unless its rebuilt centroids reproduce `03_regional_averages.csv` and its baseline β reproduce the committed `03_03_cluster_mechanistic_coefficients.csv` — measured at 2.2e-16 m and 1.3e-14 on the first run — so that a future session cannot quote it against the wrong baseline. 4. **DEFERRED, and owed: whether §S.1/§S.3 should NAME the two months.** Not decided here. The Supplement discloses the policy fully — `limit=1`, the provenance file, its totals, and that the canonical fit runs `exclude_interpolated=False` — but **127 of the 275 interpolated cells (46.2 %) are these two months**, and neither is named anywhere in the document corpus. That is a transparency question for Martin, not a correctness one, and the evidence for whatever sentence he wants is in `NRG_interpolated_sensitivity.csv`.
...
### D-178 — Flooded area is modelled as two Sentinel-2 NIR classes driven by the median well level — a whole-warren AREA model, hands-free; the vetted extents are its ground truth on meaning, not its calibration

*2026-09-16*

(Martin, 2026-09-16: "I think this is good enough"; "fit the model to the NIR dark and dark ring independent of my vets"; "I think we now have the model … let's write it up") **Yes, as an AREA model with two classes, and no more than that.** 1. **Classes.** On the phase 29 floor inside `warren.kml`, per scene, with B8 normalised by its own clear-floor median: **open water** = B8 ≤ `NIR_BLACK_RATIO` (0.5) × median; **wet floor** = 0.5 < B8 ≤ `NIR_DARK_RATIO` (0.8) × median; both scaled to the whole warren by the clear fraction. B8 alone maximises the black class; every blend with B4 or B11 lowers it. 2. **Curves.** Fitted on the 41 usable winter scenes 2016–2026 (haze gate 25 %/95 %; 2016-12-26 excluded as the 12° sun scene), level interpolated to the scene date from the bracketing month-end readings: **open water = 23.0·exp(4.42·h) ha** (ρ +0.81, σ ×/÷ 1.96); **wet floor = 55.5·exp(0.94·h) ha** (ρ +0.85, σ ×/÷ 1.15); h the median well level in metres, 0 at ground; fitted range −0.80 to +0.03. Wetting and drying limbs do not separate (drying 7 % below, wetting 2 % above the common curve). The parameters live in `working/updates/W94_27_wet_area_model.csv`, written by `sentinel_wet_floor.py --two-class`; quote from there, never from this entry. 3. **Drive.** The SSM's monthly median level (phase 27 recurrence) through the two equations, 2005-10 to 2026-03. Against the same curves on the observed level over 238 months: wet floor median ×0.96, 68 % range ×0.85–1.14 (Mode R); open water median ×0.84, ×0.47–1.87 — the open-water curve is steep, so a 0.1 m level error is 55 % of area. Both modes rank the winters against the Sentinel index at +0.75, the wells' own figure and D-170's bar. 4. **The map is an illustration, not a product (D-177 stands).** Each floor cell's switching level for each class is read off the scene stack (`W94_27_cell_thresholds.npz`), so a monthly rendering under the SSM level shows where Sentinel has seen the classes appear at that level. Totals agree with the curves mid-range and are coarse at both ends; sub-cell water is absent; thresholds are known to the scene spacing. 5. **What the vets are for.** `flood_extent_2021-03-24_vetted.kml` and `_2020-03-30_vetted.kml` enter no fit. They say what the classes mean: the vetted extent (surface water AND damp ground, D-170) lies between the two curves on both dates — 20 ha (2020) and 108 ha (2021) against 4 and 23 ha of open water and 36 and 56 ha of wet floor. Open water at 10 m is the pool core of a flood; the ring is shallow flooded and saturated sward; at the wet end Google Earth at 1.5 m sees about a third more water than both classes together.

**Revisit if** a third vetted extent falls outside the band between the two curves at its level; a refit on a later decade moves a parameter by more than its σ; the SSM's level skill changes (the curves inherit it); or a summer pair of curves is needed — the winter curves applied to summer months say what the water table would give, not what Sentinel sees.

### D-179 — The Sentinel wet-area line joins the pipeline as PHASE 18; its scenes are LISTED, not bundled

*2026-09-17*

(Martin, 2026-09-17: *"It should all be part of the pipeline. why hasnt it been included."*; *"we should list the sentinel inputs, others rerunning the analysis will have to pull their own copies from copernicus. building the film is an output step separate."*) **Yes — as a new PHASE 18, all display tier.** 1. **Steps.** `45_wet_area_model.py` fits the two curves from the committed scene series and runs phase 27's SSM level through them; `46_wet_area_feed.py` assembles the curves, the per-cell switching levels and the SSM history into `living/wet_area_model.json`; `47_hindcast_film.py` (the century film, renamed from Script 45 by the Stage-2 renumber so filenames ascend with run order) is a separate display/output step, on demand only (`--hindcast-film`). `total_registered` 54 → 57, `total_phases` 17 → 18. 2. **Inputs, listed not bundled.** The committed input of record is `data/sentinel/sentinel_scene_manifest.csv` — Copernicus scene identifiers plus derived-B8-array checksums — not the scenes. A re-runner pulls their own from Copernicus by STAC id and verifies against the committed derived series (`two_class_series.csv`). Five committed inputs live under `data/sentinel/` (series, cell thresholds, manifest, well_fit, hindcast_monthly) and are hashed by `input_provenance`. No registered step touches the network: the fit reads the committed series. 3. **Tier.** Display, not analytical (D-177: an illustration of the area model on the Sentinel grid, not a flood map), so `by_tier.analytical_toplevel` is unchanged at 43.

**Revisit if** the manifest + checksum reproduction is shown not to reproduce the fit from a clean Copernicus pull.

### D-180 — The wet-area validation is written up as report §4.8.5 and belongs to Paper 1; the GE-ordinal cross-check (RB-18) is not cited

*2026-09-17*

(Martin, 2026-09-17: *"I still think the sentinel analysis belongs with paper 1"*; *"drop rb18, if we include it then it needs to be in the pipeline"*) 1. **Report placement.** A new results subsection **§4.8.5 "Slack-floor wet area: an independent spatial check on the modelled water table"**, immediately after §4.8.4 (the Ellenberg-F vegetation cross-validation), carrying two figures: the area–level fit (Figure 47, `45_01`) and the SSM-through-curves reproduction (Figure 48, `45_02`, R² shown per class per mode). Mid-chapter placement was taken knowing it renumbers figures 47–78 → 49–80; executed by the W96/D-141 two-pass placeholder method (fresh `renumber_plan_wetarea.csv`; `repoint_refs` shifted 275 typed references, 0 left for review). 2. **Framing.** Led as the report's only SPATIAL, out-of-sample validation of the modelled water table — the wells (§3.4.2) validate it in time at points and the vegetation record (§4.8.4) against ecology at points, neither testing the wetted extent between the wells. Aggregate over the warren floor, not per-slack (D-168); warren only, forest excluded; an illustration, not a flood map (D-177). 3. **Paper.** Belongs to **Paper 1**, as the spatial validation of the water table (revising an earlier working assumption of Paper 2 / standalone); the same area curves may be reused for any management framing in Paper 2. 4. **RB-18 not cited.** The Google-Earth ordinal wetness series is NOT included: to be cited it would have to be registered and reproducible in the pipeline, which it is not (ordinal-only, n=14, GE imagery, unadopted). The Sentinel extent check (Figure 48) is the stronger, in-pipeline corroboration and stands alone.
...
### D-188 — The abstract compares forestry scenarios with climate projections as comparable magnitudes — neither is read as negligible against the other

*2026-09-21*

(reworded 2026-09-20 under ruling 2a, flagged for confirmation; Martin, 2026-09-21: *"the 2a wording is confirmed"*) The abstract reads: *"The forestry-scenario magnitudes are comparable to the projected climate-driven changes at the same clusters (UKCP18 at C4: −6.2 mm w.e./month by the 2050s, −10.9 mm by the 2080s), so neither canopy management nor climate trajectory can be read as negligible against the other at this site."* The forestry figures are Script 19's annual `19_scenario_summary` row (`we_mean_mm`, season = annual); the climate figures are `09b_04_scenario_comparison`'s UKCP18 annual rows at C4; both are registered and drift-checked. The 09b summer bookends are not set against annual forestry figures anywhere in the report.

**Revisit if** the UKCP18 projection rows or the Script 19 annual scenarios move by more than the quoted precision on a rerun (cite_check will show it), or a projection scenario other than RCP8.5 is adopted as the report's climate reference.

### D-189 — Spring is the readings dated March to May — the bucketed frame's months 2-4 — for every spring quantity in the pipeline

*2026-09-21*

(Martin, 2026-09-21: *"do (b)"* … *"B ii"*) `MSL_SPRING_MONTHS = (2, 3, 4)` — the readings dated March to May — for EVERY consumer of the constant: MSL5 (Scripts 26, 26b, 26c, 19's ΔMSL5 row, the living MSL5 feed via `living/msl_common.py` and `living/newborough_report.py`), the spring transfer functions (11, 14) and the spring-mean robustness analyses (09c/10d/10l via `clearfell_common`, 25, 32, 33, 35, 36, 38, `envelope_metric`). One meaning of spring, the literature's. Curreli et al. (2013) checked (`literature/curreli.pdf`): monthly manual readings, "averages for spring months (March, April, May and June)" — labelled by date, the same convention. Every spring quantity regenerates on the next full run; the documents' spring numbers are refreshed from the committed CSVs afterwards, and "March–May" in prose stays true by reading date while any "end-of-March to end-of-May levels" wording is stale. The reproduction claim leaves the abstract (it was never an independent check); §4.8.3 states what the comparison is, and Supplementary Table S7.2 tabulates the per-quadrat datum offsets so a level here can be set against a quadrat-referenced one.

**Revisit if** Script 01's bucketing convention changes (the month labels move with it, and this tuple must move back); a later vegetation study at the site publishes spring levels under end-of-month labelling; or Curreli's thresholds are shown to rest on a spring definition other than reading-dated months — note her SD15b/SD16 values (−0.61, −0.98 m) are 4-year averages of the annual MINIMUM (her Table 4), which the report applies to summer minima correctly and to MSL5 in §4.8.3 by extension (see the follow-up finding of 2026-09-21).

### D-190 — The Curreli SD15b/SD16 values are applied to the quantity they are means of — the rolling annual minimum — and not to MSL5

*2026-09-21*

(Martin, 2026-09-21: *"rebuild Table 20 and Fig 44 on the annual minimum"*; window: *"Should we do both, does 5 yrs refine?"* → both computed, four presented.) Script 26 (1.13.0) computes the per-well annual minimum over the same hydrological year as MSL (`MSL_HYDRO_YEAR_START_MONTH`), admitted only when every `SUMMER_MINIMUM_MONTHS` month is in the QC record and the well reports the year's closing May level, and rolls it over `config.CURRELI_MIN_WINDOW_YEARS` = 4 (Curreli's own averaging period) with every year required, as MSL5's window is. The thresholds are counted on that series only (`26_curreli_min_cluster_threshold_summary.csv`, Table 20's source); the cluster-mean MSL5 at the same window-end rides along as a reference column with no threshold count. Each length in `CURRELI_MIN_WINDOW_SENSITIVITY_YEARS` (5) is emitted in the same files. Figure 44 becomes two panels (Script 26c 1.2.0): (a) MSL5 with no threshold lines, (b) the four-year mean annual minimum against SD15b/SD16 with the SD16 zone shaded. `MSL5_EXCLUDED_WELLS` is not applied to the minimum series: D-146 scopes it to the MSL5 analysis, and an observed trough is not an SSM product. Table 20's columns: cluster, n wells, MSL5, four-year mean annual minimum, window-ends below SD15b, below SD16.

**Revisit if** a threshold for MSL, or for the five-year MSL, is published for these communities (van Willegen's stated next step) — then MSL5 gets its own reference values and Figure 44(a) its own lines; or Curreli's Table 4 is shown to be something other than four-year means of the annual minimum; or `SUMMER_MINIMUM_MONTHS` changes (the admission rule follows it).

### D-191 — When the pipeline moves a finding, the report's conclusion moves with it — no claim is preserved by wording

*2026-09-22*

(Martin, 2026-09-22: *"If the pipeline says the numbers have changed, then the report has to change to match the conclusions."*) The conclusions follow the committed outputs. The driver-footing passages now read as a bounded confirmation — each driver is separable from zero in the window where it acts, at a scale consistent with unity, without its amplitude resolved; the common-mode intercept remains the larger term in every window. The transect passages will state the decomposition the committed rows give once Script 38 1.7.0's emits (the two anchors' absolute slopes, the leave-one-out range) are run on the L14, and Scripts 32 and 33's new cluster summaries do the same for §4.12. No sentence is kept because it was the earlier finding.

**Revisit if** a later run moves either finding back — the passages follow the file again, in the same way; or a claim is shown to rest on a quantity the pipeline does not emit, in which case the emit is added first (as here for Scripts 32, 33 and 38) and the sentence waits for the committed value.

### D-192 — Per-well recession constants at slow wells are reported on the full record; the comparison window stays for ranking and the atlas

*2026-09-24*

(Martin, 2026-09-24: *"Agree t½ at slow wells go to the full record."*) The split is by what the analysis is for. Quantities that COMPARE or RANK wells — the coefficient atlas (RB-05), the SSM-vs-TLM benchmark (RB-06), the per-well coefficient tables and any pooled or ranked statistic — stay on the comparison window, as D-002 set. Quantities that ask what a well's OWN recession is — t½ = ln(2)/β₃ per well, the identifiability diagnostics, the drawdown reach that consumes a per-well β₃ — are reported on the full record, named as such, with the window value beside it where the document already shows one. The first consumers are §4.9.3 and Figure 53; `record_basis.csv` RB-04 splits into RB-04 (window; comparison) and RB-04b (full record; recession constants).

**Revisit if** an analysis needs per-well t½ ranked across wells — then it uses the window, and says so; or Script 48's identifiability rule is changed, which moves which wells the full-record value is defensible at.

### D-193 — Chapter 4 reports and Chapter 5 interprets: the register is a property of the section, and a result that lives only in the Discussion moves to Results

*2026-09-24*

(Martin, 2026-09-24: *"4.10.3 reads largely as discussion. we need to have a results or discussion prose check too"*; then *"go ahead with all of the proposed edits"*.) A Chapter 4 paragraph states what was measured, fitted, counted or rendered — the number, its sign and uncertainty, what a figure shows, which method produced it — and points ("Section 5.n.m") to where it is read; mechanism, driver-weighing, implication, literature and alternative readings are Chapter 5's. The rule is applied by move, not by deletion: an argument found in §4 goes to the §5 subsection that owns it, transferred where §5 did not already carry it (the C5 anomaly's three candidates to §5.7.2, the far-field constant to §5.7.7, the transect's meaning to §5.7.3, the propagation bound to §5.4.3, the C3 gradient's single control to §5.1.1); a result found only in §5 goes to §4 — the hindcasts into a new §4.14 (with the Ranwell figure), the C4 diagnostic into §4.2.2 with its numbers re-read from Script 30, the crossing year and its figure into §4.8.1/§4.8.3, the footing table into §4.12.1 as Table 25 — and §5 keeps a pointer and its reading. A caveat that bounds a number, a statement of method choice and a pointer to §5 are not interpretation and stay.
...
### D-199 — Figure 55 (aquifer diagnostic synthesis) is dropped from the report and is not to be used in Paper 1

*2026-09-25*

(Martin, 2026-09-25: "drop fig 55"; "make a note not to include fig 55 in paper 1".) The figure, its §4.9.3 paragraph, the §5.1 paragraph, the Conclusion 1 sentence and the report8 methods paragraph are removed, and the report8 heading becomes "Drainage Timescales". Figures 56–83 become 55–82 (`tools/renumber_plan_drop55.csv`). Script 18 still draws the figure as a diagnostic output. The Methods Supplement says the report does not use it. Paper 1 (on hold, D-194) must not carry it as Figure 15, nor carry the "third, fully independent diagnostic" sentence that cites it.

**Revisit if** a diagnostic independent of the clustering inputs (not β₃ and not a TLM-relative skill) separates the centroids; Script 18 would then need a spec for a replacement figure.

### D-200 — No off-site scraping drawdown is resolved at WMC3: the raw era steps are within the gap's month-to-month variability

*2026-09-25*

(Martin, 2026-09-25: "I approve the fig 30 edits and the number updates".) No. Script 10m 1.3.0 gives each era step a Newey-West SE (12 lags, eras in quadrature), t and a moving-break placebo share. On the committed record: 2015 −46 ± 63 mm (t −0.73, placebo 0.73), clearfell +40 ± 57 (t 0.70, 0.78), 2023 −53 ± 68 (t −0.78, 0.65). The 2015 step also depends on a baseline set by the 2010–2012 excursion. WMC3's climate-corrected scraping-era step on the whole-array map (10b) is within the array's spread (−106 mm now, −46 before, when it equalled the array median). The report now states that no off-site level effect of scraping is resolved, in agreement with the Δβ₃ null. The neighbour drawdown of the drain-cone model stays a modelled scenario that the record can neither confirm nor exclude. The ANCOVA scraping covariate is kept, on the CEH36 level shift and as a precaution. The 10m legend label "C4 Impact well" is corrected: WMC3 is in C3.

**Revisit if** a longer post-2023 record, or a climate-corrected single-well test at WMC3, gives a step clearly outside its variability; or a second long-record uphill well comes into the design.

---

200 decisions. Generated by `tools/build_public_decisions.py`.
