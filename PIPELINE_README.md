...
**52 registered pipeline steps across 17 phases** (canonical count: `pipeline_manifest.json`). By tier: 42 analytical, 4 display/utility, 6 diagnostic. By execution: 49 in a default pass, 3 opt-in. The two breakdowns partition the same 52 steps on different axes and are not additive with one another.
