...
**Other.**
  ...
  - Categorical bin edges (1.5 and 2.5) are operational choices, not derived from a data-driven break: the m_P < 1.5 boundary is taken directly from Conclusion 4 text; the marginal-vs-unreachable boundary at 2.5 matches the abstract's wet-winter framing.
...
- `25_01_panel_fit_parameters.csv` — all 6 fits (3 specs × 2 forms): δ₀, L_cg, c, SEs, 95% CIs, AIC
...
- `25_11_matched_window_sensitivity.csv` — **reported only, not adopted**: the headline specification refitted on the long-record well subset and on its complement. Nothing downstream reads it; δ₀ and L_cg in `25_01` are unchanged
