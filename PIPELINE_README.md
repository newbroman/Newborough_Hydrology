...
- `19_scenario_summary.csv` — per (scenario, season, cluster) Δh / Δstorage summary; under v2.8.0 includes a new `season="msl5"` block (6 rows per scenario: 5 clusters + a well-count-weighted SITE row). Consumed by Script 26c (step 32) for the §4.12.1 Δsummer-minimum bars.
...
**Other.**
  ...
  - The viewer's ΔMSL5 row uses *per-well* aggregated β (mean of well-level OLS within each cluster), consistent with the rest of the viewer; this differs from Script 26b's *cluster-centroid* OLS by 0.5–3.7 mm per cluster per UKCP18 scenario. Both are defensible summaries of the same SSM. The viewer row matches `26b_msl5_ukcp18_projection_summary_perwell.csv` (per-well-aggregated reference) to ≤0.5 mm; the canonical numbers in §3.7.5 / §4.8.3 / §4.12.1 remain anchored to the centroid-fitted `26b_msl5_ukcp18_projection_summary.csv` (consumed by Script 26c).
...
- `26b_msl5_ukcp18_projection_summary.csv` — **canonical centroid-fitted** per (cluster, scenario) summary: β₁, β₂, spring Δh mean, observed window mean, perturbed window mean, mean shift. β fitted by OLS on the cluster-centroid hydrograph (`03_regional_averages.csv`). The reference source for the §3.7.5 / §4.8.3 / §4.12.1 report numbers and the §S.18b chapter; consumed by Script 26c.
...
- `fig_msl5_vs_summer_min_projection.png` — two-panel ΔMSL5 vs Δsummer-minimum contrast (§4.12.1)
