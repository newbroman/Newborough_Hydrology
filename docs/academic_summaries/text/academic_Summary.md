<!-- GENERATED MIRROR of docs/academic_summaries/academic_Summary_v1_22.odt — do not edit. source-sha256=174d444b7b65258f pandoc=3.1.3 -->
...
A 21-year dipwell monitoring dataset (2005--2026) covering 88 wells (66 reference, 22 extended) across Newborough Warren SAC was analysed using a 58-step reproducible Python pipeline. Monthly water levels were combined with RAF Valley climate data (rainfall, Thornthwaite PET). The core analytical tool is a state-space model (SSM) fitted independently to each well, estimating three physical coefficients: recharge sensitivity (β₁), atmospheric draw (β₂) and drainage (β₃). SSM performance was benchmarked against a transfer function lacking the drainage term; the SSM achieved positive Nash--Sutcliffe efficiency in iterative forecast mode at 65 of 66 reference wells (vs 44 of 66 for the transfer function).
