...
*run_analysis.py*\*\* --- 52 registered steps across 17 phases\*\* (canonical count: committed *outputs/pipeline_manifest.json*, emitted on every run --- cite that, not this line, if the two ever disagree). Those 52 steps are classified two independent ways. By tier: 42 analytical, 4 display/utility (Scripts 26c, 09f, 09g, 27) and 6 diagnostic. By execution: 49 run in a default pass, and 3 --- Scripts 24b, 31 and 31b, the Phase 16 remainder --- run only with *\--with-supplementary*. The two classifications each account for the same 52 steps and are not additive with one another.
  ...
  15      36--41    Differential change, climate envelope, per-well amplification, absolute climate-removed trend, driver validation, Part B comparative footing (32, 33, 35, 36, 37, 37b --- all analytical-default)   §S.20
  16      42--49    Standalone diagnostics: 24b, 31, 31b (opt-in); 34, 38 (analytical-default); and 39, 40, 41 --- CCW hindcast, shoreline retreat, canopy cover --- analytical-default                                 §S.21--§S.23
...
The supplement covers the registered pipeline steps (Phases 1--17) in script-by-script chapters. The registered total is not typed here: it is \`total_registered\` in the committed pipeline_manifest.json, which run_analysis.py guards against a recount, and the manifest counts the pipeline on two independent axes --- execution and tier --- which must not be mixed. Script 40 is documented in §S.22b and Script 41 in §S.23. Four of those steps are display/utility rather than analytical: Script 26c (MSL5 report-format figures, step 32, §S.18c), Script 09f (spatial-reach synthesis, step 50, §S.15c), Script 09g (mechanism diagrams, step 51, §S.15d), and Script 27 (greyscale, step 52, Appendix A). Each chapter addresses seven concerns: motivation, inputs, methodology, site-specific choices and rationale, outputs, limitations and known caveats, and where the result appears in the report. Some chapters are short (figure-only or post-processing scripts); some are longer (Scripts 01, 02, 03, 10 suite, 17, 21, 25, 26). Total document length is approximately 260 pages.
...
Several dicts in the codebase are physical-data-keyed-by-integer and need verification under any future partition change: *SUMMER_TRENDS* and *FLOOD_FREQ* in Script 16 --- both since REMOVED, along with *RESIDUAL_PCT_SE*, when D-021 made the water-balance partition Sy-free; the only survivor is *CLUSTER_PEAK_MONTH* in Script 11. These dicts are now read dynamically from upstream pipeline CSVs where possible (*03_cluster_peak_months.csv* for the peak months, *16_water_bal_table.csv* for residuals), but any remaining hardcoded entries should be revisited if the partition changes.
...
-   **Script 11c** (*11c_pflood_achievability.py*) --- Per-well P_flood achievability categorical priority map. Operationalises Conclusion 4's m_P \< 1.5 priority criterion. Step 13/52, Phase 3. Routed from the 2026-05-29 post-review cascade. Documented in S.9.3.
-   **Script 14b** (*14b_year_of_crossing.py*) --- Bootstrap year-of-crossing for Curreli (2013) ecological thresholds. Step 16/52, Phase 4. Routed from the 2026-05-29 post-review cascade. Documented in S.8.5.
-   **Script 25** (*25_coastal_gradient.py*) --- Coastal-retreat gradient analysis. Step 26/52, Phase 11. Documented in S.15.
-   **Script 26** (*26_van_willegen_msl.py*) --- Van Willegen 2025 observational 5-year MSL aggregation, plus (v1.3.2) the equilibrium wetness index and its Ellenberg-F cross-validation. Step 30/52, Phase 13. Documented in S.18.
-   **Script 26b** (*26b_van_willegen_msl_projections.py*) --- UKCP18 RCP8.5 MSL5 climate projections (Tool B). Step 31/52, Phase 13. Documented in S.18b.
-   **Script 26c** (*26c_msl5_report_figures.py*) --- MSL5 report-format figures for §4.8.3 and §4.13.1 of the main report. Display-only companion to Scripts 26 and 26b; reads only canonical outputs from Scripts 26, 26b, and 19; no recomputation. Step 32/52, Phase 13. Display/utility tier, not analytical. Documented in S.18c.
-   **Script 28** (*28_c3_detrend_check.py*) --- C3 detrend check; quantitative validation of the aquifer-architecture framing. Step 33/52, Phase 14. Routed from the 2026-05-29 post-review cascade. Documented in S.19.1.
-   **Script 29** (*29_c3_within_variance_check.py*) --- Within-C3 variance attribution. Step 34/52, Phase 14. Routed from the 2026-05-29 post-review cascade. Documented in S.19.2.
-   **Script 30** (*30_c4_drainage_identifiability.py*) --- C4 drainage identifiability diagnostic and reported sensitivities. Step 35/52, Phase 14. Added 2026-06-23 as a constrained-β₃ triangulation sensitivity; superseded by the identifiability diagnostic 2026-07-24. Documented in S.19.3.
-   **Script 32** (*32_differential_movement.py*) --- Secular differential water-table movement; report Figure 64. Step 36/52, Phase 15. Documented in S.20.1.
-   **Script 33** (*33_envelope_amplification.py*) --- Climate-swing amplification and dry-year spring floor; report Figures 66 and 67. Step 37/52, Phase 15. Documented in S.20.2.
-   **Script 35** (*35_per_well_amplification.py*) --- Per-well climate-sensitivity coefficient (discrete companion to the Figure 66 surface). Step 38/52, Phase 15. Documented in S.20.3.
-   **Script 36** (*36_absolute_climate_trend.py*) --- Absolute climate-removed per-well secular trend map. Step 39/52, Phase 15. **Analytical tier** (promoted 2026-07-13, Task E). Documented in S.20.4.
-   **Script 37** (*37_driver_validation.py*) --- Predicted-vs-observed driver-change validation (scatter + residual map). Step 40/52, Phase 15. **Analytical tier** (promoted 2026-07-13, Task E). Documented in S.20.5.
-   **Script 37b** (*37b_driver_footing.py*) --- Part B: comparative driver footing --- forest, scrape, and coastal-retreat effects on common currencies (peak local head change, area-integrated volume, ecological-threshold crossings). Step 41/52, Phase 15. **Analytical tier** (promoted 2026-07-13, Task E). Documented in S.20.6.
-   **Script 24b** (*24b_residual_climatology.py*) --- Cluster-stratified residual climatology; supplementary diagnostic. Step 42/52, Phase 16. **Opt-in diagnostic tier.** Documented in S.21.1.
-   **Script 31** (*31_cluster_validation.py*) --- Independent k=5 partition validation; supplementary diagnostic. Step 43/52, Phase 16. **Opt-in diagnostic tier.** Documented in S.21.2.
-   **Script 31b** (*31b_separation_vs_recoverability.py*) --- Cluster separation versus recoverability; supplementary diagnostic. Step 44/52, Phase 16. **Opt-in diagnostic tier.** Documented in S.21.3.
-   **Script 34** (*34_window_sensitivity.py*) --- MSL5 two-window sensitivity demonstration. Step 45/52, Phase 16. **Analytical tier** (promoted 2026-07-13, Task E). Documented in S.21.4.
-   **Script 38** (*38_coastal_transect.py*) --- Coast-to-inland MAM transect; observational delta_0 (coastal-retreat) diagnostic distinguishing a growing (erosion-consistent) from a static (substrate-geometry) coast-inland head gradient. Step 46/52, Phase 16. **Analytical tier** (promoted 2026-07-13, Task E; wired into *run_analysis.py* 2026-07-08, previously standalone). Documented in S.21.5.
-   **Script 09f** (*09f_management_effects.py*) --- Management-interventions-versus-coastal-retreat spatial-reach synthesis figure. Step 50/52, Phase 17. Display/utility tier, not analytical. Documented in §S.15c.
-   **Script 09g** (*09g_mechanism_diagrams.py*) --- Mechanism-diagram suite: the §5.8 combined schematic grid and the standalone coastal-vs-climate reach figure. Step 51/52, Phase 17. Display/utility tier, not analytical. Reads only committed outputs of Scripts 09f, 10m and 10a; no recomputation. Documented in §S.15d.
-   **Script 27** (*27_greyscale_figures.py*) --- Post-pipeline greyscale figure-rendering utility. Step 52/52, Phase 17. Display/utility tier, not analytical. Documented in Appendix A.
...
-   **Spatial constants and distance functions** for scraping-propagation modelling (the L_s = 300 m exponential weight).
...
Two consequences follow, and both are deliberate. The cluster-centroid coefficients of Table 3 are fitted on each cluster\'s full record, while the per-well coefficients behind the coefficient atlas are fitted on an equal trailing window; the two are therefore on different bases, and Table 3 prints both so the difference is visible rather than implied. The benchmark of §S.5 refits both models on the same capped frame, which is the one place an equal window is load-bearing rather than conventional.
  ...
  SSM vs TLM benchmark (08)          66                                 trailing 100 months, both models       scored over the same 100 months; the CEH7/8/37 quality exclusion sits upstream of the partition and removes no reference well
...
The script replaces a former *07_boundary_intercept.py* that ran a Model A vs Model B intercept audit. Under the displacement formulation (F.3), the headline SSM fits well across all clusters (Script 08 median iterative NSE = 0.72), so the intercept audit added little beyond what direct coefficient mapping reveals. The current Script 07 is a pure visualization: no refit, no statistical test, no Model B fit. Per-well coefficients are inherited unchanged from *03_master_data.csv*.
...
-   A **dual-panel showdown** for CEH6 --- top panel is one-step diagnostic fit, bottom is iterative forecasting. CEH6 was chosen as the showcase because it produces a clear visual contrast between the two models without being the most extreme case in the network. The TLM drifts to NSE = −1.89; the SSM holds at NSE = +0.67.
...
Converting a volumetric bar to a water-table **head** change (mm), as used for the Curreli et al. (2013) thresholds, requires dividing by an appropriate specific yield; this is not straightforward, because the WTF specific yields are reliable for ranking clusters but high in absolute terms (§S.12, Appendix B --- the coarse Sy gradient is robust across three methodologically distinct estimators; the **only** downstream quantity that takes an absolute Sy is the Figure 57 reach λ = √(Kb/(Sy·β₃)), used as an order-of-magnitude input; the storage--drainage index Sy/β₃ is a diagnostic and is no longer relied on in the discussion; the residence time 1/β₃ is Sy-free), so any head equivalent is approximate. The figures therefore carry a caption note to that effect rather than presenting a head axis.
...
-   **S.12** --- Script 18 per-well WTF Sy (*18_wtf_01_well_sy_estimates.csv*, Sy_median), consumed by 09d (CEH36's own-well Sy) and, as a cluster median, by *load_cluster_params()* (D-038).
...
**Methodology.** 10a fits the same ANCOVA model across four control tiers crossed with two target zones (Impact, Edge). Three are the primary counterfactuals --- Forest, Climate and Combined --- yielding the six primary ANCOVA results per intervention; a fourth, supplementary far-field tier is carried alongside them as a coastal diagnostic (below), so the comparison table holds eight rows in all. The three primary controls are:
...
-   **Edge (four wells):** +30 mm, 95 % CI \[−21, +80\], p = 0.253, R² = 0.457, N = 159.

The Climate-control runs give a small non-significant Impact step (−15 mm, p = 0.562) and an Edge step of −107 mm, 95 % CI \[−167, −46\], p \< 0.001. Under the published coastal design the coastal-drift correction is fitted in every contrast and is significant in seven of the eight --- from p \< 0.001 at the Forest tiers to p = 0.030 at Combined Impact --- the exception being Combined Edge, where the amplitude is indistinguishable from zero (ξ = −0.01, p = 0.99); the retired easting design instead dropped it from the Forest runs by a 200 m easting-range rule (see *Site-specific choices* below). The Combined-control runs give an Impact step of +70 mm (p \< 0.001) and an Edge step of −20 mm (p = 0.128), pooling Forest, Coastal, and Climate controls; the Combined Impact estimate is tighter than either Forest or Climate alone because of the larger control sample. Each (control, zone) pair is exported as a row in *10a_01_ancova_comparison_table.csv*. Full coefficients, coastal-drift effects, and the per-pair model diagnostics are in *10a_02_ancova_full_coefficients.csv*. The corrected BACI displacement timeseries --- observed minus the fitted climate and easting effects --- is exported in *10a_03_baci_timeseries.csv* and underlies the §4.6 figures.

The Climate-control Edge step is small in magnitude but its sign is opposite to the Forest-control Edge step. The two controls answer different counterfactual questions: the Forest control isolates the clearfell signal against canopy-matched substrate-matched wells, while the Climate control compares the forest-edge wells against open-dune climate-baseline wells that sit further from the coast. The negative Climate-Edge step reflects residual coastal-zone drift between the C3 climate wells, which sit further west and away from the coast, and the four C4-edge wells, which sit closer to the receding coastline. The coastal-drift correction in the ANCOVA absorbs progressive divergence between the impact wells and their controls and is significant at p \< 0.001 in the Climate runs, but it does not remove the residual zone-level drift between two well clusters at different distances from a coastline retreating episodically through the post-felling era (Script 25 documents the gradient in detail).
...
The annual Forest × Impact step is the durable headline of the clearfell analysis. Its sample size (163 months) carries the inference. The Jun--Sep subset is only 52 months and the same specification on that subset does not reach significance. The summer signal corroborates the direction of the annual step but is not retained as a separate statistical claim. The arithmetic-construct fallback (BACI_ANNUAL × 1.5034) that was retained transiently as a Script 21 fallback and removed in Script 21 v1.0.3 / 10a v1.3.0 is therefore not replaced by a summer-specific scaling.

**Curvature (CWB² × felling) sensitivity variant.** The headline ANCOVA fits a *linear* CWB × felling interaction, which tests whether felling changed the linear sensitivity of the water table to climate forcing --- the canopy-buffering hypothesis in its simplest form. That interaction is non-significant at both Forest-control zones (Impact p = 0.63, Edge p = 0.35): on a whole-range linear slope, the pre- and post-felling climate sensitivities are not distinguishable. A linear slope is, however, a blunt instrument for a hypothesis about climate *extremes* --- a buffering effect concentrated at the wet or dry tails can average to near-zero across the full CWB range. 10a v1.4.0 therefore also fits a non-linear extension on the full-data Forest-control Impact and Edge frames: the headline design matrix plus a centred CWB² main effect and a CWB² × felling interaction. The CWB² × felling term is negative at both zones but significant at neither (Impact −1.55 × 10⁻⁶ m mm⁻², p = 0.17; Edge −9.12 × 10⁻⁷ m mm⁻², p = 0.28). The curvature model does not improve on the linear model at Impact (ΔAIC +0.6) and improves on it modestly at Edge (ΔAIC −2.3); the joint F-test of the two added curvature terms is non-significant at Impact (p = 0.20) and marginal at Edge (p = 0.051). The CWB² *main* effect is non-significant at both zones --- the pre-felling relationship is linear; only the post-felling relationship bends. The negative interaction sign means the post-felling response is concave in CWB, i.e. the felling uplift is larger in dry (low-CWB) conditions. This is a reported sensitivity variant only: the headline clearfell step and the linear model are unchanged, and the curvature coefficients, the re-referenced clearfell step (which differs from the headline step because the step is evaluated at mean CWB under a curved fit --- +127 mm Impact, +34 mm Edge), the curvature-model R², the ΔAIC, and the joint-F statistic are emitted to *10a_report_numbers.csv* under the *ANCOVA_Forest\_{zone}\_coeff_cwb2_x_fell* / *\_curv\_\** keys. The variant does not establish state-dependence: the estimates point the way a dry-period canopy-buffering mechanism would, and the record does not separate that from no state-dependence at all. Even were the term significant it would not be proof of the mechanism, since climate-state-dependence has alternative explanations (post-felling non-stationary drift, the coastal-retreat gradient) that the variant does not exclude. The main report (§4.6) reports it neutrally as a flagged preliminary finding, not as a confirmed buffering effect.

**Per-control-well spread.** Each tier's ANCOVA is fitted against the tier *centroid*, so the result is a tier mean and says nothing about whether the tier's members agree --- which matters most for a tier admitted on distance rather than setting. Section 3c refits the same specification with each control well used singly as the control set and reports the resulting step and easting range beside the tier estimate, for every tier (a spread is only interpretable against the spread the established tiers show). These are diagnostics: none replaces a tier fit, and the tier fits above are untouched. The per-well refits are exported to *10a_09_control_well_spread.csv* and carried through to Script 25's corroboration spread table.
...
**Motivation.** The three-counterfactual ANCOVA in 10a (Forest, Climate, Combined control tiers) accounts for climate forcing via the cumulative water balance (CWB) covariate and for coastal-retreat drift via a coastal-drift covariate, included in every contrast. Both covariates carry independent physical justification --- CWB tracks atmospheric forcing month by month, and the coastal-drift term absorbs the progressive divergence that the documented coastal-retreat gradient at the western margin would otherwise produce between the impact wells and their controls (Script 25; CEH4 retreat rate of order 29 mm per year over 2010--2024, with documented six-week losses during Storm Brendan, January 2020). However, the headline ANCOVA's identification of the felling step is materially dependent on the drift specification: a sensitivity run that omits the term (10j internal diagnostic, reported below) reduces the Forest-control Impact step from its full-specification value to a small positive figure that is not statistically distinguishable from zero. The dependence is mechanistically defensible --- coastal retreat is real, and well-documented --- but a sceptical reading of the headline benefits from a corroborating estimator whose identification does not pass through any drift covariate at all.
...
**Easting-term sensitivity (internal diagnostic).** 10j also runs a sensitivity check on 10a in which the drift covariate is omitted from each of the six ANCOVA configurations. The result is reported in the main-report editorial queue as a robustness paragraph for §4.6; 10j does not write the sensitivity to a pipeline CSV because it is a diagnostic on a different script's specification rather than an output of 10j itself. The Forest-control Impact step collapses to a small non-significant figure when the easting term is removed, so the headline depends materially on the specification. That dependence was previously read as evidence that the term absorbs coastal-retreat physics rather than over-fitting a felling-era residual, on the strength of two further estimators. Neither carries the weight. The 10f synthetic control draws its donors from CEH1, CEH5, CEH6, CEH10, CEH11 and CEH24 --- clusters C1, C2 and C3, with no forest well among them --- so its counterfactual is open dune rather than the unfelled forest the headline is estimated against; and the 10f SSM forward residual computes its step within each well and forms no contrast at all.

Script 10n supplies the normalisation both lacked. It builds a synthetic for every zone from the same donor pool and differences the gap series, so the contrast is against unfelled forest and no easting enters. The Forest control carries a gross step of +50.0 mm in its own right, against the Impact zone's +99.3 mm; normalised, the Impact step is +47.3 mm (95 % CI −14.7 to +109.2 mm, HAC p = 0.135) and is not distinguishable from zero. No easting-free estimator corroborates the headline, and the report says so (§4.6.7).

10n does establish a reason to keep the covariate that says nothing about coastal retreat. The Impact and Forest-control wells were already diverging before felling --- at −13.2 mm yr⁻¹ across the pre-felling record (p = 0.007), and at −50.7 mm yr⁻¹ across the post-scrape part of it alone (p = 0.048), so it is not the 2015 scrape. A linear divergence of that size between treatment and control is exactly what a linear-divergence control is for. The direct Impact-vs-Edge contrast remains the estimator whose identification does not pass through the drift covariate at all.
...
-   **Three control structures (Forest, Climate, Combined), not a single counterfactual.** Each isolates a different aspect of the clearfell signal. Forest controls for canopy effects; Climate is the unconfounded climate baseline; Combined pools the three control tiers for tighter intervals at the cost of mechanistic specificity. The three are reported together rather than reduced to one because they answer different questions and the agreement (or disagreement) across them is itself the headline. There is no standalone "Coastal Forest" counterfactual --- the C5 wells enter only inside Combined, because the C5 tier carries a confounding coastal-retreat signal whose drift is more cleanly absorbed by the coastal-drift covariate inside the ANCOVA than by treating C5 as a third counterfactual. A fourth *FarField* tier --- five distance-selected inland wells --- is fitted alongside these as a supplementary coastal diagnostic rather than a clearfell counterfactual; see the far-field note under sub-script 10a.
-   **Coastal-drift correction in the ANCOVA.** Coastal retreat at Newborough is episodic and acute (approximately 50 m of retreat between 2014 and 2020, with the acute loss during Storm Brendan in January 2020). The coastal-drift correction in the BACI ANCOVA is the suite's local control for the drift that retreat produces between the impact wells and their controls. The covariate is the coastal profile fitted by Script 25, evaluated at each tier's mean distance from the shore and differenced between target and control, then scaled by that differential so the fitted coefficient ξ is dimensionless: ξ = 1 is exactly the amplitude fitted at network scale. It is included in every contrast, the differential being defined for every tier, and is significant in seven of the eight --- p from 0.0004 at Forest Impact to 0.030 at Combined Impact --- the exception being Combined Edge, where the amplitude is indistinguishable from zero (ξ = −0.01, p = 0.99).
...
### []{#anchor-239}[]{#anchor-240}[]{#anchor-241}Outputs (consolidated)
  ...
  10a          10a_01_ancova_comparison_table.csv                                                       8-row summary: 4 control tiers (Forest, Climate, Combined primary; FarField supplementary) × 2 zones; clearfell step, CI, p, R², N
  10a          10a_02b_drift_design_equivalence.csv                                                     Coastal vs easting drift-design equivalence check (step, p, AIC per pairing)
  10a          10a_09_control_well_spread.csv                                                           Section 3c per-control-well spread: single-control-well refits per tier; read by Script 25
  10a          10a_09_coastal_scale_factor.csv                                                          s_coast per (control, zone): the Script 25 coastal amplitude each pair feels (M14 / D-076)
  10a          10a_10_coastal_fixed1_sensitivity.csv                                                    Clearfell step with ξ pinned at unity (coastal_fixed1) vs free, per tier and zone
...
-   **Table 8** --- The coastal-drift term fitted in each ANCOVA contrast, with the differential drift each absorbs, source: *25_04_baci_corroboration.csv*.
...
-   §4.6 robustness paragraph --- drift-covariate sensitivity and the direct Impact-vs-Edge contrast (10j); values sourced from *pipeline_site_observations.csv* (entries *impact_vs_edge_clearfell_monthly_step* and *impact_vs_edge_clearfell_summer_step*) rather than from a cached constant.
...
-   **S.12** --- the per-well WTF Sy_median (Script 18, *18_wtf_01_well_sy_estimates.csv*), aggregated to a cluster median for the volumetric translation used downstream (not directly in 10d; the volumetric step appears in the §4.6 narrative).
-   **S.14** --- Script 21 forestry scenarios, which consume 10e's BACI-corrected β₂ multiplier via *load_clearfell_b2_multiplier()*.
-   **S.15** --- Script 25 coastal-retreat gradient, which informs the coastal-drift correction in 10a's ANCOVA. 10j's identification design is the corroborator that bypasses that correction; the two estimators are designed to be cross-checked rather than to agree by construction.
...
### []{#anchor-263}[]{#anchor-264}[]{#anchor-265}Outputs
  ...
  00_climate_summary/00_03_summer_warming_trend.png                  RAF Valley JJA max-temp anomaly, 1931--2025                    Report §4.1.1, Figure 5
...
-   §4.1.1 *Climate record* --- Figure 5 (RAF Valley summer warming trend) from Script 00.
-   §5 *Discussion / climate trajectory* --- summer- and winter-trajectory figures and exceedance summary from Script 14; the Script 00 warming-trend figure (§4.1.1 / Figure 5) provides the long-baseline temperature context that §5 draws on.
...
-   §4.13.1 of the main report --- the figure *14b_year_of_crossing.png* lands here as supporting evidence for the seasonal-prediction discussion in §5.2.4.
...
#### []{#anchor-280}[]{#anchor-281}Outputs (Script 11)
  ...
  11_forecasting_thresholds/11_forecast_pflood_summary.csv               Reviewer-friendly summary: A, B, m_P, horizon, P_flood per cluster
...
#### []{#anchor-287}[]{#anchor-288}Outputs (Script 11b)
  ...
  11b_spatial_thresholds/11b_03_pflood_per_well.csv              Per-well P_flood, m_P, ρ_h, S_P, S_E, h₀, coeff_source, unreachable flag
...
**Limitations.** The m_P values come from Script 11b's per-well calculation; they inherit Script 11b's assumptions about cluster β coefficients and the climatological winter rainfall baseline. The categorical bin edges (1.5 and 2.5) are operational choices, not derived from natural breaks in the data; Conclusion 4's text explicitly identifies the m_P \< 1.5 boundary, and the marginal-vs-unreachable boundary at m_P = 2.5 is selected to match the abstract's wet-winter framing. The achievability category describes only whether the cluster summer minimum can be raised above the Curreli threshold by winter recharge alone --- scrape-as-drainage geometry effects (S.6) and forest-management interventions (S.14) are separate degrees of freedom in the scenario framework. Wells flagged as scraped in the existing per-well CSV (CEH36, CEH18, CEH21) retain their categorical assignment based on present-day m_P; the category reflects post-intervention behaviour where applicable.
...
### []{#anchor-303}[]{#anchor-304}[]{#anchor-305}Inputs
  ...
  01_climate.csv           Script 01 --- monthly P and Thornthwaite PET (RAF Valley, 53.25 °N)
...
> Δh(t) = β₁·P(t) − β₂·exp(−κ·d(t−1))·PET(t) − β₃·(z₀ + h(t−1))

The β₁ rainfall and β₃ drainage terms are unchanged from the canonical SSM (F.3). The β₂ atmospheric-draw term is modified by an exponential decay factor in *d(t−1)*, the depth below ground at the start of the month. The functional form has two important properties: at κ = 0 the model reduces exactly to the canonical SSM (*exp(0) = 1*), and as κ grows the PET draw is progressively attenuated when the water table is deep. The decay factor is a multiplier on β₂, not a replacement: a cluster's PET sensitivity at the surface is still β₂, but the effective β₂ at depth *d* is β₂·exp(−κ·d).

**Two uses of depth, one term.** The script's docstring is emphatic that *d* (depth below ground, used in *exp(−κ·d)*) and the displacement *z₀ + h* (used in the β₃ predictor column) are not the same quantity. The displacement is referenced to the drainage datum 3.7 m below ground (F.3) and is the hydraulic head available to drive Darcy drainage; *d* is referenced to the ground surface and is the physical distance the capillary fringe must reach. The two coincide only in the trivial case where the datum equals the ground surface. Conflating them --- for example, by passing displacement into the exponential --- would change the physical meaning of κ from "decay length of capillary connectivity from the soil surface" to "decay length above the drainage datum", which has no obvious interpretation.
...
**Grid search.** *grid_search_lambda()* scans κ over \[0, 6\] m⁻¹ in steps of 0.05 --- 121 grid points per cluster. At each κ, the term *−exp(−κ·d\_{t−1})·PET(t)* is a known column once d\_{t−1} is fixed, so β₁, β₂, β₃ are recovered by no-intercept OLS on a three-column design matrix (*beta_1_recharge*, *beta_2_atmospheric_draw*, *beta_3_drainage*). Physical-sign filtering follows: a grid point is accepted only if all three fitted β values are positive. Grid points with β₃ ≤ 0 are dropped --- these are statistical artefacts where the depth-coupling has shifted the OLS solution onto an unphysical drainage term. The accepted (β, κ) pair is then forward-simulated iteratively (*iterative_simulate()*, identical recurrence to Script 08 and *model_utils.simulate_ssm()* apart from the decay factor in the β₂ step), and the iterative Nash--Sutcliffe efficiency (NSE) is computed against the observed centroid. Best κ per cluster is the grid point that maximises iterative NSE.
...
-   **κ grid range \[0, 6\] m⁻¹.** The lower bound is the canonical-SSM limit: κ = 0 returns the fixed-β₂ model exactly, so the null hypothesis (no depth coupling) is a special case in the search. The upper bound is generous: at κ = 6 m⁻¹ and a water-table depth of 1 m, *exp(−6) ≈ 0.0025*, so PET draw is effectively suppressed. No cluster's best κ approaches this bound, so the choice of upper limit does not bind any result.
-   **Grid step 0.05 m⁻¹.** This resolution is finer than the cross-cluster spread of best κ values (0.20--2.25), so quantising at 0.05 does not blur any cluster's position. A finer step would change reported κ values at the third significant figure at most.
...
-   **Physical-sign filtering during the grid search.** The β₃ ≤ 0 filter is the principle of failing loudly: a coefficient that flips sign as κ varies is a statistical artefact, and feeding such a fit into the iterative simulator would produce numerically valid but physically uninterpretable trajectories. In practice the filter removes the C4 band κ ∈ \[0.25, 0.95\], where β₃ goes mildly negative --- C4's best κ at 0.20 sits at the boundary just below this band. C1, C2, C3, and C5 produce physically valid fits at every grid point.
...
### []{#anchor-309}[]{#anchor-310}[]{#anchor-311}Outputs
  ...
  ------------------------------- ----------------------------------------------------------------------------------------------------------------
  Output                          Description
  15_00_lambda_profiles_raw.csv   Per-cluster κ-grid sweep with β₁, β₂, β₃, one-step R², iterative NSE at each grid point
  15_01_lambda_profile.png        One panel per cluster: iterative NSE vs κ, with dashed horizontal line at the canonical-SSM baseline
  15_02_fit_comparison.png        One panel per cluster: observed centroid vs canonical SSM vs best depth-coupled fit (time series)
  15_03_benchmark_table.csv       Per-cluster summary: SSM NSE, depth-coupled NSE, Δ NSE, best κ, fitted β values
...
The depth-coupled model improves iterative NSE over the canonical SSM at all five clusters. Differences are graded:
  ...
  --------------------- --------- ------------------- ------- -------------- ------------------
  Cluster               SSM NSE   Depth-coupled NSE   Δ NSE   Best κ (m⁻¹)   Mean upstand (m)
...
Three observations on the pattern. First, the cluster where depth coupling improves the fit most is C1 Lake Edge --- the cluster whose water table sits closest to the ground surface for the largest fraction of the record. A high κ at C1 (2.25 m⁻¹, the steepest decay in the network) means that the PET draw is heavily attenuated even at shallow depths: at *d* = 0.5 m, *exp(−2.25·0.5) ≈ 0.32*, so the effective β₂ is reduced by more than half. The canonical fixed-β₂ at C1 has to compromise across the seasonal swing between shallow winter levels and deeper summer levels, and the depth-coupling formulation resolves that compromise by letting β₂ vary with the season as the water table drops. Second, the forest clusters (C4, C5) and the western dune (C2, C3) take smaller κ values --- between 0.20 and 0.95. These clusters sit deeper on average, and a smaller κ reflects an effective β₂ that decays only modestly with depth across the working range. Third, C4 Main Forest's best κ at 0.20 m⁻¹ sits at a boundary in the OLS solution: κ ∈ \[0.25, 0.95\] produces a negative β₃ that the physical-sign filter rejects, so the C4 result picks up the largest contiguous band of physically valid fits at the low-κ end. The headline number is robust, but C4 is the cluster where the depth-coupled and canonical formulations are closest to being statistically indistinguishable on coefficient signs alone.

The cross-cluster κ pattern is consistent with the capillary-connectivity hypothesis: clusters where the water table is shallowest on average show steeper decay (because the working range of *d* is small, a larger κ is needed to produce a meaningful difference between summer and winter PET draw), while clusters where the water table is deepest show shallower decay (the canonical fixed-β₂ already roughly approximates the effective PET draw across the cluster's working depth range). The pattern is suggestive rather than definitive --- five points are not enough to fit a quantitative relationship between mean cluster depth and κ --- and the depth-coupling formulation is one of several physically motivated alternatives. It is the simplest one-parameter form consistent with monotonic decay of connectivity with depth.
...
-   **The PET data is Thornthwaite-derived.** The sensitivity analysis is to the PET *signal* as Thornthwaite represents it (mean monthly temperature, day length, latitude 53.25 °N), not to a fundamentally improved PET formulation. Penman--Monteith PET would change the absolute magnitudes of both β₂ and the inferred κ. F.4 covers the Thornthwaite choice; §5.3 of the main report discusses the trade-off explicitly.
-   **The improvement in iterative NSE is graded, not transformative.** Two clusters (C3, C5) gain Δ NSE between 0.03 and 0.05 and C2 gains 0.09; two (C1, C4) gain more substantially (0.13--0.18). The canonical fixed-β₂ formulation is therefore an acceptable approximation for most of the network at most depths, and the depth-coupled model is a refinement rather than a fundamentally different description. This is the central reason Script 15 remains a sensitivity analysis: the gain does not justify retiring the simpler model.
-   **C4 sits at a coefficient-sign boundary.** The best-fit κ at 0.20 is the last physically valid grid point before β₃ flips negative for a band of intermediate κ values. This is a feature of the OLS geometry at that particular cluster centroid and may not survive perturbations (alternative climate input, longer fit window, different cluster membership). The C4 result is reported but should not be over-interpreted as a quantitative estimate of the cluster's depth-coupling parameter.

### []{#anchor-316}[]{#anchor-317}[]{#anchor-318}Where the result appears in the report

-   **§5.2.2 *****Displacement Formulation and Depth-Dependent PET*** --- the Thornthwaite limitations discussion cites Script 15's depth-coupled κ values as evidence that the fixed-β₂ formulation is defensible across the network. The graded improvements (largest at C1, smallest at C3 and C5) appear in the limitations paragraph.
...
*save_volumetric_table()* implements the partition. Annual rainfall (P_annual = 892.2 mm/yr, the same for all clusters by construction) is reduced by interception at forest clusters (*I = 0.24 × P* per F.4 for C4 and C5; zero elsewhere), giving *P_net = P − I*. P_net is then split into ET and drainage according to the cluster's drainage fraction. The result is a per-cluster mm/yr breakdown that can be read alongside the 892.2 mm/yr rainfall input and the 657 mm/yr Thornthwaite PET ceiling.
...
**Headline values.** Approach B is the pipeline-consumed canonical estimator: the per-well event-based values written by Script 18 to *18_wtf_01_well_sy_estimates.csv* are what the downstream scripts consume; the consumer list is enumerated in the script ledger, which is checked against the code rather than restated here. Approaches A and C are reported alongside as independent cross-checks that propagate to no downstream calculation. Two Approach B aggregations are in circulation and should not be conflated: the cluster-level event median in *17_wtf_01_sy_estimates.csv* (C3 = 0.3283), tabulated below and in Paper 1 Table 4; and the median of the per-well event estimates in *18_wtf_01_well_sy_estimates.csv* (C3 = 0.3057), which is the value the drawdown-reach figure and the other downstream consumers actually use. Approach A, Approach B, and Approach C produce the following cluster Sy estimates (Script 17 v1.4.0; verify against *17_wtf_01_sy_estimates.csv* before citing --- do not cache):
...
Three cross-approach observations: C3 has the tightest spread of the five clusters (0.319--0.352 across the three methods), which solidifies the Fig-17 λ anchor at that cluster. C1 has B and C both well below A (0.210 and 0.180 vs 0.342), consistent with β₃ over-correcting in the lake-buffered cluster --- a diagnostic reading, not grounds for rejecting A. The coarse Sy gradient across the five clusters is robust to estimator choice; the fine top-end ranking (C3 vs C5) remains method-dependent: A puts C5 highest, B and C put C3 highest. The pipeline-consumed canonical for forest clusters is the corrected Approach B value (Script 18 → *18_wtf_01_well_sy_estimates.csv*); Table 4c of the main report uses Approach B with interception correction.
...
-   **Per-well Sy is treated as a diagnostic, not a parameter.** Downstream calculations (Script 19's spatial groundwater work, Script 21's forestry scenarios) take the per-well *Sy_median* from Script 18's *18_wtf_01_well_sy_estimates.csv*, aggregated to a cluster median where a cluster value is needed (D-038); Script 17's cluster-level event median is the Paper 1 Table 4 figure, not the value these calculations substitute. The per-well spatial map is for pattern recognition; the per-well Sy_median values are what feed the volumetric work.
...
#### []{#anchor-355}[]{#anchor-356}Outputs
  ...
  17_wtf_well_sy.csv                           RETIRED 2026-08-19 (D-038). Was an intermediate copy at *outputs/* root; the per-well table is now Script 18's *18_wtf_01_well_sy_estimates.csv* and nothing writes the old path   Internal pipeline
  -------------------------------------------- ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- ----------------------------
...
**Outputs.** A single self-contained HTML file (*scenario_viewer.html*, typical size 250--300 KB including the embedded base64 hillshade) and a tidy summary CSV (*19_scenario_summary.csv*, one row per scenario × season × spatial unit, where spatial unit is each of C1--C5 plus a site-wide mean). The CSV shares the viewer's *\_well_dh()* engine and reports, per row, the mean and median displacement (*dh_mean_m*, *dh_median_m*) and the storage change it implies as water equivalent --- *we_mean_mm* and *we_median_mm* (Sy × Δh, computed per well and then averaged across the cluster rather than as a product of means), with *sy_mean* the mean of the per-well specific yields applied (the Script 18 WTF median with the cluster floor, not a viewer slider endpoint). These columns were added at v2.15.0; the quantity had lived only in the viewer's JavaScript off a Sy slider, untraceable, and the report, supplement and pipeline briefly held three different vintages of it. Displacement and its water equivalent are computed on the comparison-window coefficient basis (each well's recent-record fit in *03_master_data.csv*); the viewer itself defaults to the whole-record basis (*03_15_per_well_window_sensitivity.csv*), which matches the report, and offers the recent basis as a toggle that raises an on-screen note because on it several clusters --- C4 above all --- carry a drainage term the shorter record cannot resolve (D-034). The MSL5 rows are centroid-level and carry no water equivalent. To anchor with one scenario: for UKCP18 2080s annual the site-wide mean Δh is −33 mm, with the largest impacts in C4 Main Forest (−46 mm) and C3 Western Residual (−34 mm) and the smallest in C1 Lake Edge (−17 mm). Forest clusters show the most muted winter response (C4 +25 mm, C5 +26 mm) against summer drawdowns of −118 mm at C4 and −71 mm at C5, giving C4 the largest annual decline of any cluster; the steepest summer drawdown is at C2 Dune (−134 mm). Clearfell produces the inverse --- C4 +40 mm annual, C5 +39 mm annual --- with winter recovery exceeding summer recovery in both forest clusters (C4 winter +47 mm vs summer +32 mm; C5 winter +45 mm vs summer +33 mm). These values appear directly in §4.13.2 of the main report.
...
*Erosion drawdown.* Applies the Script 25 forest-free linear-capped coastal-retreat fit δ(d) = δ₀·(1 − d/L_cg) to a single discrete shoreline-retreat event of magnitude COAST_RETREAT_M = 6 m (the Storm Brendan early-2020 exemplar). The retreat magnitude is converted to an edge water-table drawdown via h₀ = COAST_RETREAT_M · (δ₀ / E_c), where E_c = 2.32 m yr⁻¹ is the shoreline retreat rate measured by this study over 2006--2026 from dated aerial imagery (Script 40), read live from 40_01_epoch_series.csv at generation time rather than held as a constant, and emitted only when the measurement's own control and generalisation gates pass. The field decays inland from a dune-edge front derived by offsetting the DEM mean-waterline contour (COAST_SHORE_LEVEL_M = 0.5 m AOD) landward by COAST_DUNE_OFFSET_M = 100 m to the dune toe. δ₀ and L_cg are read live from *25_01_panel_fit_parameters.csv* (Script 25, S.15) at generation time --- currently δ₀ ≈ 0.13 m and L_cg ≈ 1.5 km --- so the field tracks the live coastal-gradient fit. The divisor is a *measurement*, not a literature value, and it is period-matched to δ₀ rather than chosen for its storminess. It is **not** the chronic profile rate of Pye & Blott (2024) (≈ 1.5--1.8 m yr⁻¹ at the southern frontage), which describes a different extent over a different window.

*Sea-level-rise head response.* Models the gradual rise as a finite-window boundary perturbation diffusing inland according to Δh(d) = ΔSLR · erfc(d / (2·√(D·t))), where D = K·b/Sy is the hydraulic diffusivity. The boundary is referenced to mean sea level (SLR_SHORE_LEVEL_M = 0 m AOD), as appropriate for a water-table head response (a tidal-inundation construction would instead reference MHWS ≈ +2.9 m AOD from the Caernarfon tide tables, but is not used here). ΔSLR = SLR_RISE_M = 0.02 m over SLR_WINDOW_YEARS = 5 yr matches the UKCP18 north-Wales near-term central estimate (≈ 4 mm yr⁻¹). Aquifer parameters are SLR_K = 6 m day⁻¹ (Betson 2002) and SLR_B = 5 m saturated thickness; Sy is read live as the C3 (Western Residual) cluster median of per-well WTF estimates from *18_wtf_01_well_sy_estimates.csv* (currently ≈ 0.308), matching the propagation-medium convention used for the forest drawdown figure. The figure annotates √(D·t) on the inland axis.
...
**Parameter table.** All controlling parameters for the three coastal-process figures, with their sources and the figures in which each appears. Live-pipeline values are read on each pipeline run and revise if their upstream outputs change; the snapshot values stated here apply at time of writing under the live pipeline state.
  ...
  Specific yield (C3 propagation medium)         Sy                    ≈ 0.308 (live; C3 WTF median from *18_wtf_01_well_sy_estimates.csv*)                    Live: Script 18, S.12                                    Forest drawdown, SLR, Net
  ...
  Measured shoreline retreat rate, 2006--2026    E_c                   2.32 m yr⁻¹ (live; Script 40 40_01_epoch_series.csv)                                    Live: Script 40 (this study)                             Erosion, Net
...
**Matching the periods.** δ₀ is a deepening per year, fitted over 2005-03 to 2026-02, so the divisor that converts it to head per metre of retreat must span *the same period*. The measured 2006--2026 rate E_c overlaps that window by 99 per cent; Script 40 computes the overlap on every run and warns below 90 per cent, so the match is checked rather than asserted. The magnitude of the single illustrated event --- the 6 m Storm Brendan exemplar --- enters separately, through COAST_RETREAT_M. Published retreat rates for this frontage differ in period, extent and method, and are not interchangeable with the rate used here: Pye & Blott (2024) report approximately 1.5--1.8 m yr⁻¹ at the southern frontage from profile surveys, and a contemporary account of the 2014--2020 window reports roughly 50 m of loss (Forgrave 2020). Those appear in §5 of the main report as context for cumulative shoreline change. Published rates appear in §5 of the main report (the Pye & Blott discussion weave-in); the measured rate E_c appears here and in §3.4.5.
...
**Equation forms.** The model forms are stated in §3.4.5 of the main report in LibreOffice StarMath syntax (Δh = H₀·exp(−d/λ), δ(d) = δ₀·(1 − d/L_cg), Δh(d) = ΔSLR·erfc(d/(2√(D·t)))). The supplement does not duplicate these forms; cross-reference §3.4.5 of the main report for equations.
...
**Updated parameter table --- additions for the v1.1.0--v1.11.3 figures.** The coastal-process parameter table above documents the erosion, SLR, and net fields. The additional parameters introduced by the scrape and composite figures are:
  ...
  Inferred scrape excavation depth   d_ex                        ≈ 0.42 m (back-calculated; H₀ / Sy)                                                                   Inferred output (not an input)                                    Scrape (informational only)
...
### []{#anchor-381}[]{#anchor-382}[]{#anchor-383}Outputs
  ...
  19_spatial_groundwater/19_scenario_summary.csv          Per-scenario × season × cluster Δh table (6 scenarios × 3 seasons × 6 spatial units = 108 rows)                                                                                                                                                                                                                            §4.13.1 / §4.13.2 *Scenario analysis*
  ...
  20_spatial_figures/20_drawdown_propagation_nohead.png   Flow-weighted forest drawdown propagation with λ length-scale annotation; filled-contour no-head form on the shared discrete drawdown scale (primary form emitted by *main()* since v1.4.0; the with-head form is callable but not in the default run)                                                                     §4.11 *Forest drawdown propagation* (Figure 57)
  20_spatial_figures/20_coastal_erosion.png               Coastal-erosion drawdown field for a single Brendan-class retreat event; dune-edge front; δ₀ and L_cg from live Script 25 fit                                                                                                                                                                                              §3.4.5 *Drawdown-Field Visualisation* / §4.10.5 *Coastal-Margin Processes* (Figure)
  20_spatial_figures/20_slr_response.png                  Sea-level-rise head response over a finite five-year window; erfc transient, mean-sea-level boundary; D = K·b/Sy with Sy live from C3 WTF                                                                                                                                                                                  §3.4.5 / §4.10.5 (Figure)
  20_spatial_figures/20_coastal_net_effect.png            Cell-by-cell net coastal change (SLR head gain minus erosion drawdown); diverging scale                                                                                                                                                                                                                                    §3.4.5 / §4.10.5 (Figure)
  20_spatial_figures/20_scrape_drawdown_nohead.png        Dune-scrape drawdown field, all eight cuts, leaky-aquifer superposition with method-of-images coastal boundary correction; H₀ empirically anchored at three monitored cuts (CEH36, CEH21, CEH18) from live Script 09a BACI shifts; shared discrete drawdown scale                                                          §3.4.5 / §5.4.2 (Figure 58)
...
-   §4.10.5 *Coastal-Margin Processes and the Easting Gradient* --- the three coastal-process figures (*20_coastal_erosion.png*, *20_slr_response.png*, *20_coastal_net_effect.png*) and their interpretive context.
-   §4.9.3 Drainage Decay Half-Life and §4.11 Management influence on the water table --- the forest drawdown propagation map (*20_drawdown_propagation_nohead.png*) from Script 20, published as report Figure 57.
-   §4.13.2 *Forest Management Scenarios* --- values from *19_scenario_summary.csv* for the clearfell, broadleaf and thinning rows.
-   §4.13.1 *Climate Scenario Projections* --- values from *19_scenario_summary.csv* for the 2050s and 2080s rows.
...
-   §5.4.3 *Clearfell BACI confounder framing* --- the clearfell pre-fell baseline composite (*20_clearfell_baseline_drawdown.png*) renders the scrape + Storm Brendan retreat composite that the BACI's coastal-drift correction implicitly controls for.
...
**Volumetric companion (***21_forestry_06***).** Alongside the volumetric panel, *plot_scenario_comparison()* also emits *21_forestry_06_summer_scenario.csv*: the per-cluster equilibrium volumetric metric *Delta_vol_summer_mm_per_month* for the three forest-management scenarios (clearfell, thinning, broadleaf), taken directly from the monthly scenario values and byte-identical with the forestry rows of *09b_05* by construction. The former summer-minimum amplification conversion (*scraping_common* flux → head ÷ Sy → summer minimum × amplification factor) was removed on 2026-07-02. This gives the §4.13.2 forestry narrative a forestry-module source for the volumetric ecological metric rather than having to reach into the scraping suite for it; the climate-scenario comparison remains with *09b_05*.
...
### []{#anchor-397}[]{#anchor-398}[]{#anchor-399}Outputs
  ...
  21_forestry_06_summer_scenario.csv       Per-cluster equilibrium volumetric metric *Delta_vol_summer_mm_per_month* for clearfell, thinning, broadleaf --- byte-identical with *09b_05* forestry rows; former summer-minimum amplification conversion removed 2026-07-02                                                                                                                                                                                                                                                                             Main report §4.13.2
  ---------------------------------------- ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- ------------------------------------
...
-   **Broadleaf β₂ --- canonical here, consolidated in config.** The 12 monthly multipliers above are the canonical source. The seasonal means consumed by downstream code are 0.8817 (Nov--Apr) and 1.0750 (May--Oct), and these are what *config.BROADLEAF_B2_WINTER* and *config.BROADLEAF_B2_SUMMER* export. Script 19 imports the same names directly from config. §4.13.2 of the main report carries these values.
...
-   **Sy enters at the scenario-comparison panel only.** The hydrograph and the distributions are in head units (m) throughout. The volumetric scenario-comparison panel converts through *the per-well Sy_median* from *18_wtf_01_well_sy_estimates.csv* (chapter S.12); the interception correction on forest-cluster Sy is the one Script 18 carries, with the uncertainty discussed there. The scenario-comparison panel inherits the same Sy uncertainty.
...
The eleven Phase 4 chapters (S.8--S.14) cover the climate-and-spatial scripts that feed §4 of the report. Phase 5 collects six further chapters whose methodological purpose is supplementary: each consumes pipeline intermediates, none feeds downstream pipeline scripts, and the conclusions they produce inform the report's discussion rather than its core results chain. S.15 covers Script 25 (coastal-retreat gradient, step 26/52, Phase 11 in *run_analysis.py*). S.15c covers Script 09f (spatial-reach synthesis figure, step 50/52, Phase 17; display/utility). S.15d covers Script 09g (mechanism-diagram suite, step 51/52, Phase 17; display/utility). S.16 covers Scripts 22, 23, and 24 (residual diagnostics, steps 27--29/52, Phase 12 in *run_analysis.py*). S.18 covers Script 26 (van Willegen 5-year MSL aggregation, step 30/52, Phase 13). S.18b covers Script 11 Section 5 (Tool A --- spring MSL transfer function, embedded in Script 11 at step 11/52, Phase 3) together with Script 26b (Tool B --- UKCP18 RCP8.5 MSL5 projections, step 31/52, Phase 13). S.18c covers Script 26c (MSL5 report-format figures, step 32/52, Phase 13). S.19 covers Scripts 28, 29 and 30 (cluster framework diagnostics, steps 33--35/52, Phase 14). S.20 covers Scripts 32, 33, 35, 36, 37 and 37b (observed differential change, climate-response envelope, and driver validation; steps 36--41/52, Phase 15, all analytical-default as of 2026-07-13). S.21 covers Scripts 24b, 31, 31b, 34 and 38 (supplementary standalone diagnostics, steps 42--46/52, Phase 16 --- 34/38 analytical-default as of 2026-07-13; 24b/31/31b opt-in). Three further chapters were added after that group: S.22 covers Script 39 (SSM hindcast against the 1989--96 CCW record), S.22b covers Script 40 (shoreline retreat from the digitised coastline epochs), and S.23 covers Script 41 (canopy and forest cover from the dated aerial series). S.24 is appendices; there is no chapter S.17, which is the number the appendices carried when the supplement ended at S.16, renumbered on 2026-08-31 once they came to sit after S.23. All scripts in S.15, S.15c, S.15d, S.16, S.18, S.18b, S.18c, S.19, S.20, S.21, S.22, S.22b and S.23 are part of the pipeline orchestrated by *run_analysis.py*; the "post-pipeline supplementary" framing reflects their role in the report (discussion-feeding rather than results-feeding), not their orchestration status. Scripts 26c (MSL5 report-format figures, Phase 13), 09f (spatial-reach synthesis, Phase 17), 09g (mechanism diagrams, Phase 17), and 27 (greyscale figures, Phase 17) are display/utility steps rather than analytical; Script 26c is documented in S.18c, Script 09f in S.15c, Script 09g in S.15d, and Script 27 in Appendix A.
...
The BACI ANCOVA in S.7 (Script 10a) returned a small but highly significant *coastal_x_time* term. Wells west of the clearfell block were drifting downward against time at a rate the BACI's other covariates --- cumulative water balance, scraping, the clearfell step itself --- could not absorb. That term is what allows the BACI to recover an unbiased clearfell-step coefficient; but it raised a separate question. What was that drift? The candidate answer was the retreat of the Caernarfon Bay shoreline immediately west of the site, episodic but acute (Forgrave 2020; ongoing monitoring by Walker-Springett, Bangor University; \~50 m between 2014--2020 with most of the loss concentrated in Storm Brendan, January 2020). If the eroding shoreline is exporting freshwater storage westward as the dune front loses width, wells closer to the coast should show steeper summer-minimum declines than inland wells, with the steepness decaying inland on a length scale set by the aquifer's diffusive response.

Script 25 tests that hypothesis. It fits a network-scale, physics-based non-linear regression of per-well summer-minimum trends against perpendicular distance to the eroding Caernarfon Bay shoreline, recovers a forest-independent distance-decay signal --- a coast-edge rate of about −26 mm yr⁻¹ at the reference distance, decaying over an inland reach L_cg ≈ 900 m. The third parameter of that fit, the constant c_far, should not be read as a far-field climate background, and the reason is not that it is confounded with the climate covariate. Its variance inflation factor against the cumulative water balance is 1.01, and against δ₀ it is 1.44, so the constant is separately identified. What disqualifies it is instability with respect to the fitting window: holding the well set fixed and moving only the window start, c_far runs from −0.10 to +24.20 mm yr⁻¹ while δ₀ stays negative throughout. The constant is a straight line fitted to a non-monotonic series, and it reports the slope of whatever excursion its window spans; judged against the observed far-field trend it tracks that trend only loosely (r ≈ 0.55) and is biased high by about 7 mm yr⁻¹. The far-field decline the network does show is therefore not measured by c_far, and no far-field rate is quoted from this fit. The distance-dependent parameters are unaffected --- δ₀, the reference-distance rate and L_cg are identified by the distance dependence, not by the level --- and they are what this chapter rests on. Both carry a specification uncertainty beyond their standard errors, however, because the climate covariate is itself a choice. The undecayed cumulative water balance is not the best-fitting series available: exponential accumulators of the same balance, with half-lives from two to twenty-four months, and a forward integration of the site's own state-space model all predict the water table better out of sample. Substituting any of them leaves δ₀ within about −28.5 and −31.5 mm yr⁻¹ and L_cg within about 900 and 1050 m. The sweep is a pipeline step: Script 25 refits the same forest-free linear-capped specification against each covariate in turn and publishes the spread in 25_15_covariate_specification_range.csv, whose two endpoint rows are the range quoted here. The endpoints are rounded for the same reason the reach itself is: each is a fitted parameter carrying a standard error of 50 to 65 m on L_cg and about 2 mm yr⁻¹ on δ₀, so quoting either to the metre would assert a precision the fits do not have. The undecayed form is retained because the covariate is a nuisance term, absorbed by the within-well demeaning rather than reported as a parameter, and selecting it on predictive fit would introduce a free parameter into a published estimate; those ranges are the specification uncertainty on δ₀ and L_cg, and the reach is accordingly quoted in the report as about 900 m to 1 km rather than to the metre.
...
> δ(d) = max(δ₀ · (1 − d/L_cg), 0) + c_far

(For δ₀ \< 0 --- the physically expected sign for an eroding boundary --- the *max* becomes a *min* so that the gradient does not change sign at d \> L_cg.)
...
**Controlling for forest cover.** Forest cover is addressed two ways: three increasingly stringent specifications that eliminate it by exclusion, and a fourth that controls it explicitly. The exclusion ladder:
...
-   **C3-only fit holds ***c_far*\*\* fixed.\*\* With no C3 well at d → 0, the three-parameter fit becomes under-identified: δ₀ and c_far trade off. Holding c_far to the forest-free value (−0.10 mm yr⁻¹ for linear-capped; +2.12 mm yr⁻¹ for exponential, read from *25_01_panel_fit_parameters.csv*) reduces the C3-only problem to two parameters and tests whether the same coast-edge anomaly is recoverable from C3 wells alone.
...
The three all-season linear-capped fits --- full network, forest-free and C3-only --- return δ₀ values in the range −29.0 to −31.7 mm yr⁻¹, L_cg values in the range 894--995 m, reference-distance rates of −24.5 to −27.4 mm yr⁻¹, and c_far values close to zero (−0.47 to −0.10 mm yr⁻¹). As the Cluster attribution note above sets out, c_far is not a far-field climate background: it is a window statistic rather than a rate.
...
The per-cluster attribution under the headline fit is in *25_03_cluster_partition.csv*, computed against the declared balanced basis. C5 (Coastal Forest, three wells, mean d̄ ≈ 372 m) carries by far the steepest observed decline (−16.15 mm yr⁻¹ on the balanced basis; −35.9 mm yr⁻¹ at the Script-14 centroid and −41.3 mm yr⁻¹ as a mean of per-well slopes, both retained as context columns); the gradient model attributes −18.30 mm yr⁻¹ to coastal retreat, which is 113 % of the basis, leaving −0.68 mm yr⁻¹ unexplained. The attribution therefore accounts for the whole of C5's balanced decline and slightly overshoots it. That overshoot is a statement about the subsample rather than about the curve: three wells spanning 176 to 521 m from the shoreline make this the smallest cluster in the partition and the one whose balanced basis is least well determined, and the share should be read as indicating that coastal retreat is sufficient to account for C5's decline rather than as a calibrated percentage (see the limitations note below). C3 (Western Residual, d̄ ≈ 803 m) declines by −14.15 mm yr⁻¹ on the basis, of which the gradient accounts for −3.23 mm yr⁻¹, about 23 %. C1 and C2 sit beyond the inland reach L_cg, so their gradient component is zero; C4's mean distance falls marginally inside it, at 889 m against a reach of 894 m, and carries −0.21 mm yr⁻¹ --- negligible against its own decline, and not resolvable given a reach whose confidence interval runs from 801 to 989 m and which lengthens further under alternative climate covariates (see above). Against balanced declines of −11.0, −11.1 and −6.5 mm yr⁻¹ the modelled totals are +2.8, +2.8 and +2.6 mm yr⁻¹ --- the climate term the cumulative-water-balance covariate carries, plus the fitted offset --- so the whole of their deepening, and a little more, is left unexplained. Why a mean of per-well slopes made that far-field deepening look smaller and better behaved is set out in *25_10_record_length_composition.csv*: within C2 the four wells with 20--21 years of record average −11.0 mm yr⁻¹ while the twenty-four with 11--17 years average +0.6 mm yr⁻¹, so a mean over per-well fits reports a composition of record lengths as though it were a rate.

The BACI corroboration in *25_04_baci_corroboration.csv* reports the coastal-drift term fitted in each ANCOVA contrast, the tier differential that scales it, and the differential drift each absorbs. Because ξ is dimensionless it is read directly as the fraction of the network-scale field a contrast feels: 1.59 at Forest Impact and 1.23 at Forest Edge, neither distinguishable from unity, against −1.51 at the far-field tier, where the sign reverses at the largest differential in the design. The comparison with the gradient model's prediction is an internal consistency check rather than an independent estimate of the coastal gradient --- the covariate is that field --- and the z_test_baci_vs_model and consistent columns are read in that light. Headline numbers are repeated in *25_report_numbers.csv* for downstream cross-referencing.
...
### []{#anchor-421}[]{#anchor-422}[]{#anchor-423}Outputs
  ...
  -------------------------------------------------------------------- ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- -----------------------------
  Output                                                               Description                                                                                                                                                                        Reference
  25_coastal_gradient/25_01_panel_fit_parameters.csv                   All six fits (3 specs × 2 forms): δ₀, L_cg, c_far with SE and 95% CI, AIC, n                                                                                                       Report §5.4.3, §5.7.2
  ...
  25_coastal_gradient/25_04_baci_corroboration.csv                     BACI coastal-drift absorption vs model prediction for each (control × impact zone) pair, with z-test and consistency verdict                                                       Report §5.4.3, §5.7.2
  ...
  25_coastal_gradient/25_07_cluster_decomposition.png                  Horizontal stacked-bar figure: each cluster's observed centroid slope decomposed into climate + coastal + residual. New at v1.1.0 (2026-05-29, folded in from retired Script 30)   Report §4.10.2 figure
...
-   §5.7.2 --- the BACI's *coastal_x_time* covariate, and what the drift it absorbs is and is not.
...
-   **S.7** --- Script 10 suite (BACI); Script 25's absorption comparison reads Script 10a's *coastal_x_time* coefficient directly. That comparison is an internal consistency check, not independent corroboration.
-   **S.13** --- Script 19's residual and per-well slope figures show the same coast-to-inland gradient from an independent visualization. Script 20's coastal-process figures (*20_coastal_erosion.png*, *20_slr_response.png*, *20_coastal_net_effect.png*) consume δ₀ and L_cg from this chapter's *25_01_panel_fit_parameters.csv* live at generation time; see *COASTAL_NET_VS_EASTING_MEMO.md* (project store) for the SLR-extended comparison against the BACI coastal-drift coefficient.
-   **S.15c** --- Script 09f (spatial-reach synthesis figure) reads δ₀ and L_cg live from *25_01_panel_fit_parameters.csv* produced by this chapter.
-   **S.15d** --- Script 09g (mechanism diagrams) draws its coastal reach from the columns of *09f_01_reach_profile.csv*, whose coastal decay derives from this chapter's δ₀ and L_cg.
...
Step 50/52, Phase 17. Display/utility tier, not analytical. Documented here as a companion to §S.15 (Script 25, coastal gradient), whose outputs it reads.
...
All read live from committed pipeline outputs:
  ...
  25_01_panel_fit_parameters.csv   Script 25    Coast-edge retreat gradient δ₀ and inland reach L_cg (forest-free linear-capped fit)
...
-   **§S.15** --- Script 25 produces *25_01_panel_fit_parameters.csv* (δ₀, L_cg) consumed live here.
...
**Step 51/52, Phase 17. Display/utility (***tier=\"D\"***); not analytical. Added 2026-07-18 (***run_analysis.py*\*\* v2.2.0). Reads only committed outputs of Scripts 09f, 10m and 10a; no recomputation.\*\*
...
The fit is performed via the shared *fit_ssm_intercept()* in *model_utils.py* (see F.5), with *HEADLINE_LAG = 0* from *config.py*. The intercept α_B absorbs whatever constant part the unmodelled hydrological inputs carry; the residual series e(t) = Δh_observed − Δh_predicted then represents only the *time-varying* component of the unmodelled contribution. This decomposition is methodologically important. If the residual at a ridge-adjacent well averages, say, +0.030 m/month, that constant offset is by construction absorbed into α_B --- it cannot also show up in e(t). Any signal that e(t) carries must therefore vary in time, which is the only kind of signal that can carry a lag structure against rainfall.
...
For each well's residual series, Script 22 then computes an AR(1) diagnostic: it regresses e(t) on e(t−1) via OLS and reports the AR(1) coefficient φ, its p-value, and the residual standard deviation. The decision criterion *AR1_WHITE_THRESHOLD = 0.3* flags wells whose residuals are sufficiently autocorrelated that pre-whitening would be required before any cross-correlation analysis. A second diagnostic --- the α_B-vs-φ scatter --- tests whether the wells with the largest persistent subsidies (high α_B) are also the wells with the most time-structured residuals (high \|φ\|). If both conditions held, the unmodelled input would be both high in magnitude and variable in time, the physical signature expected from a sustained lateral flux that itself responds to seasonal or interannual climate.

The Script 22 outputs are eight files: a wide residuals CSV (rows = months, columns = wells, values = e(t)), a per-well fits table with α_B, β₁, β₂, β₃, R², AR(1) statistics, mean and standard deviation of the residual, the per-well headline-SSM residual-inference table *22_05_ssm_residual_autocorrelation.csv* and the cluster-mean counterpart *22_06_ssm_cluster_mean_inference.csv* (both see below), and four figures --- an AR(1) histogram by cluster, a spatial map of AR(1) coefficient, the α_B-vs-φ scatter, and example residual time series for one well per cluster (chosen as the longest-record well with a valid AR(1) fit).
...
> Δh(t) = α_B + β₁₀·P(t) + β₁₁·P(t−1) − β₂·PET(t) − β₃·(z₀ + h(t−1))
...
Script 22's per-well AR(1) diagnostics show that the network's Model B residuals are close to white. Across the 62 wells with valid fits, the AR(1) coefficient sits at a small negative value at most wells (network mean φ ≈ −0.13, median ≈ −0.12), and only two wells exceed \|φ\| = 0.3. The α_B-vs-φ scatter shows essentially no relationship (Pearson r ≈ −0.08): wells with large persistent subsidies (high α_B) are not preferentially the wells with the most time-structured residuals. The constant part of the unmodelled input --- whatever it is --- sits in α_B, and the small remaining time-varying part is not strongly clustered in any particular well or cluster.
...
### []{#anchor-486}[]{#anchor-487}[]{#anchor-488}Outputs
  ...
  22_model_b_fits.csv                                               Per-well Model B fits: α_B, β₁, β₂, β₃, R², AR(1) φ, p, σ                                 Script 22 plots; cross-script reuse
  ...
  22_residual_lag_analysis/22_03_alpha_phi_scatter.png              α_B vs AR(1) φ scatter, coloured by cluster                                               §S6 supplementary note
...
-   §4.9.6 *Water Balance Residual Field* --- the corrected field, which closes without requiring an additional flux. Scripts 23 and 24 supply the bound on what the record could have detected.
...
-   **S.3** --- origin of the per-well α_B values that Script 22's α_B-vs-φ scatter cross-correlates against; the canonical β coefficients that Scripts 22, 23, 24 do not revise.
-   **S.5** --- the per-well 100-month windowed fits that Script 07 maps, contrasted with Script 22's full-record fit.
-   **S.13** --- Figure 50 (SSM water-balance residual map) is the visual the diagnostic suite is interrogating; CEH14 is annotated on that figure as the most negative well around which the §5.3 interpretation is anchored.
...
**Step 30 / 52. Phase 13 --- Van Willegen MSL Analyses in ***run_analysis.py***; paired with S.18b which covers the forecasting tools that operate on this chapter's outputs and with S.18c which covers the report-format figures that render this chapter's trajectory output for §4.8.3.**
...
### []{#anchor-514}[]{#anchor-515}[]{#anchor-516}Where the result appears in the report
  ...
  §4.8.3 *Five-Year Mean Spring Water Level (MSL5)*                    **Table 16** (cluster-mean MSL5 at window-end 2025 with SD15b/SD16 threshold-crossing counts, source *26_msl_5yr_per_cluster.csv*), the cluster trajectory plot (**Figure 41**) and the spatial MSL5 map (**Figure 42**); the results narrative
  §3.7.6 *Equilibrium wetness index*                                   EWI definition, calibration, and its derivation from the SSM coefficients (Methods)
  §4.8.4 *Equilibrium wetness index and vegetation cross-validation*   EWI-vs-MSL5 comparison table; Ellenberg-F cross-validation; Williams' test (Results)
...
**Step 31/52 for Script 26b (Phase 13 --- Van Willegen MSL Analyses in ***run_analysis.py***); Script 11 Section 5 lives in Phase 3 at step 11/52 inside ***11_forecasting_thresholds.py***. Companion to S.18; followed in the supplement by S.18c which documents Script 26c's display-only report-format figures.**
...
Tool B's single-step perturbation approach also avoids the SSM forward-integration drift problem documented in Script 21. Running the SSM forward from an arbitrary initial condition accumulates drift because the water-balance intercept α_B that closes the SSM at fitting time is not part of the single-step propagation rule. The single-step perturbation pattern works as a forcing-shift overlay on the observed record rather than as an integrated time projection, and is drift-free by construction.
...
UKCP18 RCP8.5 multipliers used by Tool B are central-estimate (50th-percentile) Wales values for the 2050s and 2080s time horizons. The 2050s constants also exist in *utils.config* as *UKCP18_DRY\_\** / *UKCP18_WET\_\** pairs (carried for cross-reference with Script 19's scenario viewer); both epochs live in *utils.config* as *UKCP18_SCENARIOS*, imported by Script 26b since v1.2.1 (2026-08-18) --- the centralisation previously noted here as a follow-up, and recorded as done in §S.1. Scripts 19 and 26b share a single source, is noted as a follow-up.
...
Five sensitivity variants are run to guard against under-specification of the drift model: the headline forest-free monthly-uniform variant; a summer-only Jun--Sep variant that applies the drift correction only to summer-window months; a full-network δ₀ variant that uses the full-fit gradient (which includes forest wells) instead of the forest-free fit; an L_cg = 500 m variant (shorter inland reach); and an L_cg = 1500 m variant (longer inland reach). A C2 sanity check accompanies each variant: C2 wells should not migrate to other clusters under the same procedure, since the gradient model attributes negligible drift to them (C2 wells are mostly beyond L_cg). Low C2 retention would indicate that the de-trending procedure is contaminating hydrographs rather than testing a hypothesis.
...
Inputs.
  ...
  25_01_panel_fit_parameters.csv                   Script 25              δ₀, L_cg for the exponential and linear-capped coastal predictors
...
-   **S.15** --- Script 25 coastal-retreat gradient parameters (δ₀, L_cg, c_far) feeding both scripts' coastal predictors.
...
Phase 15 (steps 36--41/52; 32/33/35 analytical-default, 36/37/37b also analytical-default --- the committed *pipeline_manifest.json* gives all six *exec=\"default\"*). Six analyses that characterize observed network change and driver attribution directly, independently of (and, for 37/37b, in comparison against) the single-mechanism driver model of §S.13--§S.14: where the spring water table is moving relative to the site (Script 32, Figure 64), how far it swings between climate extremes (Script 33, Figure 66), how deep it sits in dry years against the ecological thresholds (Script 33, Figure 67), a per-well frame-independent coefficient extending the amplification metric to short-record wells (Script 35), an absolute climate-removed secular trend map placing the coastal and forest drying on one figure without mean-referencing or residual-construction artefacts (Script 36), a predicted-versus-observed validation of the modelled driver-change map (Script 37), and a comparative footing of forest/scrape/coastal effects on common currencies (Script 37b, Part B). All six read committed pipeline outputs and re-fit nothing in the SSM; their value layers are observational.

### []{#anchor-564}[]{#anchor-565}[]{#anchor-566}S.20.1 Script 32 --- Secular differential water-table movement (Figure 64)
...
### []{#anchor-567}[]{#anchor-568}[]{#anchor-569}S.20.2 Script 33 --- Climate-swing amplification and dry-year spring floor (Figures 66 and 67)
...
**Outputs.** *outputs/33_envelope_amplification/*: *33_envelope_per_well.csv* (dry/wet state, swing, amplification, cluster); *33_amplification_field.png* (Figure 66); *33_dry_spring_depth.png* (Figure 67); *33_results.txt*. A recent-window companion re-runs the same construction on recent extreme years (dry {2019, 2020, 2025}, wet {2021, 2024}) so that late-installed wells absent from the 2011/2014 extremes can be placed on the field: *33_envelope_per_well_recent.csv*, *33_amplification_field_recent.png* and *33_dry_spring_depth_recent.png*. Its dry extreme is milder than 2011/12, so the recent panel is a conservative recent lower bound (site-mean swing ≈ 604 mm) and is not magnitude-comparable to the canonical panel.
...
**Report location.** Figures 66 and 67; main-report §4.12 and §5.7.5.
...
**Step 39/52, Phase 15. Added 2026-07-05 (v1.0.4); promoted to analytical tier (***exec=\"default\"***) 2026-07-13 per the Task E signed-off audit. Analytical tier. See the Pipeline-at-a-glance table and ***outputs/pipeline_manifest.json***.**

**Motivation.** Script 32's differential map (Figure 64) answers "where relative to the site", re-referencing every well to the network mean, which over a wet-spring-lifted window inverts the reading (the forest interior reads as rising because it amplifies the lifted mean, not because it is genuinely wetting). The MSL5 change map (Figure 63) is absolute but retains the common climate signal, so the whole site deepens together and spatial structure is masked. Script 36 fills the gap: an **absolute** per-well secular trend with the climate signal removed by an **external** index (spring CWB), not re-referenced to the network mean, so the real site-wide recharge decline and coastal drying survive while the inter-annual climate wobble is removed.
...
Inputs.
  ...
  δ₀, L_cg                                          Script 25 *OUT_25_FIT_PARAMETERS*   Forest-free linear-capped fit; δ₀ = −31.35 mm yr⁻¹, L_cg = 894 m
...
**Limitations and known caveats.** The result is a **bounded null on attribution**, and should be stated as such. Every scale-factor CI in the primary specification spans zero (s_coast 0.51 \[−0.10, 1.12\] full-record), because only \~15 wells sit within the coastal field's reach and \~12--20 within the clearfell field, the two fields are collinear (r ≈ −0.48 with easting), and per-well residual scatter (±150--200 mm) exceeds the driver amplitudes. The only tightly-determined terms are the uniform intercept (−593 mm through the 2006--2012 drought; +108 mm post-2017) and, in the 2018--2025 sensitivity variant that carries it, the broadleaf-restock covariate (−4.16 \[−6.43, −1.90\] --- the one scale factor in the analysis whose interval excludes zero, and not carried in the primary specification), so the dominant, resolvable component of change is common-mode, not driver-shaped. The coastal *shape* (δ₀, L_cg) is fitted from the same network (Script 25), so the spatial test is partly self-confirming; the implied-δ₀(t) trajectory is the only independent check and it is too drought-contaminated to be conclusive. Script 37 does not resolve scraping, does not close the coastal budget, and cannot separate coastal erosion from the collinear clay-dip substrate gradient --- only characterise the change as dominated by a common-mode background against which the modelled coastal field cannot be independently confirmed.
...
**Step 41/52, Phase 15. v1.0.1 (2026-07-07); promoted to analytical tier (***exec=\"default\"***) 2026-07-13 per the Task E signed-off audit. Analytical tier.**
...
**Step 45/52, Phase 16. Analytical tier (***exec=\"default\"***), promoted 2026-07-13 per the Task E signed-off audit.**
...
**Step 46/52, Phase 16. v1.3.0 (2026-07-08, wired into ***run_analysis.py*\*\* --- previously standalone); promoted to analytical tier (***exec=\"default\"***) 2026-07-13 per the Task E signed-off audit. Analytical tier.\*\*
...
**Inputs.** MAM spring-mean water levels (m AOD) for the transect wells, from *01_wells_clean.csv*; δ₀ and L_cg from Script 25 (*25_01_panel_fit_parameters.csv*) for comparison. No modelled fields.

**Methodology.** A line running coast→inland (bearing ≈ 45° SW→NE): coastal anchor **CEH22** (SD13, most coastal), interior **NW5**, inland anchor **NW4** (\~1 km inland, outside the L_cg ≈ 894 m drawdown band); CEH40 and CEH41 are annotated profile-only points (Feb-2013 eastern-scrape offset, never in the metric). The headline metric is the coast-minus-inland MAM head difference *h_CEH22 − h_NW4* per year, fitted with an AR(1)-corrected OLS trend --- a model-free observational estimate of δ₀. The climate-corrected profile panel anchor-references every well to the erosion-free inland anchor NW4, stripping common-mode climate without a model.
...
**Step 47/52, Phase 16 in ***run_analysis.py***. Analytical tier (***exec=\"default\"***). The step skips cleanly when the historic inputs are absent, so a default full run cannot fail over an optional raw input.**
...
**Step 48/52, Phase 16 in ***run_analysis.py***. Analytical tier (***exec=\"default\"***). Added 2026-08-29; D-085, D-086, D-098. The step withholds its rate rather than the pipeline, so a default full run cannot fail over it.**

**Motivation. **The coastal-retreat rate the pipeline needs --- the divisor that turns the Script 25 coastal-gradient deepening δ₀ into a head change per metre of shoreline lost (§S.15, §3.4.5) --- began as a session computation in a working note. Under D-006 nothing that lives only in a note is citable, and D-060's Revisit-if asks for exactly this promotion: make the retreat a pipeline output so the downstream field reads a rate rather than inheriting a hand-carried number. But a retreat figure that cannot tell a rigid shift of a digitised line from real erosion is not a measurement, so the diagnostics that decide whether the number means anything are the substance of this chapter, not an appendix to it.

**The estimator, and the defect designed out. **The earlier computation measured the nearest distance from each sampled point on one coastline to the other. That quantity is always positive: it cannot see progradation, cannot change sign, and cannot register a floor --- which is why a frontage with no advance anywhere, whose least-eroding point coincided with the highest independently surveyed rate, once read as a clean result. Script 40 measures signed shore-normal displacement instead: a ray along the local shore normal, positive landward (retreat), negative seaward (progradation), so the floor, the count of prograding samples and the alongshore trend fall out of the sign directly. The nearest-distance quantity is kept as a labelled comparator column so the historical figures stay legible beside the corrected one.

**Inputs. **Read via *utils.paths* from *data/geo/* (provenance in *GEO_PROVENANCE.md*): the digitised coastline epochs --- the 1899 Ordnance line, the 2006, 2017, 2021 and 2026 frames, the September-2019 / March-2020 pair bracketing the Storm-Brendan winter, and a blind re-digitisation of the 2006 line used only for control; the site DTM, for the shore-normal profile and the mean-waterline contour; and the Script 25 forest-free coastal-gradient fit, for the δ₀ → h₀ conversion.

**Methodology and the gate (D-085). **For each epoch pair the script casts shore-normal rays at a fixed alongshore spacing, records the signed displacement at each, and summarises the frontage by its median, its 10th and 90th percentiles, the count of prograding samples and the alongshore slope. Three tests then decide whether a rate is admissible. The control test re-measures the 2006 line against an independent blind re-digitisation of the same coastline and reports the repeatability floor --- a median absolute displacement of about 2.3 m and a 95th percentile of about 5.5 m, the noise a rate must clear to be real. The generalisation test asks whether the digitised line resolves the shape it traces, from its vertex spacing and the sagitta of its longest chords. The floor test asks whether the measured displacement exceeds the control noise at all. A value is written to *rate_m_yr* only when all three pass; otherwise the field is left NA with a populated *withheld_reason*. This is D-085: D-006 makes a pipeline output citable by the fact of its existence, so the refusal cannot be a note beside the number --- it has to be the absence of the number. The script neither fails the pipeline nor warns-and-continues with a value, so nothing downstream can quote what is not there.

**Outputs. ***outputs/40_shoreline_retreat/* carries *40_01_epoch_series.csv* (per epoch pair: signed median and percentiles, progradation count, alongshore trend, the emitted rate or its withheld reason), *40_02_normals.csv* (the shore-normal ray geometry), *40_03_control.csv* (the blind-re-digitisation repeatability), *40_04_generalisation.csv* (vertex spacing and sagitta), *40_05_dtm_profile.csv* (the DTM contour and seaward distance), *40_06_coastal_sensitivity.csv* (the δ₀ → h₀ conversion the coastal driver reads), *40_07_storm_pair.csv* (the 2019/20 winter displacement) and *40_report_numbers.csv*. The headline the downstream figures consume is the 2006--2026 rate, E_c ≈ 2.32 m yr⁻¹, emitted because its control and generalisation tests pass and matched to the δ₀ fitting window to within one per cent; Script 20 reads it live from *40_01* at generation time rather than as a constant.

**Site-specific choices and limitations. **The 2006 → 2017 pair prograded at 23 of 131 samples, and the long 1899 → 2006 pair returned a slower rate (about 0.64 m yr⁻¹) --- both consistent with a frontage that has not eroded uniformly, and both reported rather than hidden under a positive-only distance. The single 2019/20 storm sequence is reported as a displacement (median about 8.9 m over 202 days) and is deliberately not annualised: a storm season is not a rate (D-098). The measured E_c is a period-matched divisor for δ₀, not a chronic profile rate, and it is not interchangeable with the published figures for this frontage --- Pye & Blott (2024)'s ≈ 1.5--1.8 m yr⁻¹ from profile surveys, or the ≈ 50 m of loss over 2014--2020 reported by Forgrave (2020) --- which differ in period, extent and method and appear in §5 of the main report as context.

**Where it appears. **The measured rate E_c enters §3.4.5 and the Script 20 coastal-erosion driver field (§S.15); the published rates it is distinguished from are discussed in §5. The provenance rulings are D-085, D-086 and D-098.
...
### []{#anchor-621}[]{#anchor-622}Data provenance and permission
...
### []{#anchor-625}[]{#anchor-626}Methodology
...
### []{#anchor-633}[]{#anchor-634}Where it appears in the report
...
with *\--dpi* overriding the source DPI, *\--skip-maps* excluding the spatial-map outputs that benefit from manual review, and *\--dry-run* listing the files that would be converted without writing anything. Script 27 is orchestrated by *run_analysis.py* as PHASE_17 (step 52/52) but is a post-analysis utility rather than an analytical step --- it is display/utility rather than analytical and lives here in Appendix A. The script is idempotent: deleting *outputs_bw/* and re-running rebuilds it cleanly.

Script 27's filename prefix (*27\_*) and orchestrator step number (52/52) deliberately do not match --- the same convention applied to Script 26 (*26_van_willegen_msl.py* at step 30/52), Script 26b (*26b_van_willegen_msl_projections.py* at step 31/52), Script 26c (*26c_msl5_report_figures.py* at step 32/52), Script 09f (*09f_management_effects.py* at step 50/52), and Script 09g (*09g_mechanism_diagrams.py* at step 51/52). The filename groups Script 27 alphabetically with the other *2x\_* scripts; the orchestrator number reflects its position in the run order at the end of Phase 17. Script 27 carries no analytical-step number of its own --- it is the post-analysis utility documented in this appendix. Script 26c, the MSL5 report-format figure-rendering companion, is similarly display/utility and is documented in §S.18c. Script 09f, the spatial-reach synthesis figure, is also excluded and is documented in §S.15c. Script 09g, the mechanism-diagram companion to 09f, is likewise excluded and is documented in §S.15d.
...
#### []{#anchor-646}[]{#anchor-647}Intervention machinery
  ...
  *SCRAPING_DATE*, *SCRAPING_DATE_2*                              F.5 / *clearfell_common.py*        2015-04-01 and 2023-10-01
  ...
  Scraping L_s = 300 m                                            F.5 / *clearfell_common.py*        Exponential distance weight for scraping propagation
...
#### []{#anchor-649}[]{#anchor-650}Coastal and spatial
  ...
  -------------------------------------------- --------------------------------- --------------------------------------------------------
  Constant / concept                           Reference                         Value / meaning
  Coastal-retreat gradient parameters          S.15 / *25_coastal_gradient.py*   δ₀ = −31.35 mm/yr, L_cg = 894 m, c_far = −0.10 mm/yr
...
#### []{#anchor-653}[]{#anchor-654}Orchestration and rendering
  ...
  *paths.py* *OUT\_\** / *INT\_\** / *DIR\_\**                              F.5 / *utils/paths.py*                                  All file paths; no script hardcodes a path
  ...
  Greyscale post-processor                                                  Appendix A (this chapter) / *27_greyscale_figures.py*   Phase 17 in *run_analysis.py* (step 52/52); display/utility rather than analytical
  Spatial-reach synthesis figure                                            §S.15c / *09f_management_effects.py*                    Phase 17 in *run_analysis.py* (step 50/52); display/utility; display/utility rather than analytical
  Mechanism-diagram suite                                                   §S.15d / *09g_mechanism_diagrams.py*                    Phase 17 in *run_analysis.py* (step 51/52); display/utility; display/utility rather than analytical
