<!-- GENERATED MIRROR of docs/report/Supplementary_Material_v1_41.odt — do not edit. source-sha256=838bb67f392c0c09 pandoc=3.1.3 -->
...
The Eastern Block (C1: β₁ = 4.578; C2: β₁ = 3.896) and Western Residual cluster (C3: β₁ = 3.758) contrast is consistent with the geological interpretation of Stratford et al. (2007): the Eastern Block sits on shallow till and estuarine deposits, producing a thin, storage-limited aquifer with a flashy response; the Western Residual Cluster occupies deep, clean aeolian sand, producing a capacious, buffered aquifer with an attenuated response. The Forest cluster (C4: β₁ = 2.476) is anomalously low even relative to C3, reflecting canopy interception rather than additional aquifer depth --- consistent with the depth-dependent PET analysis (Script 15), which returns the weakest depth coupling in the network at C4 (best κ = 0.05 m⁻¹, against 2.30 m⁻¹ at C1), indicating that capillary disconnection at depth is not the primary control. C5 (Coastal Forest: β₁ = 2.412) shows a comparable value to C4, consistent with both clusters carrying the same Corsican pine canopy on the same deep sand substrate.
...
This persistence is consistent with the interpretation that the Forest cluster signature at these wells reflects deep sandy substrate at high topographic elevation rather than canopy influence alone --- in which case convergence toward open-dune behaviour may not be achievable regardless of management intervention. C4 wells sit at the highest topographic positions in the dune system (mean head 9.24 m AOD), and their deep unsaturated zones and low recharge sensitivity reflect their geological position, not only their canopy cover. This conclusion is independently supported by the NW10 broadleaf comparison (§5.6.3): NW10, situated within the 1993 clearfell and 1995 broadleaf restocking block, has maintained C4 cluster affinity (r = 0.986) throughout the 18-year monitoring period despite its different canopy cover, confirming that substrate position rather than canopy type is the dominant control on cluster identity.
...
The clearfell scenario applies a β₂ multiplier of ×1.047 at C4 and C5 wells, derived dynamically from the BACI-corrected Edge-tier post-felling β₂ ratio reported in §4.6.6 (Table 14): Edge mean ratio 0.958 minus Climate Control mean drift 0.910, plus 1.0. Canopy interception is set to 0%. The framework predicts modest annual water table rises at both forest clusters (C4: +0.037 m/month head, equivalent to +9.5 mm w.e./month; C5: +0.040 m/month head, +12.2 mm w.e./month), with a strongly asymmetric seasonal profile reflecting the dominance of summer PET in the β₂ sacrificial shielding mechanism established in §4.6.6 and §5.5.2.

The thinning scenario applies a β₂ multiplier of ×1.024 (half the clearfell perturbation) with canopy interception reduced from 24% to 12%. The framework predicts approximately half the clearfell response at both forest clusters (C4: +0.019 m/month, +4.7 mm w.e./month; C5: +0.020 m/month, +6.1 mm w.e./month), as expected.
...
**Visualisation device: **Figure 80 synthetic hydrograph with seasonally-varying broadleaf β₂ (§5.7.4; Script 21)
...
The β₁₀ and β₁₁ coefficients absorb the vadose-zone response between them; any remaining lag structure in ε′(t) is therefore post-vadose and is the candidate ridge-transport signal. Fitted coefficients show that the extended parameterisation is only partly identified: β₁₁ is statistically significant (p \< 0.05) at 32 of the 66 wells, and its sign is mixed (negative at 33 wells, positive at 33), so the two rainfall terms do not partition the vadose-zone response cleanly at every well. The cluster-median lagged-response fraction β₁₁ / (β₁₀ + β₁₁) nonetheless rises across the open-dune-to-forest sequence, from −0.08 in C1 and −0.03 in C2 to +0.08 in C3 and +0.27 in C4 (C5 +0.16), consistent with the slower drainage of forest soils. Mean R² rises only marginally, from 0.758 under the single-period model to 0.768 under the extended model across the 63 wells fitted by both: the second rainfall term buys very little explanatory power, which is consistent with β₁₀ and β₁₁ dividing one vadose-zone response between them rather than capturing two distinct processes. Per-well fits are in 23_ridge_lag_fits.csv and 22_model_b_fits.csv. The extended model is used solely for this diagnostic test; the β₁, β₂, β₃ and α_B values reported in the main text remain the authoritative parameterisation for all other analyses.

## S5.3 Cross-Correlation and Pre-Whitening

For each well with at least 140 months of data (n = 63 after excluding CEH3, CEH4, CEH7, CEH8, CEH37 and llynrhos for reasons noted below), the Pearson cross-correlation r(ε′(t), P(t − N)) was computed for lags N = 0 to 12 months. Prior to cross-correlation, both the rainfall and residual series were pre-whitened using a first-order autoregressive filter: x′(t) = x(t) − φ·x(t−1), where φ is the AR(1) coefficient of the rainfall series (+0.207 at RAF Valley). Wells whose residuals retained AR(1) structure above \|φ\| = 0.2 were additionally pre-whitened against their own residual autocorrelation before the common filter was applied. Statistical significance was evaluated using the Bartlett 95% confidence interval for a white-noise series, \|r\| ≥ 1.96/√N, which for typical series lengths gives a threshold of approximately 0.15.
...
Of 66 wells fitted, 49 had a peak cross-correlation that exceeded the Bartlett significance threshold. Across these 49 wells the peak lag has a mean of 2.49 months, a median of 2 months and a standard deviation of 1.91 months: 42 wells peak at lag 2, five at lag 3, one at lag 11 and one at lag 12, with no systematic geographic pattern. The Spearman rank correlation between peak lag and ridge distance was ρ = −0.028 (p = 0.85) --- no distance structure whatever. Cluster mean peak lags are 2.00 months at C1, C2 and C3 and 3.67 at C4; one C5 well reaches the significance threshold, at lag 11. Forty-eight of the forty-nine carry a cluster assignment --- ceh22 is fitted and significant but sits outside the k = 5 partition, so the per-cluster counts sum to 48. A Mann-Whitney comparison of the C4 ridge-adjacent wells against the C2 and C3 dune-body wells does return significantly longer lags at C4 (3.67 against 2.00 months; U = 232.5, p \< 0.001, computed from the peak lags in 23_ridge_lag_fits.csv), but that ordering is a cluster-level contrast in drainage behaviour rather than a distance effect: within the network as a whole, lag is uncorrelated with ridge distance. The hypothesis of distance-dependent ridge transport is not supported.
...
**Table S7.**1. Equilibrium wetness index and MSL5 reconstruction, per well (n = 84). Levels are metres below ground surface, negative below the surface. The index is reconstructed from the open-dune calibration MSL5 = +0.378 + 0.964·EWI (r = 0.97, RMSE = 66 mm), fitted on the 62 in-scope wells carrying an observed five-year mean spring level. A further 2 open-dune well(s) carry an index but no valid five-year spring window and are reconstructed only; the 20 C4/C5 forest wells are reconstructed but held out of scope, their coefficients being the least constrained on the site. Status reads: \'Calibration\', an in-scope well that entered the fit; \'Reconstructed\', in scope but with no observed spring level; \'Out of scope\', a forest well outside the calibration\'s scope. Standard errors on the index propagate the drainage coefficient alone, the dominant term; the full three-coefficient variant is carried in 26_equilibrium_wetness_index_per_well.csv. Source: 26_equilibrium_wetness_index_per_well.csv, 26_ewi_msl5_comparison.csv.

  ------- ----------- ----------------------- -------- -------- ------ -------- -------- ------ ---------------
  CEH11   Reference   C1 (Lake Edge)          0.0925   -0.636   312    -0.233   -0.216   17     Calibration
  CEH23   Reference   C1 (Lake Edge)          0.0944   -0.570   320    -0.159   -0.153   7      Calibration
  CEH25   Reference   C1 (Lake Edge)          0.1343   -0.511   296    -0.107   -0.097   10     Calibration
  CEH26   Reference   C1 (Lake Edge)          0.0876   -0.513   346    -0.149   -0.098   51     Calibration
  CEH27   Reference   C1 (Lake Edge)          0.0984   -0.439   303    -0.045   -0.028   17     Calibration
  CEH5    Reference   C1 (Lake Edge)          0.0983   -0.530   300    -0.114   -0.115   -1     Calibration
  CEH6    Reference   C1 (Lake Edge)          0.1222   -0.407   263    -0.049   0.003    52     Calibration
  CEH12   Extended    C1 (Lake Edge)          0.0880   -1.209   403             -0.763          Reconstructed
  P2      Extended    C1 (Lake Edge)          0.1004   -0.487   350    -0.205   -0.074   132    Calibration
  CEH24   Reference   C2 (Dune)               0.0745   -0.441   447    0.015    -0.030   -45    Calibration
  CEH28   Reference   C2 (Dune)               0.0781   -0.648   394    -0.128   -0.227   -99    Calibration
  D10     Reference   C2 (Dune)               0.0595   -0.487   447    -0.050   -0.074   -24    Calibration
  D17     Reference   C2 (Dune)               0.0724   -0.430   485    -0.035   -0.019   16     Calibration
  D41     Reference   C2 (Dune)               0.0664   -0.501   441    -0.097   -0.087   10     Calibration
  D43     Reference   C2 (Dune)               0.0770   -0.667   312    -0.207   -0.246   -38    Calibration
  D44     Reference   C2 (Dune)               0.0902   -0.756   389    -0.305   -0.330   -25    Calibration
  D5      Reference   C2 (Dune)               0.0707   -0.616   432    -0.168   -0.197   -29    Calibration
  D6      Reference   C2 (Dune)               0.0672   -0.602   397    -0.138   -0.184   -46    Calibration
  D7      Reference   C2 (Dune)               0.0760   -0.525   383    -0.108   -0.110   -2     Calibration
  D8      Reference   C2 (Dune)               0.0568   -0.456   491    -0.058   -0.044   14     Calibration
  L7      Reference   C2 (Dune)               0.0853   -1.624   222    -1.137   -1.160   -23    Calibration
  NW3     Reference   C2 (Dune)               0.0674   -0.685   416    -0.178   -0.262   -84    Calibration
  NW4     Reference   C2 (Dune)               0.0597   -0.499   521    -0.102   -0.085   17     Calibration
  NW4B    Reference   C2 (Dune)               0.0611   -0.489   530    -0.118   -0.075   43     Calibration
  T41B    Reference   C2 (Dune)               0.0659   -0.413   450    -0.053   -0.002   50     Calibration
  T41C    Reference   C2 (Dune)               0.0708   -0.450   387    -0.073   -0.038   35     Calibration
  T41D    Reference   C2 (Dune)               0.0691   -0.679   408    -0.269   -0.257   12     Calibration
  WMC1    Reference   C2 (Dune)               0.0582   -0.621   489    -0.235   -0.201   33     Calibration
  CEH37   Extended    C2 (Dune)               0.0807   -0.602   291    0.037    -0.184   -220   Calibration
  CEH7    Extended    C2 (Dune)               0.0536   -0.819   421    -0.162   -0.390   -228   Calibration
  CEH8    Extended    C2 (Dune)               0.0334   -0.746   487    -0.299   -0.321   -22    Calibration
  PE      Extended    C2 (Dune)               0.0723   -0.878   405    -0.511   -0.447   64     Calibration
  CEH1    Reference   C3 (Western Residual)   0.0666   -0.682   413    -0.165   -0.260   -95    Calibration
  CEH10   Reference   C3 (Western Residual)   0.1012   -1.326   372    -0.880   -0.875   5      Calibration
  CEH18   Reference   C3 (Western Residual)   0.0607   -0.985   441    -0.586   -0.549   37     Calibration
  CEH21   Reference   C3 (Western Residual)   0.0437   -1.157   389    -0.848   -0.713   135    Calibration
  CEH36   Reference   C3 (Western Residual)   0.0506   -0.722   429    -0.393   -0.298   95     Calibration
  CEH39   Reference   C3 (Western Residual)   0.0335   -0.521   681    -0.174   -0.106   68     Calibration
  CEH4    Reference   C3 (Western Residual)   0.0691   -0.848   290    -0.493   -0.419   74     Calibration
  CEH40   Reference   C3 (Western Residual)   0.0371   -0.646   749    -0.309   -0.225   84     Calibration
  CEH41   Reference   C3 (Western Residual)   0.0373   -0.535   705    -0.143   -0.120   23     Calibration
  CEH42   Reference   C3 (Western Residual)   0.0518   -0.504   515    -0.082   -0.090   -8     Calibration
  CEH9    Reference   C3 (Western Residual)   0.0350   -0.725   647    -0.371   -0.301   71     Calibration
  D15     Reference   C3 (Western Residual)   0.0628   -0.423   417    0.055    -0.012   -67    Calibration
  D25     Reference   C3 (Western Residual)   0.0662   -0.721   341    -0.133   -0.297   -164   Calibration
  D38     Reference   C3 (Western Residual)   0.1089   -0.726   283    -0.257   -0.301   -45    Calibration
  D9      Reference   C3 (Western Residual)   0.0623   -0.566   445    -0.130   -0.149   -19    Calibration
  NW1     Reference   C3 (Western Residual)   0.1002   -1.626   281    -1.165   -1.161   4      Calibration
  NW11    Reference   C3 (Western Residual)   0.0756   -0.862   355    -0.382   -0.432   -50    Calibration
  NW13    Reference   C3 (Western Residual)   0.0885   -0.799   301    -0.334   -0.371   -37    Calibration
  NW2     Reference   C3 (Western Residual)   0.0816   -0.849   350    -0.348   -0.419   -71    Calibration
  NW5     Reference   C3 (Western Residual)   0.0498   -0.724   542    -0.290   -0.300   -10    Calibration
  NW6     Reference   C3 (Western Residual)   0.0585   -0.667   421    -0.199   -0.245   -47    Calibration
  NW7     Reference   C3 (Western Residual)   0.0462   -1.436   438    -0.923   -0.980   -57    Calibration
  T41A    Reference   C3 (Western Residual)   0.0963   -0.492   377    0.017    -0.078   -95    Calibration
  WMC2    Reference   C3 (Western Residual)   0.0474   -0.657   499    -0.233   -0.236   -3     Calibration
  WMC3    Reference   C3 (Western Residual)   0.0346   -1.314   685    -0.877   -0.864   13     Calibration
  WMC4    Reference   C3 (Western Residual)   0.0889   -0.831   287    -0.369   -0.402   -33    Calibration
  CEH22   Extended    C3 (Western Residual)   0.0560   -0.751   223    -0.473   -0.325   148    Calibration
  CEH35   Extended    C3 (Western Residual)   0.0652   -0.799   394    -0.552   -0.372   180    Calibration
  CEH38   Extended    C3 (Western Residual)   0.0600   -0.813   292    -0.499   -0.384   114    Calibration
  P1      Extended    C3 (Western Residual)   0.0467   -0.290   698             0.114           Reconstructed
  PW      Extended    C3 (Western Residual)   0.0550   -0.772   450    -0.402   -0.346   56     Calibration
  CEH2    Reference   C4 (Main Forest)        0.0191   -1.206   1462   -0.733   -0.760   -27    Out of scope
  CEH20   Reference   C4 (Main Forest)        0.0233   -1.156   1008   -0.857   -0.712   144    Out of scope
  CEH30   Reference   C4 (Main Forest)        0.0264   -1.665   821    -1.353   -1.199   154    Out of scope
  CEH32   Reference   C4 (Main Forest)        0.0163   -1.507   1658   -1.281   -1.048   234    Out of scope
  CEH33   Reference   C4 (Main Forest)        0.0226   -1.636   1014   -1.350   -1.171   179    Out of scope
  CEH34   Reference   C4 (Main Forest)        0.0094   -0.519   2574   -0.411   -0.104   307    Out of scope
  NW10    Reference   C4 (Main Forest)        0.0400   -1.080   611    -0.560   -0.640   -80    Out of scope
  CEH15   Extended    C4 (Main Forest)        0.0354   -1.373   538    -0.698   -0.920   -222   Out of scope
  FE1     Extended    C4 (Main Forest)        0.0156   -0.906   1298   -0.355   -0.473   -119   Out of scope
  LIS1    Extended    C4 (Main Forest)        0.0274   -0.948   1046   -0.319   -0.513   -195   Out of scope
  CEH16   Reference   C5 (Coastal Forest)     0.0390   -0.960   509    -0.646   -0.525   121    Out of scope
  CEH17   Reference   C5 (Coastal Forest)     0.0591   -2.199   303    -1.893   -1.708   185    Out of scope
  CEH19   Reference   C5 (Coastal Forest)     0.0445   -1.029   400    -0.761   -0.591   170    Out of scope
  CEH31   Reference   C5 (Coastal Forest)     0.0402   -1.618   630    -1.300   -1.154   146    Out of scope
  NW9     Reference   C5 (Coastal Forest)     0.0508   -0.913   415    -0.592   -0.481   111    Out of scope
  CEH3    Extended    C5 (Coastal Forest)     0.0559   -1.168   256    -1.027   -0.724   303    Out of scope
  FE2     Extended    C5 (Coastal Forest)     0.0473   -1.713   366    -1.269   -1.244   24     Out of scope
  FE3     Extended    C5 (Coastal Forest)     0.0627   -2.031   316    -1.641   -1.548   93     Out of scope
  FE4     Extended    C5 (Coastal Forest)     0.0475   -1.615   475    -1.153   -1.151   2      Out of scope
  NW8     Extended    C5 (Coastal Forest)     0.0522   -1.462   354    -1.322   -1.005   317    Out of scope
...
At the scraped well CEH36, the spring mean shifts post-scraping by +95 mm against the paired CEH4 control (p = 0.022) and +19 mm against the climate centroid (p = 0.79, not significant), versus +195 mm and +161 mm respectively for the summer minimum. Scraping raises the spring level too, by less than it raises the summer minimum, and the two controls --- which agreed in summer --- disagree in spring because every non-impact well also shifts negative against the climate centroid, the signature of the regional spring centroid itself having risen rather than of the impact wells falling. With n_pre = 4 years these p-values are fragile.

## S8.3 Clearfell BACI (Scripts 10d, 10l)

The clearfell tier-mean spring shifts against the Forest control (Script 10d) are Impact −2, Edge −107, Forest-control −9 and Climate-control −63 mm, none individually significant. The four-zone spring model (Script 10l; 14-year panel, R² = 0.59) returns the differential felling steps in Table S8.1. The Impact (−2 mm, p = 0.88) and C3/Warren (−11 mm, p = 0.56) steps are not significant, and the Impact step agrees with the per-well comparison against the same control (WMC3 −2 mm, p = 0.97; Script 10d). The Edge step is −53 mm (p = 0.027): negative, and so not a felling benefit; it is consistent with the maturing-replant confound at the Edge wells (main report Section 4.6.8) and rests on cluster-robust standard errors over very few clusters, so it is reported for completeness, not as a felling effect. In summer the felling signal sits at the Impact zone in the monthly ANCOVA, and neither seasonal branch of the four-zone model resolves a positive felling step. The spring panel spans 14 years (2011 + 2013--2025) against the summer panel\'s 13, because WMC3\'s Mar--May record is near-complete and 2019 --- unusable in summer --- is fully measured in spring, even under the stricter 3-of-3 rule.
...
# Supplementary Note S10: Reading the Network-Change Maps (Figures 72--75)

## S10.1 Purpose

The four network-change products of report Section 3.8.1 (Figures 72--75) are constructed from the same per-well spring (March--May) series and differ only in climate handling and frame of reference. Because the four constructions are described in necessarily dense parallel prose, this note provides the comparison at a glance: Table S10.1 sets out what each map asks, over what window, and in which frame; Figure S10.1 shows the shared construction as a flow schematic --- one source series, two frames (common mode removed versus absolute), four products.
...
Table S10.1. Construction of the four network-change products. Full construction detail is given in report Section 3.8.1; per-script methodology in the Methods Supplement (chapters S.20).
  ...
  Figure 72 --- Differential movement (Script 32)            Which wells are gaining or losing ground against the site as a whole?               2011--2025                                  Network-mean spring subtracted each year (common mode removed)        Relative to the site mean; mm yr⁻¹
  Figure 73 --- Absolute climate-removed trend (Script 36)   How fast is each well itself drifting, once the shared climate signal is removed?   2005--2025                                  Shared climate signal removed; not re-referenced to the network       Absolute, per well; mm yr⁻¹
  Figure 74 --- Amplification field (Script 33)              How strongly does each well swing between dry and wet springs?                      Dry and wet extreme springs of the record   Common mode removed; expressed as a ratio to the network-mean swing   Relative amplification; dimensionless (×)
  Figure 75 --- Dry-year spring depth (Script 33)            How close does a dry spring already sit to the ecological thresholds?               Dry extreme springs of the record           None --- the absolute dry state is the point                          Absolute depth below ground; m
  ---------------------------------------------------------- ----------------------------------------------------------------------------------- ------------------------------------------- --------------------------------------------------------------------- -------------------------------------------

![](Pictures/1000000000000BB8000004386772590A.png){width="15.921cm" height="5.731cm"}

Figure S10.1 (S10_network_change_schematic.png). Construction schematic for the network-change products: the per-well spring series feeds two frames --- common mode removed (Figures 72 and 74) and absolute (Figures 73 and 75) --- yielding the four maps. A per-well climate-sensitivity coefficient (Script 35) and a two-window sensitivity demonstration (Script 34; Note S11) accompany the group.
...
The 2017→2023 pair gives −105 mm, but this is one interior cell of a wide envelope: across the 55 admissible window pairs the site-mean change spans −0.16 to +0.23 m (−161 to +230 mm) and changes sign --- 17 falling, 38 rising --- with the most negative pairing 2017→2019 (−161 mm) and the most positive 2015→2024 (+230 mm, resting on the exceptionally wet 2024 spring). A single admissible change of window pair can therefore move the site-mean estimate by more than a third of a metre and reverse its sign, and a trend fitted to the site-mean spring level across the full record is not significantly different from zero (−4.8 mm yr⁻¹, AR-corrected, p = 0.63). The window-independent secular signal is recovered instead by the differential-movement and absolute-trend maps (report Figures 72 and 73).
