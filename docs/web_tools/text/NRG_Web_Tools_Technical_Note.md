...
# A2. Data Bundle Structure
  ...
  **Key**                 **Contents**
  ...
  winter_climatology_mm   Mean Oct--Mar cumulative rainfall (\~516 mm), the denominator for λ.
...
R² values range from 0.41 (Lake Edge summer) to 0.92 (Forest summer). When R² \< 0.50 the forecast card displays a low-explanatory-power caveat. P_winter is the climatological Oct--peak_month total scaled by λ; P_summer is the climatological Apr--Sep total (always at λ = 1.00).
...
where d is depth below ground (m, positive), A is slope (mm/m), and B is intercept (mm). The result is normalised as λ = P_flood / P_clim_total.
...
The Scenario Viewer (scenario_viewer.html) is generated by Script 19 (19_spatial_groundwater.py, v2.8.1). All computation is client-side JavaScript; no server requests are made after the initial page load.
...
The seasonal extremes scatter (14_seasonal_extremes_scatter.html) is generated by Script 14 (14_climate_projections.py, v1.2.1). It reads per-well summary statistics from the well network table (00_well_network_table.csv) and cluster assignments from 02_cluster_stats.csv.
