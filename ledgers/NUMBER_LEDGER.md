...
| N-04b | Analytical phases | **15** | manifest `analytical_phases` | stable | **none — do not cite** | ⚠ the working-rules box says "17 analytical phases"; the manifest says 15 and the field is explicitly not for citation. See D-012. |
...
| N-06b | Wells carried in `01_wells_clean.csv` | **78 columns = 77 dipwells + `llyn rhos`** | `outputs/01_wells_clean.csv` | stable | Scripts 33/35 | **77 is the amplification-metric well count** (lake gauge excluded, CEH13/CEH14 included — D-017), confirmed against the 2026-06-27 delta. The 11 wells in reference+extended but absent here (`FE4, L1, LIS1, ceh12, ceh35, ceh38, nw12, p1, p2, pe, pw`) lack usable series. **88 ≠ 78 ≠ 77 — all three are correct on different bases; never substitute one for another.** |
...
| N-07 | Cluster membership | C1 **7** · C2 **24** · C3 **21** · C4 **9** · C5 **5** | `outputs/02_clustering/02_07_cluster_membership_k5.csv`, config labels | stable | R8, MS, P1 | supersedes April-2026 8/28/22 — see D-008 |

## C. SSM cluster coefficients — `03_03_cluster_mechanistic_coefficients.csv`

Volatility: **stable**, but **N-08…N-12 for C4 are `MOVING`** — D-004 (CEH13/CEH14
...
|----|----------|-----------------|--------|-------|
| N-13 | C4 centroid β₃ (canonical) | **0.0183**, p = 0.0017 | `30_c4_report_numbers.csv` `c4_centroid_beta3` | matches N-11; `MOVING` under D-004 |
| N-14 | C4 VIF (identifiability) | **1.0934** — network minimum | `30_c4_report_numbers.csv` | the number that refuted the triangulation premise (D-003) |
...
| N-18 | C4 open-dune conservative bound | **0.058** | retained as a bound only | ⚠ **not** a triangulation result — see D-003 |

## D. Datum

| ID | Quantity | Committed value | Source | Volatility | Notes |
|----|----------|-----------------|--------|-----------|-------|
| N-19 | `DRAINAGE_DATUM` | **3.7 m** below ground | `src/utils/config.py` | constant | D-002 |
...
## E. Specific yield — **the two aggregations are not interchangeable (D-007)**
...
## F. λ — the drawdown reach (**by name, never by number** — D-013)
...
## H. Water balance — `16_water_bal_table.csv` (**Sy-free**, D-010)
...
`MOVING` — C4 and the site aggregate change under D-004. The C4 closure-minimising
...
|----|----------|-------|-------|
| N-43 | `HEADLINE_LAG` | **0** | no rainfall lag — D-009 |
| N-44 | `FOREST_INTERCEPTION` | **0.24** | a partition of the PET budget, applies to C4 **and** C5 — D-011 |
...
| ID | Source | Problem |
|----|--------|---------|
| N-52 | `outputs/30_c4_constrained_fit/` | archived outputs of a **retired** script; produced by no live script (D-003) |
