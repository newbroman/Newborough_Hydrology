...
*Document version: 1.6.2 (June 2026).*
...
3. **Intermediate files** — outputs written to `OUT_DIR` root by upstream scripts and read by downstream scripts. These are named with the `INT_` prefix: `INT_WELLS_CLEAN`, `INT_CLIMATE`, `INT_LOCATIONS`, `INT_MASTER_DATA`, `INT_REGIONAL_AVG`, `INT_CLUSTER_AVG_MAOD`, `INT_CLUSTER_PEAK_MONTHS`, and so on.
4. **Final outputs** — figures, tables, and report CSVs written to per-script subdirectories. These are named with the `OUT_NN_` prefix to indicate which script writes them: `OUT_03_MECHANISTIC_TABLE`, `OUT_10E_COEFF_SHIFTS`, `OUT_25_FIT_PARAMETERS`, and so on.
...
$$\text{PET}_{\text{u}} = 16 \left( \frac{10\,T}{I} \right)^{a}$$
...
1. Loads `INT_MASTER_DATA` and merges DEM ground elevations from `INT_WELL_ELEVATIONS` on the normalised well name.
...
**Outputs.** Sharing `paths.DIR_14` with Script 14:
- `14b_year_of_crossing.csv` — per-cluster × threshold table with median + 5th/95th percentile crossing years, OLS slope, intercept, and current-year (2025) predicted depth.
- `14b_year_of_crossing.png` — five-panel figure (one per cluster) showing observed minima, OLS trend, 90% bootstrap CI cone (5th–95th percentile), threshold lines and crossing-year CI bands.
- `14b_year_of_crossing_results.md` — memo with headline table, decision rule, and caveats.
...
- §7 Conclusion 11 of the main report — the "around 2030–2032" qualitative date band is replaced by the bootstrap CI from this script (median 2022, 90% CI 2017–2043 for C1 SD16).
...
| Cluster | Winter − Summer (mm) | *p* | Interpretation |
|---|---:|---|---|
| C3 Western Residual | **+7.4** | < 0.001 | Substantive positive contrast |
| C4 Main Forest | +6.2 | 0.086 | Marginal positive contrast |
...
**Analysis-specific exclusion of CEH13 and CEH14 from MSL5 (v1.2.0, 2026-06-25).** Two wells — CEH13 and CEH14 — are excluded from every derived MSL5 product: the Method A cluster-mean trajectory, the latest-per-well table, the IDW change map, and the quadrat figure. Both wells have a degenerate SSM drainage coefficient (CEH13 near-zero β₃, a τ outlier; CEH14 negative β₃, SSM failure, NSE = −3.21) that gives them anomalously long drainage memory. Within a five-year window, this autocorrelation makes their spring readings correlated across years, so their MSL5 value — and especially their MSL5 change between windows — is unreliable as a monitoring metric. The exclusion criterion is the MSL5-side application of the same drainage-memory rule that already excluded CEH13 and CEH14 from the τ = Sy/β₃ storage–drainage index map (§S.12): both are excluded from the τ map on β₃ ≤ 0 or near-zero grounds; the same rationale applies here to the five-year windowed spring metric. The excluded wells are held in `config.MSL5_EXCLUDED_WELLS` (keys lowercase to match the normalised well column in the per-well CSV). The rows are **retained** in `26_msl_5yr_per_well.csv` with two new columns, `msl5_excluded` (boolean) and `msl5_excluded_reason`, for transparency; a filtered subset with excluded rows removed feeds all downstream products. This is an analysis-specific exclusion that does not affect either well's membership of the clustering, SSM, BACI, or any other pipeline analysis.
...
## S.19  Scripts 28, 29, 30 — Cluster framework diagnostics (Phase 14)
...
| Input | Source | Role |
|---|---|---|
| `03_regional_averages.csv` | Script 03 | C4 centroid monthly hydrograph |
| `03_03_cluster_mechanistic_coefficients.csv` | Script 03 | Unconstrained centroid β coefficients |
| `16_water_balance/16_01_water_balance_summary.csv` | Script 16 | Cluster-mean P, PET, h_disp for partition |
| `03_master_data.csv` | Script 03 | Per-well β₃ values for the identified-wells summary |

**Methodology.** The C4 centroid hydrograph is re-fitted through `model_utils.fit_ssm(fixed_beta_3=…)` with β₃ held at a triangulation anchor: C4 β₃ ≈ C2 β₃ (0.073) + Δβ₃_tree, where Δβ₃_tree = −0.015 is inferred from the inland-C5 minus C3 contrast (coastal-corrected), giving an anchor of 0.058 month⁻¹. The script then partitions the C4 loss budget at three β₃ values — the unconstrained centroid, the residual-minimising β₃, and the anchor — reading cluster means (P, PET, h_disp) from Script 16's table so the unconstrained partition is identical to Table 4a of the main report (verified: the centroid re-fit at the unconstrained β₃ reproduces Script 16's β₂ = 2.548). The partition convention follows Script 16 exactly: drainage = β₃ · h_disp, atmospheric draw = β₂ · PET, residual = β₁ · P − (drainage + draw). No values are hardcoded — the means come from Script 16 and the anchor is recomputed at run time.

**Headline result.** Per-well fitting reveals a β₂–β₃ trade-off. Five of the nine C4 wells constrain β₃ (NW10 0.043, CEH20 0.030, CEH30 0.024 at p ≤ 0.05; CEH2 0.023, CEH33 0.020 at p ≈ 0.10); three do not (CEH13, CEH32, CEH34); and CEH14 fails the SSM outright (iterative NSE = −3.21). The identified wells centre on 0.025–0.030, below the anchor (0.058). Holding the centroid at the anchor moves β₂ from 2.55 to 1.86 — close to the open-dune values (C2, 1.66; C3, 1.77) — and shortens t_R = 1/β₃ from 49 to 17 months. The water-balance partition remains atmospheric-draw-led across the identified range: drainage takes 24 % of the loss budget at the unconstrained centroid, approximately 30 % at the residual-minimising β₃ (0.0255), and crosses into a drainage-led 56 % only at the anchor. The closure residual is smallest at β₃ ≈ 0.026 (essentially zero) and grows in both directions — +0.8 % of the loss budget at the unconstrained value, −4 % at the anchor (the largest closure residual in the network).

**Implication.** The forest-interior drainage coefficient is bounded, not pinned. The identified wells, the β₂–β₃ trade-off, and the water-balance closure converge on β₃ ≈ 0.026 (t_R ≈ 39 months), so C4 drains more slowly than the open dune (C2 t_R ≈ 14 months) — a real canopy-and-substrate effect — but remains atmospheric-draw-dominated at the identified range, neither as sluggish as the unconstrained centroid nor as drainage-led as the anchor. The report retains the unconstrained fit as its primary record; this script's numbers appear in main-report §5.2.3 only.

**Outputs.**

| File | Contents | Downstream consumer |
|---|---|---|
| `30_c4_constrained_fit/30_c4_constrained_perwell.csv` | Per-well β₃, β₂, p-value, identification status, exclusion flag | §5.2.3 per-well table |
| `30_c4_constrained_fit/30_c4_constrained_fit.png` | β₂–β₃ trade-off and water-balance partition sensitivity figure | §5.2.3 figure |
| `30_c4_constrained_fit/30_c4_constrained_report_numbers.csv` | 25 named keys — single source for all §5.2.3 citations | §5.2.3 |

**Trace map (§5.2.3 citation → CSV key).** β₂ 2.55 → `c4_beta2_centroid_uncon`; β₂ 1.86 → `c4_beta2_centroid_anchor`; β₃ 0.020 → `c4_beta3_centroid_uncon`; β₃ 0.058 → `c4_anchor_beta3`; β₃ 0.026 → `c4_beta3_bestclosure`; t_R 49/17/39/14 months → `c4_tr_uncon`/`c4_tr_anchor`/`c4_tr_bestclosure`/`c2_tr`; drainage partition 24 %/30 %/56 % → `c4_drainage_pct_uncon`/`_bestclosure`/`_anchor`; closure +0.8 %/−4 % → `c4_wb_resid_uncon`/`c4_wb_resid_anchor`; per-well β₃ → `30_c4_constrained_perwell.csv`.

**Limitations.** The triangulation anchor is a cross-cluster construct; its C5–C3 tree-effect proxy demonstrably under-estimates the suppression seen at the wells that do constrain β₃, so it functions as an upper bound on the forest drainage rate rather than a best estimate. Per-well fits are individually noisy (≈ 100-month windows). CEH14 is excluded throughout (ridge-flank SSM failure, NSE −3.21). The sensitivity is diagnostic only — it is not propagated into the canonical coefficient masters, the water balance, or the spatial maps, which all retain the unconstrained fit.

**Cross-references.**

- §S.3 — canonical unconstrained SSM fits; `03_master_data.csv` and `03_03_cluster_mechanistic_coefficients.csv`.
