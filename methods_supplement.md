...
# Newborough Warren Methods Supplement
...
Cross-references work in both directions. The supplement points to the report wherever a result is discussed (`§4.2`, `Table 3c`, `Figure 8b`); the report’s §3 Methods should be read in conjunction with the relevant chapters here when methodological detail is needed.
...
# F.2 Field protocol, bucketing convention, and date semantics
...
### History: why HEADLINE_LAG was 1 and is now 0
...
### Note on maOD invariance to ground-surface change
...
### Reporting dates in the report and supplement
...
# F.3 The state-space model — displacement formulation

The state-space model (SSM) is the methodological core of the analysis. Every cluster characterisation, every BACI step, every forecasting threshold, every scenario response ultimately rests on it. This section gives the canonical form.
...
- D is the drainage datum, `DRAINAGE_DATUM = 3.7 m`. See Drainage datum below.
...
- The β₃ column is `−(D + h_prev)`. A positive β₃ means displacement above the datum drives drainage downward (Δh decreases as displacement increases).
...
```
`fit_ssm(h_series, climate, lag=None, window=None,``        drainage_datum=DRAINAGE_DATUM, min_obs=MIN_OBS,``        provenance=None, exclude_interpolated=False)``fit_ssm_intercept(h_series, climate, lag=None, window=None,``                  drainage_datum=DRAINAGE_DATUM, min_obs=MIN_OBS,``                  provenance=None, exclude_interpolated=False)`
```
...
### Two regimes: no-intercept (A) and with-intercept (B)
...
# F.4 Configuration, constants, and the cluster partition
...
| `UKCP18_DRY_*`, `UKCP18_WET_*` | seasonal P and PET scaling factors | UKCP18 RCP8.5 Wales, 50th percentile, 2050s |
...
| ID | Label | Anchor wells | Block label | Substrate / vegetation |
|---|---|---|---|---|
...
### The identity-vs-integer keying principle
...
### Specific yield: two values per cluster

Two independent methods produce per-cluster Sy estimates, and both are reported in the report. The supplement chapters for Scripts 16 (water balance), 17 (WTF cluster Sy), and 18 (WTF spatial Sy) describe each in detail. For reference here:

| Cluster | Fetter mass-balance Sy | WTF cluster median Sy (corrected, Script 17) | WTF well-level cluster mean (Script 18) |
|---|---|---|---|
| C1 Lake Edge | 0.08 | 0.223 | 0.217 |
| C2 Dune | 0.12 | 0.234 | 0.227 |
| C3 Western Residual | 0.12 | 0.259 | 0.257 |
| C4 Main Forest | 0.12 | 0.227 (corrected) / 0.215 (uncorrected) | 0.202 |
| C5 Coastal Forest | 0.12 | (corrected variant produced by Script 17 under `FOREST_CIDS = (4, 5)`; see Phase 6 chapter) | (see Phase 8 chapter) |
...
- Script 11c (`11c_pflood_achievability.py`) — Per-well P_flood achievability categorical priority map. Operationalises Conclusion 4’s λ < 1.5 priority criterion. Step 13/35, Phase 3. Routed from the 2026-05-29 post-review cascade. Documented in S.9.3.
- Script 14b (`14b_year_of_crossing.py`) — Bootstrap year-of-crossing for Curreli (2013) ecological thresholds. Replaces the qualitative “around 2030–2032” framing of Conclusion 11 with stated CIs. Step 16/35, Phase 4. Routed from the 2026-05-29 post-review cascade. Documented in S.8.5.
- Script 25 (`25_coastal_gradient.py`) — Coastal-retreat gradient analysis. Step 26/35, Phase 11. Documented in S.15.
- Script 26 (`26_van_willegen_msl.py`) — Van Willegen 2025 observational 5-year MSL aggregation. Step 30/35, Phase 13. Documented in S.18.
- Script 26b (`26b_van_willegen_msl_projections.py`) — UKCP18 RCP8.5 MSL5 climate projections (Tool B). Step 31/35, Phase 13. Documented in S.18b.
- Script 26c (`26c_msl5_report_figures.py`) — MSL5 report-format figures for §4.8.4 and §4.10.1 of the main report. Display-only companion to Scripts 26 and 26b; reads only canonical outputs from Scripts 26, 26b, and 19; no recomputation. Step 32/35, Phase 13. Excluded from the supplement’s 33-step analytical count. Documented in S.18c.
- Script 28 (`28_c3_detrend_check.py`) — C3 detrend check; quantitative validation of the aquifer-architecture framing. Step 33/35, Phase 14. Routed from the 2026-05-29 post-review cascade. Documented in S.19.1.
- Script 29 (`29_c3_within_variance_check.py`) — Within-C3 variance attribution; characterisation of the within-cluster hydrogeological axis. Step 34/35, Phase 14. Routed from the 2026-05-29 post-review cascade. Documented in S.19.2.
- Script 27 (`27_greyscale_figures.py`) — Post-pipeline greyscale figure-rendering utility. Step 35/35, Phase 15. Excluded from the supplement’s 33-step analytical count. Documented in Appendix A.
...
- `MSL_DEFAULT_WINDOW_YEARS = 5` — the 5-year averaging window length; sensitivity-tested in van Willegen 2025 Figure 4.
...
UKCP18 multipliers used by Tool B are central-estimate Wales 50th-percentile values for RCP8.5 at the 2050s and 2080s. The 2050s multipliers exist in `utils/config.py` as the `UKCP18_DRY_*` / `UKCP18_WET_*` constants (already used by Script 19’s scenario viewer); the 2080s multipliers are currently hardcoded in Script 26b’s `UKCP18_SCENARIOS` dictionary. Centralising the 2080s constants in `config.py` is documented as a follow-up in the Tool B changelog and in §S.18b.6.

### `pipeline_params.py` — the consolidated scenario parameter file
...
Single source of truth for every input and output file path. The module is organised into four blocks:
- Root directories — `ROOT_DIR`, `DATA_DIR`, `OUT_DIR`, and per-script subdirectories `DIR_00` through `DIR_27`, plus `DIR_26B` for Script 26b’s outputs. A helper function `make_all_dirs()` creates any missing directories. Scripts call this at the top of their main function rather than checking individual directories.
- Data inputs — paths to raw CSV and GIS files in `data/`: `DATA_WELLS_RAW` (the cleaned monthly dipwell CSV), `DATA_LOCATIONS_RAW`, `DATA_CLIMATE_RAW`, the KML files (`Features.kml`, `streams.kml`, `clearfell.kml`, `broadleaf_restock.kml`), the DEM (`newborough_dem.tif`), and the precomputed coastal-distance CSV (`DATA_DIST_COAST`).
- Intermediate files — outputs written to `OUT_DIR` root by upstream scripts and read by downstream scripts. These are named with the `INT_` prefix: `INT_WELLS_CLEAN`, `INT_CLIMATE`, `INT_LOCATIONS`, `INT_MASTER_DATA`, `INT_REGIONAL_AVG`, `INT_CLUSTER_AVG_MAOD`, `INT_CLUSTER_PEAK_MONTHS`, and so on.
- Final outputs — figures, tables, and report CSVs written to per-script subdirectories. These are named with the `OUT_NN_` prefix to indicate which script writes them: `OUT_03_MECHANISTIC_TABLE`, `OUT_10E_COEFF_SHIFTS`, `OUT_25_FIT_PARAMETERS`, and so on.
...
- `fit_ssm_intercept(...)` — with-intercept OLS (Model B), returning the same dict plus α and its p-value. Used by Scripts 07, 08, 22, 24.
- `simulate_ssm(h0, P, PET, b1, b2, b3, drainage_datum)` — iterative forward simulation using the displacement recurrence `h(t) = (1−β₃)·h(t−1) + β₁·P(t) − β₂·PET(t) − β₃·D`. Used by Scripts 07, 08, 09, 15.
...
- `compute_intercept_audit(target_well_name, df_clean, df_climate)` — Model A vs Model B comparison for a single well, used by Scripts 07 and 08 in the LCSC benchmarking.
Two named thresholds: `MIN_OBS = 30` (minimum aligned rows for a per-well fit, applied inside `fit_ssm` and `fit_ssm_intercept`) and `LCSC_DATA_LIMIT = 100` (most-recent-window length for per-well fits in the intercept audit).
...
Shared module for the Script 10 clearfell BACI suite (10a–10l). Provides:
...
The suite-shared module makes it possible to change the impact-well list, the BACI dates, or the distance-weight function in one place, and have all twelve sub-scripts pick up the change consistently.
...
```
`from utils.config import DRAINAGE_DATUM, CLUSTER_LABELS, FOREST_CIDS``from utils.paths import (``    INT_WELLS_CLEAN, INT_CLIMATE, INT_MASTER_DATA,``    OUT_NN_FIGURE, OUT_NN_TABLE,``)``from utils.model_utils import fit_ssm, build_ssm_frame, assert_physical_signs``from utils.data_utils import clean_well_series, normalize_well_name`
```
...
## S.1 Script 01 — Data preparation
...
```
d = pd.to_datetime(wells.index, dayfirst=True, errors="coerce")prev = (d.to_period("M") - 1).to_timestamp()this = d.to_period("M").to_timestamp()wells.index = np.where(d.day <= 15, prev, this)
```
...
| File | Contents | Downstream consumers |
|---|---|---|
...
| `01_well_elevations.csv` | Ground elevation, upstand, pipe-top per well | Scripts 03 (upstand correction), 10–21 |
...
## S.2 Script 02 — Behavioural clustering
...
The script proceeds in seven blocks executed in order in `__main__`:
...
6. Cluster assignment write-out. The remapped labels are saved to `02_cluster_stats.csv` with columns `Match_ID` (normalised well name), `Name_Original`, `Cluster` (canonical integer ID), and `Cluster_Label` (human-readable label from `CLUSTER_LABELS`). This file is the contract for downstream consumers — Script 03 reads it to define the per-cluster SSM fits, and most analysis scripts from 04 onwards depend on it.
...
- Drought-summer normalisation in the amplitude descriptor. The post-2018 window contains three drought summers (2005, 2018, 2022) that disproportionately inflate raw `p90 − p10` in any well that responds to those summers. The climate-normalised variant drops the Jun–Sep months of those years, identified empirically against the 1931–2017 RAF Valley mean rather than against a model-derived definition; this preserves auditability and means the list does not change if a future analysis revises the PET model. The unnormalised variant is reported alongside so the reader can see what the normalisation does.
...
| File | Contents |
|---|---|
| `02_cluster_stats.csv` (INT_) | Per-well: `Match_ID`, `Name_Original`, `Cluster`, `Cluster_Label` |
...
- §4.2 Cluster characterisation and description — built from the cluster-hydrograph figure and the amplitude descriptors.
- Table 2 — cluster mechanistic coefficients (β₁, β₂, β₃) per cluster, sourced from Script 03 but indexed by the partition defined here.
- Figures 6–8 — dendrogram, validation, and cluster-hydrograph figures from `02_clustering/`.
...
## S.3 Script 03 — State-space regression and LCSC
...
| `01_wells_clean.csv` | Cleaned depth time series, depth-below-pipe-top convention (Script 01) |
...
Centroid construction — `build_cluster_centroids` and `build_upstand_lookup`. Each cluster centroid is the arithmetic mean of its member wells’ depth series, with one twist: each member well is upstand-corrected before averaging. A well’s raw depth reading is depth below the pipe top, which sits some `Upstand_m` above the ground surface; subtracting the upstand converts the series to depth below ground, the common datum shared by every well. The upstand lookup (`build_upstand_lookup`) is built from `01_well_elevations.csv`; wells without an upstand entry are averaged uncorrected, with a console count so the investigator can audit the warning. An `upstand_audit()` diagnostic prints the wells with `Upstand_m > 0.30` — under the live network, CEH2 (~0.71 m), NW2 (~0.59 m), and t41a (~0.40 m) are the three flagged; CEH2 is deliberately tall to remain visible in the forest understorey, NW2 and t41a are the other two ordinary cases. The flag is informational only.
Per-well fits — `per_well_fits`. Each reference-network well is fitted individually via `model_utils.fit_ssm()` (F.3 covers the function’s interface), with the well’s depth series upstand-corrected first so the `DRAINAGE_DATUM = 3.7 m` displacement reference is anchored consistently to the ground surface. The per-well fit uses `window=LCSC_DATA_LIMIT = 100` — the most recent 100 valid monthly rows — so that all per-well coefficients describe behaviour over the same recent window. This is the LCSC window subsequently used by Script 07 and 08; the rationale for choosing it here is downstream consistency. Per-well sign violations (β₁ < 0 or β₂ < 0) are reported but do not halt the pipeline — only centroid violations halt, because centroid fits are the values the report cites. A second LCSC estimate, the empirical LCSC, is computed alongside the regression LCSC for cross-check: rainfall divided by Δh on each unambiguous recharge event (Δh > 0.02 m), with the screen for spurious ratios (LCSC_raw outside [0.05, 1.0]) and a 10-event minimum. The regression LCSC is the one carried forward.
...
| Cluster | n | β₁ | β₂ | β₃ | R² | LCSC (mm per 10 cm) |
|---|---|---|---|---|---|---|
| C1 (Lake Edge) | 236 | 4.58 | 0.96 | 0.090 | 0.732 | 21.8 |
| C2 (Dune) | 247 | 3.98 | 1.77 | 0.066 | 0.747 | 25.1 |
| C3 (Western Residual) | 248 | 3.57 | 1.85 | 0.058 | 0.813 | 28.0 |
| C4 (Main Forest) | 236 | 2.52 | 2.55 | 0.020 | 0.685 | 39.7 |
| C5 (Coastal Forest) | 238 | 2.41 | 1.32 | 0.045 | 0.680 | 41.5 |
...
Lag diagnostic — `lag_diagnostic`. Each cluster centroid is refitted at rainfall lags 0, 1, 2, 3 months. The result is a 5 × 4 grid of fits (`03_04_lag_diagnostic.csv`). The diagnostic is not a model-selection step — `HEADLINE_LAG` is fixed in `config.py` — but it is a check that the bucketing-fix-era data (F.2) actually places the rainfall–response pairing where it belongs. Under the live data, all five clusters maximise R² at lag 0: C1’s R² drops from 0.732 at lag 0 to 0.037 at lag 1, C2 from 0.747 to 0.136, C3 from 0.813 to 0.197, C4 from 0.685 to 0.384, C5 from 0.680 to 0.221. At lag 3 the β₁ sign goes wrong for three of five clusters. This confirms the F.2 reasoning: once Script 01 buckets readings by field convention, current-month rainfall is the right predictor.
Bootstrap — `bootstrap_centroid_fits`. For each cluster, draw `n_members` wells with replacement, build the upstand-corrected centroid over that resample, refit `fit_ssm()`. Repeat `N_BOOTSTRAP = 1000` times with `BOOTSTRAP_SEED = 20260424`. The replicates yield a distribution per cluster from which 2.5 % and 97.5 % percentile CIs are computed for β₁, β₂, β₃, R², and LCSC, plus the fraction of replicates with β₁ > 0. The well-level (not month-level) resample is deliberate: the question being asked is how sensitive is the cluster β estimate to which wells happen to be in the cluster? — not how sensitive is a single well’s β to which months are observed? The latter is the per-well CI question that the OLS p-values already answer. Under the live data, all five clusters’ β₁ CIs are comfortably positive (β₁ frac positive = 1.000 in every cluster), the CIs are narrow (C1: 4.25–4.93; C5: 2.20–2.65), and the bootstrap β median is within 1.3 % of the centroid headline β₁ everywhere (the largest gap is C4 at 1.2 %; the other four clusters are within 0.4 %). The bootstrap output is also the source of the error bars on `03_01_mechanistic_signatures.png`.
Leave-one-out — `leave_one_out_fits`. For each cluster with ≥ 4 members, remove each well in turn, refit the centroid SSM on the remaining members. Useful for spotting clusters dominated by a single well. The full table is `03_06_leave_one_out.csv`. Under the live data, no cluster shows catastrophic single-well dependence: β₁ ranges from leaving out the most-influential well are within ~ 7 % of the headline. The single most-influential well by R² recovery is CEH2 in C4 Main Forest — dropping it shifts β₁ from 2.52 to 2.41 and increases R² from 0.685 to 0.729. That recovery (+0.043) is the largest in the LOO sweep; CEH2 is also co-largest on β₁ shift (tied with NW10, both ≈ 0.11). The substantive interpretation, taken up in S.7, is that CEH2 sits at a ridge-recharge interface where the SSM’s single-equation form does not fully capture the local subsidy; removing it from the centroid produces a fit closer to the “pure” forest signal. C5’s β₁ LOO range (2.34–2.54) is similar in width to C4’s, despite C5 having only five members, but no single C5 well is as influential as CEH2 in C4.
...
Spatial datum maps — `make_well_datum_maps`. `plot_metric_map` from `map_utils` (F.5) produces two publication-quality spatial maps: per-well R²-maximising datum and per-well R²-gain over the uniform datum. DEM background, KML overlays (`Features.kml`, `streams.kml`, `clearfell.kml`), cluster-shape markers from `config.CLUSTER_MARKERS`, dual colorbars, legend — all the standard rendering described in F.5. These are the only spatial maps Script 03 produces; per-well β maps are Script 07’s job.
Summary table and signatures figure — `build_summary_table`, `make_signatures_figure`. The summary table merges the centroid mechanistic coefficients with the bootstrap CIs and an amplitude-heterogeneity flag (which loads from Script 02’s `02_08_cluster_amplitude_per_well.csv` if present, with a hard-coded fallback). The amplitude flag is a heuristic: a cluster is flagged “heterogeneous” if the post-2018 ratio of its highest to lowest per-well `p90 − p10` amplitude exceeds 1.5. The flag drives no analytical choice — it is a reader-facing diagnostic. The three-panel signatures figure (`03_01_mechanistic_signatures.png`) is the bar chart of β₁, β₂, β₃ per cluster with bootstrap error bars, used as Figure 8 in the main report.
...
- Per-well upstand correction inside `per_well_fits`, not just at the centroid. Before April 2026 the script only upstand-corrected the centroid. Wells with tall upstands — CEH2 at ~0.71 m, NW2 at ~0.59 m, t41a at ~0.40 m — had their depth series fitted against a `DRAINAGE_DATUM` anchored to the ground surface even though the depth values themselves were relative to the pipe top. This produced systematic, well-by-well displacement errors at the few wells where the offset was large enough to matter. The per-well correction (`h_corrected = wells_clean[col] - u` before each `fit_ssm()` call) closes that gap. The change is small for most wells (median upstand is well under 30 cm) but unambiguous for the three flagged wells.
- Cluster centroid as arithmetic mean of upstand-corrected member wells. The simplest defensible aggregation. Median, record-length-weighted mean, and PCA-derived centroids were all considered during the rebuild and rejected as adding complexity without measurable benefit on this dataset — the within-cluster amplitude distributions are well-behaved enough that the mean is not pulled around by outliers.
...
| File | Contents | Principal downstream consumers |
|---|---|---|
| `03_master_data.csv` (INT_) | Per-well β₁, β₂, β₃, p-values, R², n, LCSC, cluster ID, easting/northing | Scripts 07, 08, 11, 16, 17, 19, 21 |
| `03_regional_averages.csv` (INT_) | Cluster-centroid monthly hydrographs (depth below ground) + P, PET in mm | Scripts 11, 16, 21 |
| `03_regional_averages_maod.csv` (INT_) | Same in absolute m AOD | Script 21 (forestry scenarios) |
| `03_cluster_peak_months.csv` (INT_) | Long-term mean peak month per cluster | Scripts 11, 11b |
...
| `03_state_space_model/03_10_well_datum_r2max_map.png` | Spatial map of per-well R²-maximising datum | Report Figure (§3.4) |
| `03_state_space_model/03_10_well_r2_gain_map.png` | Spatial map of R² gain vs uniform datum | Report Figure (§3.4) |

All paths resolve through `utils/paths.py` (`INT_MASTER_DATA`, `INT_REGIONAL_AVG`, `INT_CLUSTER_AVG_MAOD`, `INT_CLUSTER_PEAK_MONTHS`, `OUT_03_*`, `DIR_03`).
...
- C1 Lake Edge changed regime in 2018. The pre/post-split-window diagnostic shows non-overlapping β CIs across both windows. The single-fit C1 cluster characterisation in Table 3 is the average across both regimes. Where C1 results are quoted in the main report, the split-window outcome should be cited alongside as a caveat.
...
- The uniform 3.7 m datum is a deliberate trade, not an empirical optimum. It sits above the live empirical minimum (1.7 m) and is the value at which the two forest clusters’ β₃ identification is comfortably significant rather than near the threshold. The cost of going to 3.7 m rather than staying at the empirical minimum is an R² reduction of −0.052 at C1 Lake Edge and −0.029 at C2 Dune, both of which remain astronomically significant on β₃ regardless of depth. The benefit is a twentyfold improvement in C4 β₃ significance and substantially better fit at C5. For cluster-level work the uniform 3.7 m is the right choice; for any single-well question the per-well datum outputs (`03_09_well_optimal_datums.csv`) should be consulted — the mean network R² penalty from using the uniform datum at every well, rather than the per-well R²-maximising value, is +0.028 across the network, heaviest at C1 Lake Edge (+0.069) and lightest at C4 Main Forest (+0.001).
- C4 Main Forest has the weakest β₃ identification of the five clusters. Even at the canonical 3.7 m datum, C4’s β₃ p-value (0.0027) is several orders of magnitude larger than the other four clusters’ (which sit at 10⁻¹⁶ to 10⁻³⁵). The datum choice was deliberately set deeper than the empirical minimum to move C4 away from the significance boundary, and the move worked — but C4 remains the cluster where the SSM most underdescribes the local hydrology. CEH14’s ridge-recharge subsidy is the canonical missing term, taken up in S.16.
...
- `03_01_mechanistic_signatures.png` (cluster β bar chart) — a diagnostic figure used in the summary documents; not a numbered figure in the main report (the report’s Figure 8 is the cluster hydrographs).
- §4.2 — cluster characterisation, drawing on the LCSC values and the cluster-mean signatures.
- §4.4 / Figure 15a — per-well β₁ map (Script 07 produces; values come from `03_master_data.csv`).
...
### Sub-script 04 — Cluster orientation map (`04_cluster_visualisations.py`)
...
| File | Contents | Downstream consumer |
|---|---|---|
...
### Sub-script 05 — Pearson affinity (reference network, `05_pearson_affinity.py`)
...
### Sub-script 06 — Pearson affinity (sitewide / extended, `06_pearson_extended.py`)
...
| `06_pearson_extended/06_pear_02_integration_map.png` | Sitewide integration map | Report §4.3 (Figure 12) |
...
### Site-specific choices and rationale (chapter-level)
...
- §4.3 / Figure 12 — `06_pear_02_integration_map.png` (sitewide integration map).
...
| Constant | Resolved file | Used by |
|---|---|---|
...
The script replaces a former `07_boundary_intercept.py` that ran a Model A vs Model B intercept audit. Under the displacement formulation (F.3), the headline SSM fits well across all clusters (Script 08 median iterative NSE = 0.75), so the intercept audit added little beyond what direct coefficient mapping reveals. The current Script 07 is a pure visualisation: no refit, no statistical test, no Model B fit. Per-well coefficients are inherited unchanged from `03_master_data.csv`.
Methodology. Per metric (β₁, β₂, β₃ as percentage, R²), the script:
- Loads `INT_MASTER_DATA` and merges DEM ground elevations from `INT_WELL_ELEVATIONS` on the normalised well name.
- Calls `map_utils.add_idw_surface()` to interpolate the per-well metric onto the project’s standard 50 m grid (`np.arange(240200, 243800, 50)` × `np.arange(362200, 365800, 50)`). The implementation routes through `scipy.interpolate.griddata` with `method='linear'` — a Delaunay-triangulation-based piecewise-linear interpolation. The grid matches the one used by Scripts 11b, 19, and 20.
- Renders the interpolated surface as a semi-transparent pcolormesh layer (`alpha=0.65`) on top of a greyscale DEM hillshade (`map_utils.load_dem_hillshade`, vertical exaggeration ×3).
- Applies a ridge mask. Grid cells where the DEM raster elevation exceeds the interpolated well-elevation surface by more than 1.0 m are set to NaN, preventing the surface from extending across inter-dune ridges where no well measurements exist. In B&W output mode the mask is disabled in favour of an annotation that warns the reader to interpret ridge-top values with caution.
- Overlays Features and clearfell KML features (streams are omitted for these maps), then plots the well markers themselves at each well’s (E, N) — each marker uses the cluster’s colour (`config.CLUSTER_COLOURS`) and cluster shape (`config.CLUSTER_MARKERS`) at a uniform size. The markers indicate cluster membership; the metric value at each well is read off the underlying interpolated surface, not the marker.
- Adds metric-value contours for β₁ (every 0.5 from 2.0 to 6.5), β₂ (every 0.5 from 0.5 to 3.5), β₃ (at 0.5, 1.0, 2.0, 5.0, 10.0 %), and R² (every 0.10 from 0.50 to 0.90).
...
- CEH14 produces a negative β₃ at the canonical 3.7 m datum (β₃ = −0.016, R² = 0.58; see `07_coeff_maps_data.csv`). It is the only well in the network where the soft physical-sign warning fires under the canonical datum. The well sits at the bedrock ridge crest and is dominated by lateral subsidy that the SSM does not represent; the negative β₃ is the spatial map’s signal that something physical is missing here rather than that the well is malfunctioning. The phenomenon is taken up in S.16.
Where the result appears in the report.
- §4.4 Spatial coefficient distribution — the four maps as Figures 15a–d.
...
### Sub-script 08 — LCSC model benchmarking
...
- The script aligns the well series with monthly climate via `build_ssm_frame()` (F.3), with `window=100` so only the most recent 100 aligned months are kept.
- The SSM design matrix is built directly (`[+P, −PET, −h_disp_prev]` with no intercept) and fit by OLS. The TLM design matrix is `sm.add_constant([P, −PET])` and fit by OLS. Both fits use `HEADLINE_LAG = 0` from config — no rainfall lag.
- One-step simulation uses the observed previous-month water level as the recurrence anchor: at each month t, the model predicts h(t) from h_obs(t−1) plus its own Δh prediction. This metric measures diagnostic fit (how well the model recovers month-to-month changes given the truth at the previous step).
- Iterative simulation is the autonomous forecast: starting from h_obs at the window’s first month, the model predicts h forward step-by-step, with each month’s prediction feeding into the next as `h_prev`. The SSM iterative simulation uses `model_utils.simulate_ssm()`. The TLM iterative is simpler — without a feedback term, it just accumulates Δh from climate forcing alone. This metric measures forecasting stability, which is the harder test.
- For each well and metric, the script computes one-step R², iterative R², iterative NSE, and iterative RMSE for both models, plus per-well Δ = (SSM − TLM) for each metric.
...
Site-specific choices and rationale.
- 100-month window rather than the full record. The choice keeps both fits on a recent, well-sampled segment of the record and avoids letting older data (with sparser sampling, pre-clearfell baselines, and possible measurement-protocol drift) dominate. The number matches `LCSC_DATA_LIMIT = 100` in `model_utils.py`, which is the same window the per-well intercept-audit historically used.
...
- Lake sentinel excluded from the max-improvement reporting. The Table 3 “Max NSE improvement” row excludes the lake-side `LlynRhos` sentinel well. The lake sentinel has a tightly bounded surface-water signal that makes its TLM-vs-SSM comparison atypical of warren hydrology.
...
| `08_model_benchmarking/08_lcsc_04_table3_benchmark_summary.csv` | Six-row summary table for the main report (Table 3) |

The live Table 3 summary records: median one-step R² of 0.91 (TLM) vs 0.92 (SSM); median iterative R² of 0.66 vs 0.80; median iterative NSE of 0.13 vs 0.75. The one-step gap is small — both models recover diagnostic fit. The iterative-NSE gap is large — the SSM contributes most of its value in forecasting stability, not in one-step diagnostic fit. Forty-three of 66 reference wells have iterative NSE > 0 under the TLM; 65 of 66 do under the SSM. The single well where SSM iterative NSE is negative is CEH14 — the same ridge-crest well that produces the negative β₃ in Script 07’s map. The two diagnostics agree on which well the canonical SSM does not describe.
Limitations and known caveats.
- The TLM is the simplest unphysical alternative, not a comprehensive comparator. Wells where the SSM iteratively underperforms the TLM (CEH13 by Δ_NSE = −0.11, CEH14 by Δ_NSE = −3.64) sit at the C4 Main Forest’s bedrock-ridge interface. The shortfall there is not that the TLM is better; both models perform poorly. The TLM happens to drift less catastrophically because its intercept absorbs some of the systematic ridge-recharge subsidy that the SSM treats as residual. Neither model represents the physical mechanism (lateral inflow from the bedrock ridge); diagnosing it requires the supplementary analyses in S.16.
...
Where the result appears in the report.
- §4.4 Model benchmarking — Table 3 of the main report draws directly from `08_lcsc_04_table3_benchmark_summary.csv`.
...
- CEH14 is the consistent outlier across both scripts. Script 07’s β₃ map flags it as the single negative-β₃ well at the canonical datum; Script 08’s iterative NSE flags it as the worst-fitting well in the network (Δ_NSE = −3.64). Both signals point to the same physical mechanism — bedrock-ridge subsidy that the displacement-formulation SSM does not represent. The investigation of that subsidy is in S.16 (ridge recharge lag hypothesis test).
...
## S.6 Script 09 suite (a–e) — Scraping intervention
...
- The paired CEH36 / CEH4 shift is the principal headline. The +0.130 m Pure-Scraping shift is the figure quoted throughout §4.5. The slightly larger net benefit against CEH21 (+0.144 m) is reported alongside for context but is not the primary number.
Outputs.
...
### Sub-script 09b — Scraping propagation
...
From monthly flux to a summer-minimum change. The cross-cluster scenario figure (`09b_05`) reports each scenario as a change in summer-minimum depth rather than as a monthly water-equivalent flux, because the summer minimum is the ecologically decisive quantity, tied to the Curreli et al. (2013) SD15b/SD16 thresholds. The conversion from the flux bars produced by `scraping_common.compute_scenario_bars()` (F.5; the same Option-3 engine used throughout the scenario work) proceeds in three steps for each cluster: the monthly flux (mm water-equivalent per month) is divided by the cluster’s specific yield to give a head displacement, and that displacement is multiplied by a per-cluster summer amplification factor to give the change in summer-minimum depth, rounded to the nearest millimetre.
The specific yield is the WTF `Sy_event_median` from Script 17 (`17_wtf_01_sy_estimates.csv`, S.12), using the interception-corrected variant for the forested clusters C4 and C5 and the base variant elsewhere; clusters without a usable Sy row fall back to 0.20. As discussed in S.12 and Appendix B, the WTF specific yields are reliable for ranking clusters but high in absolute terms, so the absolute summer-minimum magnitudes derived here should be read as order-of-magnitude scenario comparisons rather than calibrated predictions.
The summer amplification factor is the ordinary-least-squares slope of the annual summer-minimum head on the annual-mean head, regressed across the years 2006–2025 on each cluster-centroid hydrograph (`03_regional_averages.csv`). A year contributes an annual mean only if it carries at least eight monthly values and a summer minimum only if it carries at least two measured Jun–Sep months. The factor expresses, empirically and at the climatological cross-year scale, how far a unit change in a cluster’s annual-mean water table is amplified (or damped) at the summer trough; clusters with fewer than eight paired years take a neutral fallback of 0.85. It is a steady-state amplification, not a within-year dynamical propagation: the timing of the summer minimum within a year is not resolved, consistent with the linear-domain assumption stated for the scenario viewer in S.13.
Only the SSM-equilibrium scenarios (forest management and climate) pass through this conversion. The scraping bar on the same figure is an empirical BACI shift measured directly at the scraped site (CEH36 versus CEH18), not an SSM-equilibrium quantity; it is placed on the same summer-minimum axis for comparison but is derived independently and labelled as such in the figure.
Implementation. The conversion is a single source of truth in `utils/scraping_common.py` (v1.4.0): `summer_amplification_factors()`, `scenario_cluster_sy()`, `flux_to_summer_min_mm()` and `scenario_summer_min_bars()`. Both Script 09b (`09b_05`, all scenarios including scraping and climate) and Script 21 (`21_forestry_06`, the forest-management scenarios only) call these helpers, so the per-cluster forestry summer-minimum values are byte-identical between the two outputs by construction.
Outputs.
...
| `09b_05_summer_scenario_comparison.{csv,png}` | Summer-minimum cross-cluster comparison |
| `09b_report_numbers.csv` | Citable values for §4.5 |


### Sub-script 09c — Summer minima
...
Site-specific choices.
- Dual-control design. The paired control is the most direct comparator (same hydrogeological setting, same coastal exposure) but is itself affected by coastal retreat. The climate-only control is unaffected by coastal retreat but is hydrogeologically more distant. Showing both lets the reader see how much of the apparent benefit at the paired-control level is amplified by the control’s own deepening.
- Year boundary at 2015 rather than monthly boundary. Summer minima are annual quantities. The April 2015 scraping date falls within the 2015 hydrological year; including 2015 in the post-scraping group rather than the pre-scraping group is the protocol (`POST_YEAR = SCRAPING_DATE.year`).
Outputs.
...
| `09c_report_numbers.csv` | Citable values for §4.5 |


### Sub-script 09d — Scenario comparison

Motivation. Was scraping a good management choice for this site? Using CEH36’s own SSM coefficients, its WTF-derived Sy, and its mean head displacement, the script computes the equilibrium summer response to alternative interventions — clearfell, thinning, broadleaf conversion, UKCP18 dry and wet climate scenarios — and compares each to the observed scraping benefit. The comparison is a like-for-like, single-site equivalent.
...
The live monthly scenario bars at CEH36 give: scraping (observed) +46.4 mm/month, clearfell (hypothetical) +12.2, thinning 50 % (hypothetical) +6.1, broadleaf (hypothetical) +3.0, climate-dry −14.7, climate-wet +7.9. The forestry scenarios are hypothetical in the sense that CEH36 is in C3 (not forested under current land use); the question they answer is “what would each intervention have produced at CEH36’s hydrogeological setting if CEH36 were forested?” — a like-for-like comparison framework, not a prediction.
Site-specific choices.
...
| File | Contents |
|---|---|
| `09d_01_scenario_comparison.{csv,jpg}` | Monthly equilibrium Δh per scenario at CEH36 |
| `09d_02_summer_scenario_comparison.{csv,png}` | Summer-minimum scenario table and bar chart |


### Sub-script 09e — Robustness

Motivation. Three independent estimates of the CEH36 Pure-Scraping step change confirm that the headline +0.13 m benefit is not an artefact of any single method’s assumptions — specifically not an artefact of CEH4’s own progressive deepening, which is the most plausible single-source contamination of the paired BACI estimate.
Methodology. Three estimators on the same CEH36 series:
- Raw paired BACI. CEH36 minus CEH4, averaged over the baseline era versus the Pure Scraping era. The simplest possible estimator.
- Synthetic control. CEH36 minus a weighted composite of donor wells from `DONOR_CANDIDATES` (eleven candidate wells outside the scraping influence — CEH1, CEH2, CEH5, CEH6, CEH9, CEH11, CEH16, CEH17, CEH19, CEH22, CEH24). Donor weights are fitted by OLS on the pre-scraping baseline, then the same weighted composite is computed across the full record. The step is the post-scraping minus pre-scraping mean of the (CEH36 − synthetic) gap series.
- SSM forward residual. The SSM is calibrated on CEH36’s pre-2015 baseline via OLS on `Δh = β₁·P − β₂·PET − β₃·(D + h_prev)` (with HEADLINE_LAG = 0), then iterated forward from the last baseline observation as an autonomous forecast: at each month, the predicted Δh is added to the previous predicted level. The residual (observed minus predicted) over the Pure Scraping era is the step change.
The live estimates: raw BACI +0.130 m, synthetic control +0.137 m (with 11 donors), SSM forward residual +0.081 m. The first two are tightly convergent (within 7 mm); the SSM forward residual gives a noticeably smaller estimate, partly because the model’s drainage feedback term iteratively brings the predicted level back toward equilibrium as the post-2015 elevation rises, absorbing some of the observed step into the forecast trajectory. The methodological point is that all three are positive and substantial: the +0.13 m benefit is not an artefact of CEH4’s deepening, which only affects the raw BACI estimate.
Outputs.
...
### Limitations and known caveats (suite-level)
...
- The SSM forward-residual estimator in 09e gives a smaller step than raw and synthetic estimators. All three are positive and substantial, but the SSM forward residual at +0.081 m is materially below the raw BACI (+0.130) and synthetic control (+0.137) estimates. The most plausible source of the divergence is the drainage-feedback term iteratively absorbing the elevated post-2015 water table back toward equilibrium across the forecast horizon; the divergence is a property of the estimator rather than evidence the step is smaller than reported.

### Outputs (consolidated)


| File | Sub-script | Contents |
|---|---|---|
...
| `09b_05_summer_scenario_comparison.{csv,png}` | 09b | Summer-minimum cross-cluster comparison |
...
| `09c_report_numbers.csv` | 09c | Citable §4.5 values |
| `09d_01_scenario_comparison.{csv,jpg}` | 09d | Monthly equilibrium scenarios at CEH36 |
| `09d_02_summer_scenario_comparison.{csv,png}` | 09d | Summer-minimum scenarios at CEH36 |
...
- Figures 15–18, 20–22, 24, 25 — multiple figures from the suite.
...
The suite asks eleven questions across its eleven primary sub-scripts. How can the pre-fell window be extended back for impact-tier wells whose own record starts after the canonical pre-fell start date — specifically CEH34, whose Aug-2010 first observation truncates the Forest-control centroid (10i)? Did the clearfell raise the mean monthly water table inside and adjacent to the compartment, after controlling for climate forcing and the residual influence of the earlier scraping (10a)? Where in the network is the step-change spatially concentrated, and where is it absorbed by background drift (10b)? Did the clearfell improve the ecologically critical summer-minimum water table at impact and edge wells (10d)? Which SSM pathway — recharge sensitivity β₁, atmospheric draw β₂, or drainage feedback β₃ — shifted at the felled and edge wells after the intervention, as a mechanistic-direction diagnostic alongside the statistical step estimate (10e)? Does the headline ANCOVA result survive being recomputed by independent estimators — per-well SSM forward residual, and zone-level synthetic control (10f)? What do the diagnostic checks — NW10 broadleaf succession, a radial transect from the compartment, rolling-window SSM coefficients — say about the trajectory of the recovery (10g)? Does the result strengthen, weaken, or hold when the synthesised FE-well records are added back into the impact centroid (10h)? Does the result survive a direct Impact-vs-Edge contrast that uses only the closest spatial control and identifies the felling step without the easting × time covariate that the headline ANCOVA relies on (10j)? And does a single four-zone panel model — fitting the Impact, Edge and a shielded second-control western-dune zone against the Forest control in one internally-consistent regression — agree on the felling step at monthly resolution (10k) and at the summer minimum (10l)? The twelfth sub-script, 10c, sits outside this question chain as a supplementary spatial diagnostic of the C4–C5 forest partition.
The headline ANCOVA-BACI result is reported in the main report as Table 7 and Figures 26–28 (§4.6 Clearfell intervention); spatial maps (Figures 23, 31), summer minima (Figure 29), coefficient shifts (Figure 32), the radial transect (Figure 33), and the supplementary forest-zone analysis (Table 16) draw on the other sub-scripts. The compositional mapping is recorded in the live `PIPELINE_README.md` table.
...
| `10a_*` outputs | Script 10a | 10f, 10h (ANCOVA comparison) |
...
### Sub-script 10a — Three-counterfactual ANCOVA-BACI
...
- Computes the BACI displacement timeseries: target-zone monthly mean minus control-zone monthly mean.
- Adds a centred climate-water-balance (CWB) covariate, computed by `compute_cwb()` as the demeaned cumulative `(P − PET)` anomaly in mm.
- Adds a distance-weighted scraping covariate as the difference of target and control centroid weights, with weight `exp(−d / λ)` and λ = 300 m by default. The differential form is essential because the BACI itself is target-minus-control.
- Adds the December 2017 felling dummy, the CWB × felling interaction, and where geometrically valid, an easting × time interaction that absorbs the coastal-retreat gradient.
- Adds a sensitivity to the October 2023 re-scraping (Model 3 includes the second scraping pulse; the difference in AIC from the baseline Model 2 records whether the second pulse measurably improves the fit).
...
The clearfell-era corrected map is the canonical Figure 31 in the main report; the scraping-era equivalent is Figure 23. The per-well step CSV is also consumed by Script 09b (scraping propagation) for the network-wide propagation modelling.

### Sub-script 10d — Summer minima (dual-control)
...
### Sub-script 10e — SSM coefficient shift diagnostic
...
### Sub-script 10g — Diagnostics

Three short diagnostics that justify network choices made elsewhere and characterise the trajectory of the recovery.
...
### Sub-script 10h — Synthetic FE-well extension BACI
...
| Variant | Control | Step (mm) | 95 % CI | p |
|---|---|---|---|---|
...
### Sub-script 10i — CEH34 donor-regression hindcast
...
### Sub-script 10j — Direct Impact-vs-Edge contrast
...
| 10b | `10b_spatial_*_raw.png`, `10b_spatial_*_corrected.png` | Four spatial step-change maps |
...
| 10d | `10d_04_summer_minima_forest_ctrl.png`, `10d_05_*climate*.png` | 4-panel summer-minimum figures |
...
- Figure 23 — Scraping-era spatial step map (10b).
- Figures 26–28 — CWB vs BACI displacement, Forest-control BACI Impact and Edge tiers (10a).
- Figure 29 — Summer minima vs Forest control (10d).
- Figure 31 — Clearfell-era spatial step map (10b).
- Figure 32 — Before/after SSM coefficients across the 17-well network (10e).
- Figure 33 — Clearfell radial transect, step vs distance (10g).
...
# Phase 4 — Climate and Spatial Context


## S.8 Scripts 00, 12, 13, 14 — Climate baseline and site-overview figures

Steps 13–16 / 27 (Phase 4 in `run_analysis.py`). Phase 4 — Climate and Spatial Context.

### Motivation

The four scripts in this chapter produce the report’s foundational orientation material: the climate baseline that §2 leans on for site setting, the DEM map that opens §2 as Figure 1, the experimental-design diagram that opens §3 as the BACI network figure, and the observed-trajectory figures in §5 that frame the climate-change discussion. None of these scripts produces a CSV that feeds another analytical chain — they are visualisation and baseline-context outputs. The chapter accordingly compresses the methodological detail and concentrates on what each script chooses to render and why.
...
| `data/RAF_Valley_Climate.csv` | 00 (Figure 3 only) | Raw climate input |
...
### Sub-script 12 — Site overview map
...
### Sub-script 13 — Experimental design map
...
### Sub-script 14 — Climate projections
...
### Site-specific choices (suite-level)
...
All paths resolve through `utils/paths.py` (`OUT_00_*`, `OUT_12_DEM_OVERVIEW`, `OUT_13_EXPERIMENTAL_MAP`, `OUT_14_*`).
...
### S.8.5 Script 14b — Bootstrap year-of-crossing (post-review addition, Phase 4 step 16)
...
Cross-references.
- §7 Conclusion 11 of the main report — the “around 2030–2032” qualitative date band is replaced by the bootstrap CI from this script (median 2022, 95% CI 2017–2040 for C1 SD16).
...
## S.9 Scripts 11, 11b — Forecasting and spatial thresholds
...
### Sub-script 11 — Cluster-level temporal forecasting
...
where P̄(t) and PĒT(t) are the long-term monthly climatologies indexed by calendar month, λ is the rainfall multiplier being solved for, and D = `DRAINAGE_DATUM` = 3.7 m. Iterating from a summer-minimum head h₀ over n months gives
> hₙ = h₀ · αⁿ + β₁ · λ · S_P − β₂ · S_E − D · (1 − αⁿ)
...
> λ = ( h_target − h₀ · αⁿ + β₂ · S_E + D · (1 − αⁿ) ) / ( β₁ · S_P ), P_flood = λ · Σ P̄ᵢ
...
#### Site-specific choices (Script 11)
...
### Sub-script 11b — Per-well spatial threshold maps
...
#### Site-specific choices (Script 11b)
...
- Recovery limits derived from the BACI scraping benefit. `SD15b_REC = 0.75 m` and `SD16_REC = 1.20 m` encode the +0.143 m demonstrated benefit at CEH36 (S.6) as a per-well recovery target.
...
### Site-specific choices and limitations (suite-level)
...
- S.6 — Script 09a’s +0.143 m BACI scraping benefit at CEH36 is the basis for 11b’s recovery-limit values (`SD15b_REC = SD15b + 0.14 m`, `SD16_REC = SD16 + 0.22 m`).
...
### S.9.3 Script 11c — P_flood achievability map (post-review addition, Phase 3 step 13)
...
Headline result (2026-05-29). The categorisation produces a clean operational separation between the open-dune and forest zones. Of the 65 wells in C1 + C2 + C3 (the open-dune clusters), 57 are achievable, 8 are marginal, and none are unreachable. Of the 23 wells in C4 + C5 (the forest clusters), only 2 are achievable, 17 are marginal, and 4 are unreachable. The four unreachable wells are concentrated in C5 Coastal Forest (three wells) and C4 Main Forest (one well). The cluster pattern reflects the underlying mechanism: open-dune clusters carry higher β₁ recharge efficiency (2.5–4.6) and lower summer-minimum baselines; forest clusters carry canopy interception losses and lower β₁ (1.32–2.55), and C5 additionally carries the Section 4.8 coastal-retreat gradient pushing its summer-minimum baseline progressively further below the Curreli thresholds.
Inputs.
...
Outputs. Sharing `paths.DIR_11B` with Script 11b — outputs sit alongside 11b’s per-well CSV with `11c_` prefix:
...
## S.10 Script 15 — Depth-dependent PET sensitivity
...
| Filename | Origin |
|---|---|
| `01_wells_clean.csv` | Script 01 — cleaned, upstand-corrected monthly dipwell series |
...
- §5.3 Discussion / PET formulation — the Thornthwaite limitations discussion cites Script 15’s depth-coupled λ values as evidence that the fixed-β₂ formulation is defensible across the network. The graded improvements (largest at C1, smallest at C3 and C5) appear in the limitations paragraph.
...
## S.11 Script 16 — Water balance decomposition
...
#### Recession as an independent check
...
| File (in `outputs/16_water_balance/`) | Contents | Downstream consumer |
|---|---|---|
| `16_water_bal_table.csv` | Table 3a: head-space decomposition per cluster — β triplet, P̄, PET̄, h̄_disp, recharge, ET draw, drainage, residual, drainage and ET percentages | Report §4.2.3, Table 3a |
| `16_water_bal_vol_table.csv` | Table 3b: volumetric partition per cluster — P, PET, I, P_net, both partition methods (SSM, recession), midpoint and bracketing range in mm/yr | Report §4.2.3, Table 3b |
| `16_water_bal_bar_ms.png` | Water-balance manuscript version (300 dpi, white background, two-panel) | Report Figure 9 |
| `16_water_bal_bar_lay.png` | Figure 8 lay version (coloured background, 150 dpi) | Report companion / web version |
...
- §4.2.3 Water balance — the section is built around the two tables and Figure 8 from this script.
- Table 3a — head-space decomposition (`16_water_bal_table.csv`).
- Table 3b — volumetric partition with SSM/recession bracket (`16_water_bal_vol_table.csv`).
- Figure 8 — two-panel water balance figure (`16_water_bal_bar_ms.png`).
...
## S.12 Scripts 17, 18 — WTF Specific Yield (cluster and spatial)
...
The Water Table Fluctuation (WTF) method (Healy and Cook, 2002) translates an observed water-table rise into a recharge-equivalent volume, with specific yield (Sy) as the proportionality constant: Sy = R / Δh. The SSM in chapter S.3 characterises the dynamic response of the aquifer (the β coefficients), but its coefficients are in head units; converting any head-equivalent quantity into a volumetric flux requires Sy. The WTF Sy is also the only site-specific value of Sy available — slug or pumping tests have not been run at Newborough Warren, and a representative Sy cannot be read off the literature for a coastal dune system with this particular substrate gradient. Scripts 17 and 18 are paired because they use the same WTF event-detection logic on the same monthly time series, but answer different questions: Script 17 produces a single Sy per cluster (the value consumed by Table 3c and by Script 19’s spatial calculations and Script 21’s forestry scenarios), while Script 18 produces a Sy per well and combines it with the well-level β₃ from the SSM to generate the storage–drainage index map τ = Sy / β₃ that informs the main report’s discussion of aquifer architecture.

### Sub-script 17 — `17_wtf_specific_yield.py` (cluster-level Sy)
...
Script 18 brings the WTF estimator down to the individual well, then interpolates the result spatially. Three downstream uses motivate the per-well treatment. First, the supplementary table S1 in the main report needs a per-well Sy so readers can cross-check the cluster-level headline against the spatial pattern. Second, the spatial contour map shows where the Sy assumption is most exposed (low-Sy pockets that the cluster-level mean smooths over). Third, combining per-well Sy with per-well β₃ from `03_master_data.csv` gives the storage–drainage index τ = Sy / β₃ (months), a per-well aquifer-architecture diagnostic that is not available from the SSM alone. τ is a deliberately storage-weighted composite — it fuses two independently estimated quantities (Sy from the WTF method, β₃ from the SSM) and therefore carries information beyond β₃ alone. It is not a residence time or drainage timescale in the hydraulic sense: the head-space recession e-folding time is t_R = 1/β₃ (months), which is the genuinely time-like quantity (the interval over which a recharge perturbation decays). Because Sy < 1, τ = Sy/β₃ is always shorter than 1/β₃ — for example, C2 has 1/β₃ ≈ 14 months but τ ≈ 4 months. The two should not be conflated; t_R = 1/β₃ is reported as an interpretation layer on the β₃ atlas (see Site-specific choices below).
...
Storage–drainage index τ = Sy / β₃. The function `compute_drainage_timescale()` joins the per-well Sy table with `03_master_data.csv` on a normalised well name and computes τ for each well where β₃ > 0. Wells with negative β₃ (CEH14) or near-zero β₃ that would produce a > 10× outlier (CEH13) are flagged as excluded; the ridge / slack-floor wells (CEH12 sand–bedrock contact, CEH15 forest slack floor) are also excluded as not representative of the dune-aquifer Sy that the WTF estimates. The remaining wells produce τ values in months that are then interpolated to a contour map (`18_wtf_05_drainage_timescale_map.png`) and reported in `18_wtf_05_drainage_timescale.csv`.
Head-space recession time t_R = 1/β₃ (interpretation layer). The β₃ atlas produced in chapter S.5 (Script 07) is already a drainage-rate map; t_R = 1/β₃ is a deterministic monotone transform of β₃ that re-expresses each well’s drainage rate as an e-folding timescale in months — the interval over which a recharge perturbation decays to 1/e of its initial magnitude in the absence of further forcing. Because 1/β₃ is a one-to-one rescaling, the spatial pattern of t_R is identical to that of β₃ and no new figure is required; Script 18 does not compute t_R as a separate column. The interpretation anchor is: β₃ ≈ 0.10 month⁻¹ → t_R ≈ 10 months; β₃ ≈ 0.07 → t_R ≈ 14 months; β₃ ≈ 0.05 → t_R ≈ 20 months; β₃ ≈ 0.02 → t_R ≈ 50 months. Cluster-mean values from the canonical fits: C1 Lake Edge ≈ 10 months, C2 Dune ≈ 14 months, C3 Western Residual ≈ 20 months, C4 Main Forest ≈ 57 months (≈ 4.8 years), C5 Coastal Forest ≈ 22 months. CEH14 has negative β₃ and has no valid t_R; CEH13’s β₃ ≈ 0.002 gives t_R ≈ 526 months and is treated as off-scale in any tabulation. t_R adds no spatial information beyond the β₃ atlas; its value is the interpretable “years of drainage memory” framing that the raw coefficient does not immediately communicate. The genuinely independent fusion of storage and drainage information remains τ = Sy/β₃ (Workstream A).
Aquifer-diagnostic synthesis figure. `plot_aquifer_diagnostic_synthesis()` combines three independently derived per-well metrics into a single scatter: τ (the storage–drainage index Sy/β₃, from Sy and β₃ here), the iterative LCSC NSE improvement ΔNSE (from chapter S.5’s model-benchmarking output `08_lcsc_model_stats.csv`), and Sy itself as the marker size. Points are coloured by cluster; cluster means are anchored by larger star markers. The synthesis is the figure that turns three diagnostics into a single read of aquifer architecture. Note that τ here is the Sy-weighted composite, not the head-space recession time t_R = 1/β₃; both are discussed in Methodology above.

#### Site-specific choices

- The storage–drainage index τ is the per-well diagnostic, not the cluster mean. Cluster-mean τ averages over substantial within-cluster spread (the C2 Dune τ range is 2.0–8.7 months across 24 wells, mean ≈ 4.0 months). The map preserves the per-well variation. Note that τ = Sy/β₃ is not a residence time — see the t_R = 1/β₃ passage above for the head-space recession time and its cluster-mean anchor values.
- Ridge-zone exclusions for τ. CEH12 sits on the sand–bedrock contact where Sy is not representative of the dune aquifer; CEH15 sits on a forest slack floor where the Sy estimate is atypical of the surrounding cluster. CEH13’s β₃ is near zero (0.0019, which would give τ ≈ 124 months — physically defensible as “very slow” but compresses the contour colour bar and dominates the map). CEH14’s β₃ is negative, making τ undefined. All four are documented in the exclusion column of the τ CSV. The same exclusions apply to t_R = 1/β₃: CEH14 has no valid t_R; CEH13 is off-scale (t_R ≈ 526 months).
...
| `18_wtf_05_drainage_timescale_map.png` | τ = Sy/β₃ storage–drainage index contour map | Main report |
| `18_wtf_05_drainage_timescale.csv` | Per-well τ values with Sy, β₃, cluster, exclusion flags; τ is the storage–drainage index (not a residence time — see t_R = 1/β₃ in Site-specific choices) | Supplementary |
| `18_wtf_06_aquifer_diagnostic_synthesis.png` | τ vs ΔNSE scatter, sized by Sy, by cluster | Main report, §5 discussion |
...
- The contour interpolation does not extrapolate beyond the convex hull of well locations. Sy values at the site margins reflect the nearest interior wells, weighted by inverse-square distance, rather than any genuine measurement at those margins. The site-boundary mask hides this where it would mislead (offshore, in the lake) but the user should be aware that the dune fringes are interpolated, not measured.
- Downstream consumer note — broadleaf β₂ multiplier in Script 19. The Sy values produced here feed Script 19’s spatial calculations and Script 21’s forestry scenarios. Script 19 imports the broadleaf β₂ seasonal split — `sB2_w = 0.8817` (Nov–Apr, leaves off), `sB2_s = 1.0750` (May–Oct, full leaf) — from `utils.config` (canonical values derived from Script 21’s monthly profile, see chapter S.14). §4.9.6 of the main report carries the broadleaf-scenario values.

### Where the result appears in the report

- §4.2.4 (or the equivalent — verify against the report’s section numbering) — WTF Sy methodology and the cluster-level result.
- Table 3c — cluster-level Sy estimates from Script 17’s interception-corrected output.
- Table 3d / Figure 8b — volumetric water-balance using the WTF Sy.
- Table S1 — per-well Sy from Script 18, supplementary.
- Spatial figures — Script 18’s contour and τ maps; the τ vs ΔNSE synthesis figure appears in the discussion of aquifer architecture.
...
- S.5 — model benchmarking; supplies the ΔNSE used in the τ vs ΔNSE synthesis figure.
...
## S.13 Scripts 19, 20 — Spatial groundwater analysis
...
Scripts 19 and 20 together construct §4.9 Spatial groundwater of the main report. They share inputs (the per-well β coefficients from `03_master_data.csv`, monthly maOD heads from `01_wells_clean_maod.csv`, the 50 m project grid, the F.3 displacement formulation, the cluster-mean Sy from S.12) and methodological choices (Freeman 2008 forest interception, Betson 2002 hydraulic conductivity, the F.2 bucketing convention). They differ in deliverable: Script 19 generates `scenario_viewer.html`, a self-contained interactive HTML calculator; Script 20 generates the publication-quality static figures for §4.9. Script 19 at ~1800 lines is among the longest in the pipeline; Script 20 at ~1200 lines produces the publication spatial figures (ten outputs). The interactive viewer is part of the methods rather than a results artefact: it is the calculator that lets a reader exercise the per-cluster equation engine for any scenario combination, with sliders and parameter values exposed.

### Sub-script 19 — Scenario viewer (interactive HTML)
...
| Preset | P scaling | PET scaling | Forest interception | Forest β₂ scaling |
|---|---|---|---|---|
...
Outputs. A single self-contained HTML file (`scenario_viewer.html`, typical size 250–300 KB including the embedded base64 hillshade) and a tidy summary CSV (`19_scenario_summary.csv`, one row per scenario × season × spatial unit, where spatial unit is each of C1–C5 plus a site-wide mean). The CSV mirrors the JavaScript calculation through the shared `_well_dh()` engine. To anchor with one scenario: for UKCP18 2080s annual the site-wide mean Δh is −33 mm, with the largest impacts in C4 Main Forest (−46 mm) and C3 Western Residual (−34 mm) and the smallest in C1 Lake Edge (−17 mm). Forest clusters show the steepest summer drawdown (C4 −118 mm, C5 −71 mm) and the most muted winter response (C4 +25 mm, C5 +26 mm), reflecting the high β₂ that dominates the forest water balance. Clearfell produces the inverse — C4 +38 mm annual, C5 +38 mm annual — with winter recovery exceeding summer recovery in both forest clusters (C4 winter +47 mm vs summer +29 mm; C5 winter +45 mm vs summer +31 mm). These values appear directly in §4.9.6 of the main report.
...
| SSM drainage coefficient (C3) | β₃ | ≈ 0.058 month⁻¹ (live; C3 centroid from `03_03_cluster_mechanistic_coefficients.csv`) | Live: Script 03, S.3 | Forest drawdown |
...
#### Dune-scrape topographic-drain drawdown (`20_scrape_drawdown_nohead.png`)

A fourth drawdown field was added at Script 20 v1.1.0–v1.10.0, treating a dune-scrape excavation as a topographic drain that recruits surrounding groundwater and lowers the water table around it. It is the sibling of the forest drawdown figure (`20_drawdown_propagation_nohead.png`): same DEM hillshade base, same site-boundary clip, same flow-weighted Dijkstra cost-distance with the C3-sourced leaky-aquifer length scale λ = √(K·b / (Sy·β₃)) ≈ 223 m, and the same shared discrete six-colour banded drawdown scale (see Shared drawdown scale, below). The differences are direction (the cost-distance weighting is flipped to favour up-gradient propagation, since a coastal drain draws from the warren rather than from the sea), source geometry (a 30 × 60 m rotated rectangle, long axis bearing 45° NE, centred on CEH36 at 241 161 E, 363 306 N — the documented 2015 scrape site, ground level ≈ 5.3 m AOD, ~450 m inland), and source magnitude (see below). For the inland CEH36 site `SCRAPE_TRUNCATE_SEAWARD = False`, so the cone draws all round; the seaward half-plane truncation (`SCRAPE_TRUNCATE_SEAWARD = True`) is retained as a flag for coastal-margin scrape scenarios but is not active in the headline figure. The scrape interior is masked: the slack itself rises during a scraping intervention while the surrounding water table draws down, so the figure shows only the surrounding drawdown cone (the response is a dipole; the rise inside the slack is the §S.7 scraping BACI step, not part of this propagation field).
The edge drawdown magnitude H₀ is the chapter’s distinguishing scientific choice. Earlier prototype versions of the figure (v1.1.0 through v1.7.0) carried H₀ as a flagged scenario assumption — first H₀ = 40 mm as a literal (v1.1.0–v1.5.0), then H₀ = Sy × scrape depth = 0.311 × 0.40 m ≈ 124 mm (v1.7.0, mechanistic but parametric). The v1.10.0 form replaces both with a directly-measured CEH36 response: H₀ is read live from `09_baci_shifts.csv` (Script 09a, S.7) as the Pure_Scraping × Treatment shift at CEH36 against the CEH4 reference well — currently ≈ 129 mm. Three consequences follow. First, the figure’s edge magnitude is now empirical, not parametric. Second, the scrape excavation depth is no longer an input but an inferred output: under H₀ = Sy × D, the back-calculated D = H₀ / Sy = 0.1295 / 0.311 ≈ 0.42 m, which lands close to the assumed 0.40 m of the v1.7.0 form. The convergence is corroborative — the one field measurement not directly taken (the scrape depth) recovers from an independent calibration anchored on a different observable (the CEH36 BACI shift). Third, attempting the same back-calculation against the clearfell-well drops (WMC3 −211 mm down to CEH20 −93 mm) gives implausibly deep scrape excavations of 1.5–2.1 m; this confirms that the clearfell-well drops are dominated by the coastal-retreat gradient drift, not the scrape — a 0.4 m scrape propagates only ~ 40 mm at WMC3, far short of the −211 mm observed, exactly as the §5.4.3 propagation framing of the BACI clearfell step predicts.
The figure is the methodological evidence underpinning §5.4.2’s “scrape as topographic drain” reading of the wet-slack BACI response, and contributes the scrape-zone half of the clearfell pre-fell baseline composite below.

#### Clearfell pre-fell baseline drawdown composite (`20_clearfell_baseline_drawdown.png`)

The clearfell BACI documented in §4.6 and §S.7 controls for a number of confounders through the easting × time covariate, the CWB main effect, and the three-tier control hierarchy. The clearfell-baseline composite figure makes the confounder-set visible as a single drawdown field: scrape + Storm Brendan retreat, both head losses, both rendered on the shared sequential drawdown scale. The composite displays what the clearfell BACI is correcting against. The scrape component comes from the field above (the topographic-drain drawdown around CEH36 with H₀ from the live Script 09a CEH36 shift); the Storm Brendan retreat component is the v1.7.0 framing of the coastal-erosion drawdown evaluated at COAST_RETREAT_M = 6 m (a single Brendan-class event), with δ₀ and L from the live Script 25 fit. The composite is `net = scrape + erosion` (both positive drawdowns; not a diverging-scale subtraction).
...
#### Public summary three-driver comparison panel (`20_public_drivers_panel.png`)
...
#### Shared drawdown scale across the four drivers
...
| Scrape edge drawdown magnitude | H₀ (scrape) | ≈ 129 mm (live; CEH36 Pure_Scraping shift from `09_baci_shifts.csv`) | Live: Script 09a, S.7 | Scrape, Clearfell-baseline, Public |
...
- Forest-drawdown propagation constants from C3. The forest-drawdown map’s diffusivity inputs are sourced from the live pipeline: Sy ≈ 0.31 from the C3 cluster median of `17_wtf_well_sy.csv` (Script 17, S.12), and β₃ from the C3 row of `03_03_cluster_mechanistic_coefficients.csv` (Script 03, S.3) — ≈ 0.058 month⁻¹ under the `limit=1` policy — giving λ ≈ 223 m. K = 6 m/day (Betson 2002) and saturated thickness b = 5 m remain literature/estimate literals. The Sy and β₃ loads are dynamic, so the figure tracks pipeline state.

### Limitations and known caveats

- Displayed flow vectors carry direction only. Both the head-surface map (`20_head_surface_streams.png`) and the residual map (`20_residual_ssm.png`) normalise (U, V) to unit length before plotting. The docstring’s “normalised Darcy quiver” is accurate as to direction; the Darcy K = 6 m/day does not enter the displayed vectors. A reader expecting quiver length to scale with flow magnitude will not see that. K = 6 m/day is used in the forest-drawdown propagation λ length scale, not in the static-figure quivers.
...
- The 50 m grid does not extrapolate. All interpolations are bounded by the well-network convex hull (plus the sea-boundary anchors). Map areas near the network margins are interpolated rather than measured, particularly along the northern bedrock ridge where well density drops to zero. The static figures and the viewer’s ridge mask flag these zones; map values at the network margins should not be read as observations.
...
| `19_spatial_groundwater/19_scenario_summary.csv` | Per-scenario × season × cluster Δh table (6 scenarios × 3 seasons × 6 spatial units = 108 rows) | §4.9.6 Forestry scenarios |
...
| `20_spatial_figures/20_scrape_drawdown_nohead.png` | Dune-scrape topographic-drain drawdown at CEH36; shared discrete drawdown scale; H₀ live from Script 09a CEH36 Pure_Scraping shift; sibling of the forest drawdown figure | §3.4.5 / §5.4.2 (Figure) |
| `20_spatial_figures/20_clearfell_baseline_drawdown.png` | Composite drawdown imposed on the clearfell pre-fell baseline = scrape + Storm Brendan retreat, both head losses on the shared sequential drawdown scale; paired-confounder construction underpinning the §5.4.2/§5.4.3 BACI framing | §3.4.5 / §5.4.2 / §5.4.3 (Figure) |
...
- §4.9.6 Forestry scenarios — values from `19_scenario_summary.csv` for the clearfell, broadleaf and thinning rows; the forest drawdown propagation map (`20_drawdown_propagation_nohead.png`) from Script 20.
- §4.9.7 UKCP18 climate scenarios — values from `19_scenario_summary.csv` for the 2050s and 2080s rows.
- §5.4.2 The scrape-as-drain reading of the wet-slack BACI — the dune-scrape topographic-drain drawdown figure (`20_scrape_drawdown_nohead.png`), with its H₀ anchored on the live Script 09a CEH36 Pure_Scraping shift, supplies the propagation framing for the §5.4.2 interpretation.
...
## S.14 Script 21 — Forestry scenarios
...
| Month | β₂ multiplier | Phenological state |
|---|---|---|
| Jan | 0.85 | Leaves off |
| Feb | 0.85 | Leaves off |
...
Summer-minimum companion (`21_forestry_06`). Alongside the volumetric panel, `plot_scenario_comparison()` also emits `21_forestry_06_summer_scenario.csv`: the per-cluster change in summer-minimum depth for the three forest-management scenarios (clearfell, thinning, broadleaf), computed through the shared `scraping_common` conversion documented in S.6 (flux → head ÷ Sy → summer minimum × amplification factor) and therefore byte-identical with the forestry rows of `09b_05`. This gives the §4.9.6 forestry narrative a forestry-module source for the ecological summer-minimum metric — the metric that maps onto the Curreli thresholds — rather than having to reach into the scraping suite for it; the climate-scenario summer comparison remains with `09b_05` to avoid a second, divergent source.
...
- C4 only for the synthetic hydrograph. The hydrograph figure shows C4 (Main Forest) only. C5 (Coastal Forest) is forested and shares the same β₂ multipliers in the scenario-comparison panel, but it has no BACI clearfell observation — the 2018 felling was the C4-side compartment, not C5. The hydrograph’s value lies in the BACI overlay, so plotting C5 alongside C4 without an empirical reference would mislead. C5 does appear in the scenario-comparison panel (Figure 5) where the modelled response is presented at face value without a BACI anchor.
...
| Filename (relative to `outputs/21_forestry_scenarios/`) | Contents | Downstream consumer |
|---|---|---|
| `21_forestry_01_hydrograph.png` | Synthetic C4 mean-year hydrograph: pine baseline, full clearfell, 50 % thinning, broadleaf conversion; BACI benchmark band; SD15b / SD16 reference lines; C1, C2 observed cycles as context | Main report §4.9.6 |
| `21_forestry_01_hydrograph.csv` | Companion data file for the hydrograph figure (Script 21 v1.2.0+). Block 1 — the 12-month depth-below-ground series for every plotted line (four management scenarios, observed C4 baseline, both BACI benchmarks, observed C1/C2 context) in tidy long format. Block 2 — a trough/separation summary: each series’ deepest month and depth, its separation from the observed C4 baseline anchored at the baseline trough month, and the largest Jun–Sep summer separation with the month it occurs | Main report §4.9.6 / §5.7.4 |
| `21_forestry_02_distributions.png` | Summer-minimum depth distributions per cluster, with C4 split pre/post-felling | Main report §4.9.6 |
...
| `21_forestry_03_scraping_eras.png` | Annual summer minima per scraped well (CEH36, CEH18, CEH21) and unscraped control (CEH4), four eras | Main report §4.5 / §4.9.6 cross-link |
| `21_forestry_03_scraping_era_means.csv` | Era-level summaries for the same wells | Reference table |
| `21_forestry_04_baci_zone_violin.png` | Summer-minimum violins by BACI tier (Impact, Edge, Forest Ctrl, Coastal Ctrl, Climate Ctrl) across three phases | Main report §4.6 cross-link |
| `21_forestry_04_baci_zone_means.csv` | Tier-level summaries | Reference table |
| `21_forestry_05_scenario_comparison.jpg` | Per-cluster volumetric Δ (mm w.e./month) for clearfell, thinning, broadleaf, UKCP18 Dry, UKCP18 Wet | Main report §4.9.6 |
| `21_forestry_05_scenario_comparison.csv` | Same data in tidy form | Reference table |
| `21_forestry_06_summer_scenario.csv` | Per-cluster Δ summer-minimum depth (mm) for clearfell, thinning, broadleaf — shared `scraping_common` conversion, byte-identical with `09b_05` forestry rows | Main report §4.9.6 |
...
- Broadleaf β₂ — canonical here, consolidated in config. The 12 monthly multipliers above are the canonical source. The seasonal means consumed by downstream code are 0.8817 (Nov–Apr) and 1.0750 (May–Oct), and these are what `config.BROADLEAF_B2_WINTER` and `config.BROADLEAF_B2_SUMMER` export. Script 19 imports the same names directly from config. §4.9.6 of the main report carries these values.
...
- §4.9.6 Forestry scenarios — Figures 21-01 (synthetic hydrograph) and 21-02 (distributions); the headline narrative on clearfell, thinning, and broadleaf at C4; the BACI-modelled gap.
- §4.9.6 — Figure 21-05 (scenario comparison panel) for the cross-cluster volumetric summary.
- §4.5 and §4.6 cross-links — Figures 21-03 (scraping-era) and 21-04 (BACI tier violins) where the forestry chapter draws on the scraping and BACI evidence for context.
...
- S.6 — Scraping intervention; complement to forestry as a direct-surface management lever; `09c_01_summer_minima.csv` is the shared minima source for Figure 21-03.
- S.7 — Clearfell BACI; origin of the β₂ multiplier and of the +0.120 m annual displacement that anchors Figure 21-01’s benchmark band, along with the directly-fitted +0.046 m summer band (Script 10a v1.3.0).
...
The eleven Phase 4 chapters (S.8–S.14) cover the climate-and-spatial scripts that feed §4 of the report. Phase 5 collects six further chapters whose methodological purpose is supplementary: each consumes pipeline intermediates, none feeds downstream pipeline scripts, and the conclusions they produce inform the report’s discussion rather than its core results chain. S.15 covers Script 25 (coastal-retreat gradient, step 26/35, Phase 11 in `run_analysis.py`). S.16 covers Scripts 22, 23, and 24 (residual diagnostics, steps 27–29/35, Phase 12 in `run_analysis.py`). S.18 covers Script 26 (van Willegen 5-year MSL aggregation, step 30/35, Phase 13). S.18b covers Script 11 Section 5 (Tool A — spring MSL transfer function, embedded in Script 11 at step 22/35, Phase 1) together with Script 26b (Tool B — UKCP18 RCP8.5 MSL5 projections, step 31/35, Phase 13). S.18c covers Script 26c (MSL5 report-format figures, step 32/35, Phase 13). S.19 covers Scripts 28 and 29 (cluster framework diagnostics added in the 2026-05-29 post-review pass, steps 33–34/35, Phase 14). S.17 is appendices. All eleven scripts in S.15, S.16, S.18, S.18b, S.18c, and S.19 are part of the pipeline orchestrated by `run_analysis.py`; the “post-pipeline supplementary” framing reflects their role in the report (discussion-feeding rather than results-feeding), not their orchestration status. Scripts 26c (MSL5 report-format figures, Phase 13) and 27 (greyscale figures, Phase 15) are display/utility steps excluded from the 33-step analytical count; Script 26c is documented in S.18c and Script 27 in Appendix A.
...
All six fits (3 specifications × 2 functional forms) are written to `25_01_panel_fit_parameters.csv`. Under the forest-free specification the linear-capped and exponential forms are near-indistinguishable on AIC (ΔAIC = −0.5, exp − lin-cap, i.e. a marginal exponential preference); the linear-capped form remains the headline on the strength of its Dupuit–Forchheimer strip-aquifer interpretation rather than a decisive AIC margin. The three linear-capped fits return δ₀ values in the range −26.1 to −29.2 mm yr⁻¹, L values in the range 894–972 m, and c values around −6.5 mm yr⁻¹. The closeness of the C3-only δ₀ (−26.1 mm yr⁻¹, SE 2.55) to the forest-free network δ₀ (−28.8 mm yr⁻¹, SE 1.92) is the chapter’s primary identification claim: the distance-decay signal is recoverable from a single non-forested cluster, ruling out forest cover as the principal driver.
The per-cluster attribution under the headline fit is in `25_03_cluster_partition.csv`. C5 (Coastal Forest, mean d̄ ≈ 420 m) carries by far the steepest observed slope (−37.7 mm yr⁻¹ at the Script-14 centroid); the gradient model attributes roughly 41 % of that to coastal retreat. C3 (Western Residual, d̄ ≈ 800 m) has a modest observed slope (−6.8 mm yr⁻¹), of which the gradient accounts for about 32 %. C1, C2, and C4 sit beyond or near the inland reach L; the gradient component is zero or near-zero, and their observed deepening (~−10 to −12 mm yr⁻¹) is left as climate plus an unexplained residual.
...
Steps 27, 28, 29 of 35 in the orchestrator (Phase 12 — Residual Diagnostics in `run_analysis.py`); inside the supplement’s 33-step analytical count; second chapter under Phase 5 — Post-pipeline supplementary analyses in the supplement.

### Motivation

Section §4.4.2 and §5.3 of the main report frame the spatial pattern of the SSM water-balance residual as ridge-derived recharge: wells along the northern bedrock ridge flank, with CEH14 as the canonical example, carry persistent positive residuals that the three-term SSM does not represent. The interpretation is a physically plausible reading of the spatial map (Figure 2a, see S.13) — wells nearest the ridge carry the largest unmodelled inputs, consistent with lateral subsidy from the rock outcrop. But spatial correlation with the ridge is not, on its own, mechanistic evidence that water is travelling from the ridge to the wells. A model that misspecifies the rainfall response, for example, could produce the same spatial pattern if the misspecification happens to be largest near the ridge.
Scripts 22, 23 and 24 are the analytical apparatus that subjects the ridge-recharge interpretation to mechanistic testing. They share a single source — the SSM residual series e(t) at each well — and ask three complementary questions about that series. Script 22 characterises the residual: how time-structured is it well-by-well, and do the wells with the largest mean residuals (the intercept α from Model B) also have the most autocorrelated residuals? Script 23 puts the residual to a physical test: if it carries genuine ridge-to-well travel time, the cross-correlation peak lag against rainfall should increase with distance from the ridge. Script 24 asks the alternative-hypothesis question: does the residual carry a seasonal signature consistent with the Thornthwaite PET estimate misrepresenting summer demand, or is the seasonal structure flat (consistent with steady ridge baseflow), or something else again? Together the three diagnostics constrain what the residual can and cannot be, and in doing so constrain what the ridge-recharge interpretation can safely claim.
...
### Methodology — Script 22 (residual lag analysis)

Script 22 refits Model B — the SSM with a constant intercept — on every well with at least 140 months of usable data, on the full record rather than the 100-month recent window used by Script 07. The model is the canonical displacement-formulation SSM with an additive constant:
...
The fitted offset (a₀), amplitude (√(a₁² + a₂²)), and phase (the calendar month at which the sinusoid peaks) summarise each well’s seasonal residual structure. The summer-minus-winter difference (mean of JJA months minus mean of DJF months) is reported alongside as a model-free check.
...
- Full record vs Script 07’s 100-month window. Scripts 22 and 24 use the full per-well record to maximise statistical power for the lag and seasonality analyses, which need as many monthly observations as possible. The trade-off is that the Model B fits here are not identical to Table 2 / Figure 15a in the main report. The supplement’s headline coefficients remain the 100-month-window fits; Scripts 22, 23, 24 are diagnostic companions, not revisions.
- 140-month minimum record (MIN_MONTHS = 140). Roughly 11.7 years of continuous data. Wells with fewer observations are dropped from all three scripts. This excludes shorter-record monitoring points without compromising the Bartlett confidence threshold calculation, which becomes unreliable below 100 monthly observations.
- Common exclusion set (`EXCLUDED_WELLS_NORM = {ceh3, ceh4, ceh7, ceh8, ceh37}`). CEH7, CEH8 and CEH37 carry over the upstream exclusions from Script 07. CEH3 sits at the tidal boundary and is outside the SSM’s operational domain (report §S4.4.2). CEH4 carries both a coastal-erosion drift and a post-2017 clearfell drawdown pulse (S.7); both confound any lag or seasonality signal. The same set is shared across 22, 23 and 24 so that the three diagnostics describe a coherent well population.
...
Script 22’s per-well AR(1) diagnostics show that the network’s Model B residuals are close to white. Across the 64 wells with valid fits, the AR(1) coefficient sits at a small negative value at most wells (network mean φ ≈ −0.13, median ≈ −0.13), and only three wells exceed |φ| = 0.3. The α-vs-φ scatter shows essentially no relationship (Pearson r ≈ −0.08): wells with large persistent subsidies (high α) are not preferentially the wells with the most time-structured residuals. The constant part of the unmodelled input — whatever it is — sits in α, and the small remaining time-varying part is not strongly clustered in any particular well or cluster.
Script 23’s hypothesis test returns a null result. With 50 of 63 wells showing a significant peak in the cross-correlation against rainfall, the Spearman rank correlation of peak lag against ridge distance is ρ = −0.006, p = 0.97. The mean peak lag (2.8 months across all significant wells) does not vary systematically with distance from the ridge. The cluster ordering of mean peak lag (C1, C2 ≈ 2.0 months; C3 ≈ 3.0; C4 ≈ 4.4) does not parallel the ridge-distance ordering: C4 and C5 sit at the southern forest margin, not adjacent to the northern ridge. The b₁₀ / b₁₁ pattern by cluster is informative in its own right: forest clusters carry a larger lagged-rainfall coefficient (mean b₁₁ ≈ 0.7 in C4, 0.5 in C5, versus near zero in C1–C3), consistent with greater vadose-zone storage and longer monthly-scale rainfall propagation under tree canopies, but unrelated to ridge transport.
The Spearman test on its own does not corroborate the §4.4.2 / §5.3 ridge-recharge interpretation. As the structural caveat on Script 23’s test design records, however, the test as constructed cannot resolve the mechanism either way: the null result is consistent with both (a) the residual being largely model error and (b) ridge recharge being delivered as a near-steady baseflow that is observationally indistinguishable from the constant α that Model B already absorbs — but it is also consistent with (c) the test design lacking the temporal resolution to detect the lag pattern even if it were present. The chapter for the main report’s §5.3 reflects this: ridge recharge is retained as a candidate mechanism in the discussion, but the present monitoring design cannot quantitatively confirm or rule it out. Script 23’s null result documents the analytical attempt and the limit of what the present data can support; it should not be read as positive evidence against the mechanism.
Script 24’s seasonal diagnostic supports neither the Thornthwaite-PET-misspecification hypothesis nor the dominant-unmodelled-summer-ET interpretation. Across all 63 wells, no well’s correlation between e(t) and sunshine hours exceeds the Bartlett threshold (|r| < 0.15 at every well); the network mean is −0.03. The summer-minus-winter residual at the cluster mean is small in magnitude across all clusters (between −0.007 and +0.003 m). The seasonal amplitude per well is small (cluster mean amplitudes between 0.009 and 0.015 m). What the diagnostic does find is a strong phase preference: 47 of 63 wells have their residual climatology peaking in the November–March window (winter to early spring), and zero wells peak in the May–August window. This is not the signature of unmodelled summer ET — it is consistent with threshold or nonlinear recharge behaviour not captured by the linear β₁·P term: in mid-winter with saturated soils, rainfall reaches the water table with higher efficiency than the cluster-mean β₁ represents. The Thornthwaite-misspecification hypothesis can be set aside; what remains is a small but coherent winter-spring residual that is not ridge-related.
Taken together, the three diagnostics narrow the space of admissible interpretations of the residual but do not by themselves settle it. The spatial concentration of large residuals along the northern ridge flank, of which CEH14 is the canonical example, remains real and is the empirical observation that motivates the ridge-recharge framing. Script 24 establishes that the residual does not carry a Thornthwaite-bias signature, ruling out the unmodelled-summer-ET interpretation. Script 23 establishes that no month-scale distance-dependent lag pattern is detectable — but, as the structural caveat above records, the test design at monthly resolution cannot resolve a sub-monthly lag if one were present, and the residual amplitude itself sits at the noise floor of per-well α uncertainty. What survives across all three diagnostics is a residual that is (a) constant in time at the level the model can resolve, (b) absorbed into α at any well where it dominates, and (c) carries a small winter-spring recharge nonlinearity at the wider network. Whether the residual is genuinely lateral inflow from the ridge, a different unmodelled mechanism, or model error, cannot be resolved from these data alone — sub-monthly water-level monitoring at ridge-adjacent wells would be the design needed to test the mechanism cleanly.
...
- Spatial sparsity for the distance-lag test. The 50 significant-peak wells in Script 23 span a ridge-distance range of roughly 270 to 2000 m, distributed unevenly across the gradient. The test has reasonable power to detect a strong monotone trend; it would be less reliable against a weak trend, and a non-rejection here is not the same as a precise estimate that the slope is zero.
...
- §4.4.2 Water-balance residual — the spatial pattern that motivates the ridge-recharge framing. Scripts 23 and 24 supply the mechanistic test of that framing.
- §5.3 Ridge-derived recharge interpretation — Script 23 is the principal test; its null result on the distance-lag relationship is the substantive finding that constrains what §5.3 can claim.
...
- S.13 — Figure 48 (SSM water-balance residual map) is the visual the diagnostic suite is interrogating; the CEH14 annotation on that figure is the headline well around which the §5.3 interpretation is anchored.
...
| `outputs/01_well_elevations.csv` | Script 01 — Upstand_m for pipe-top to ground-level conversion |
...
Curreli SD15b / SD16 reference lines on the MSL5 trajectory. The two horizontal reference lines drawn on every Script 26 trajectory plot (SD15b at −0.61 m, SD16 at −0.98 m, both below ground) are the Curreli ecohydrological thresholds, calibrated against summer minima, not against MSL5. Strictly, they are not threshold lines for the MSL5 metric. They are retained on the MSL5 plot because they are the most-recognised ecological reference points in the literature and a manager reading the figure expects to see them. The figure caption notes the offset explicitly — a cluster-mean MSL5 line touching SD15b corresponds to a 5-year summer-minimum mean roughly at SD16, given the ≈ 0.54 m offset documented in the Empirical relationship to summer minima section below.

### Empirical relationship to summer minima
...
| Cluster | n wells | MSL5 (m below ground) | 5-yr SM5 (m below ground) | Gap (5-yr SM5 − MSL5) |
|---|---|---|---|---|
...
### Intervention markers and the management-response horizon
...
| Event | Date | First post-intervention HY | Solid line at end-year | Dashed line at end-year |
|---|---|---|---|---|
...
### Method A and Method B aggregation
...
- Method A: per-well annual MSL → arithmetic cluster mean across all wells assigned to the cluster (reference + extended). Written to `26_msl_5yr_per_cluster.csv`. This is the headline monitoring metric documented in the preceding sections of this chapter, used in the §4.8.4 trajectory figure and spatial map.
...
τ-dependent window sensitivity. The MSL5 metric was adopted for the change-detection analysis in §​4.9.8 because the high interannual variability of spring water levels made shorter comparison windows (one to three years either side of each intervention) too unstable to yield interpretable results, and because the five-year spring mean was recognised as the most stable available metric for this network following involvement in the van Willegen et al. (2025) research. The spring season was selected because it is the only season in which readings are reliably obtainable across the full network: summer levels frequently fall below dipwell reach at the deeper wells, and winter readings are subject to surface ponding that truncates the measurable maximum. This represents an application of MSL5 beyond its original ecohydrological design, and the validation checks in §​5.7.5 were conducted specifically for that reason. The five-year averaging assumes that spring readings within a window are approximately independent — that a single anomalous spring is dampened to one-fifth of its value in the window mean. This holds at fast-draining wells where the SSM drainage timescale τ = Sy/β₃ is short relative to one year (C1, C2: τ 2–5 months, retaining less than 8% of a head anomaly after one year). It breaks down at slow-draining wells where τ substantially exceeds twelve months: at such wells the anomaly decays as exp(−t/τ), so after one τ it retains 37% of its amplitude, and a single wet spring contributes substantially to all subsequent readings within the window. At C4 (mean τ = 43 months), a spring head anomaly retains 76% of its amplitude one year later and does not reach the 37% threshold until 43 months after the event. The practical consequence for change detection using MSL5 is demonstrated in the main report (§4.9.8, §5.7.5, §6.9): the window ending 2025 produces an artefactual +90 mm apparent gain at C4 Forest Control wells driven by the 2024 wet spring, which disappears entirely when the window is shifted to 2023. This limitation does not affect MSL5’s ecohydrological validity at C1 and C2, where τ is short and the metric was calibrated; C4 and C5 do not host the dune-slack vegetation communities to which the metric applies. However, even within the intended open-dune application, slower C3 wells (CEH9, CEH39, CEH21; τ 9–11 months) are borderline: a major anomalous spring carries a non-trivial residual into subsequent readings at these wells. Researchers applying MSL5 at other sites should compute τ per well from the SSM fit and treat any well with τ > 12 months as potentially window-sensitive. The spatial MSL5 change analysis in §4.9.8 uses window end 2023, which passes three validation checks documented in §5.7.5.
...
| File | Description |
|---|---|
| `outputs/26_van_willegen_msl/26_msl_annual_per_well.csv` | Per (well, hydro_year) annual MSL and MAX in both pipe-top and below-ground frames, with completeness flags |
| `outputs/26_van_willegen_msl/26_msl_5yr_per_well.csv` | Per (well, end_year) 5-year MSL5 and MAX5 with cluster identifiers and network flag |
...
- F.4 — Curreli SD15b / SD16 constants; the MSL_* constants; `VW_QUADRAT_WELLS`; `INTERVENTION_COLOUR_*`
- F.5 — `paths.OUT_26_*`, `paths.DIR_26`
...
- §6.9 (main report) — τ-dependent MSL5 window sensitivity, three-check validation protocol, and caveat for other researchers applying MSL5 at sites with heterogeneous drainage architecture.
- §5.7.5 (main report) — detailed validation of the 2023 window choice: spring gap stability test (Mann–Whitney p = 1.000), independent summer minimum slope correlation (Pearson r = 0.49, p = 0.0005, n = 46 wells), and early-summer PET intensification (May p = 0.002, June p = 0.030, July p = 0.006).

End of chapter S.18.

## S.18b — Spring MSL forecasting tools (Script 11 Section 5, Script 26b)

Step 31 / 35 for Script 26b (Phase 13 — Van Willegen MSL Analyses in `run_analysis.py`); Script 11 Section 5 lives in Phase 1 at step 22/35 inside `11_forecasting_thresholds.py`. Companion to S.18; followed in the supplement by S.18c which documents Script 26c’s display-only report-format figures.
...
| Cluster | α (h_max_winter) | β (P_win_to_spr) | γ (PET_win_to_spr) | Intercept | R² | n |
|---|---|---|---|---|---|---|
| C1 Lake Edge | +0.116 | +0.00113 | −0.00141 | −0.689 | 0.727 | 19 |
| C2 Dune | +0.384 | +0.00128 | −0.00171 | −0.708 | 0.843 | 20 |
| C3 Western Residual | +0.619 | +0.00099 | −0.00100 | −0.764 | 0.887 | 20 |
| C4 Main Forest | +0.830 | +0.00089 | +0.00042 | −1.000 | 0.955 | 19 |
| C5 Coastal Forest | +0.760 | +0.00039 | −0.00028 | −0.609 | 0.964 | 19 |

Coefficients on h_max_winter, P_win_to_spr are positive everywhere and statistically significant (p < 0.01) at all clusters except Lake Edge, where the winter-peak coefficient is small (+0.12) and non-significant (p = 0.54). PET_win_to_spr coefficients are negative at all clusters except Main Forest, where the value (+0.0004) is essentially zero and statistically non-significant (p = 0.75). Physical interpretation: rainfall over the October-to-May window raises the next spring’s MSL; potential evapotranspiration lowers it; and the previous winter peak — when statistically distinguishable from zero — carries information about the antecedent groundwater state.
Five-cluster R² values range from 0.727 (Lake Edge) to 0.964 (Coastal Forest), with the two forest clusters (C4 and C5) fitted essentially deterministically. The Lake Edge cluster has the lowest R² of the five but the fit is still strongly predictive; its lower R² is consistent with the lake’s hydrological buffering, where the lake stage moderates year-to-year sensitivity of the dune water table to the winter peak.

#### S.18b.2.4 A rejected variant
...
### S.18b.3 Tool B: UKCP18 RCP8.5 MSL5 climate projection


#### S.18b.3.1 Specification
...
#### S.18b.3.3 What the projection is and is not
...
| Cluster | β₁ | β₂ | Spring Δh mean 2050s (m/month) | ΔMSL5 2050s (m) | ΔMSL5 2080s (m) |
|---|---|---|---|---|---|
| C1 Lake Edge | 4.58 | 0.96 | −0.012 | −0.012 | −0.021 |
| C2 Dune | 3.98 | 1.77 | −0.017 | −0.017 | −0.031 |
| C3 Western Residual | 3.57 | 1.85 | −0.017 | −0.017 | −0.031 |
| C4 Main Forest | 2.52 | 2.55 | −0.021 | −0.021 | −0.039 |
| C5 Coastal Forest | 2.41 | 1.32 | −0.012 | −0.012 | −0.022 |
...
#### S.18b.3.5 The spring-window structural cancellation
...
#### S.18b.3.6 Climate signal in context
...
### S.18b.4 Method A and Method B aggregation
...
## S.19 Scripts 28, 29 — Cluster framework diagnostics (Phase 14, post-review additions)

Steps 33 and 34 / 35. Phase 14 — Cluster framework diagnostics (post-review additions).
...
### S.19.1 Script 28 — C3 detrend check
...
### S.19.2 Script 29 — Within-C3 variance attribution
...
| Metric | R² (full) | adj-R² | Strongest unique predictor (drop-one Δ) |
|---|---|---|---|
...
### A. Greyscale figure post-processing — Script 27
...
Two classes of figure do not survive naive greyscale conversion and are flagged in the output with `[REVIEW]`. Script 11b’s categorised ecological-zone maps (`11b_01_summer_minima_depth`, `11b_02_winter_maxima_depth`, `11b_03_pflood`) use four or five colour bands that collapse to similar mid-greys under any luminance transform — distinguishing them in print needs native B&W rendering with hatched zones rather than post-processing. The Script 19 difference maps (`difference_maps/diff_*`) use diverging RdBu colormaps with colourbar labels reading “blue = wetter / red = drier” baked into the raster; the labels become meaningless in greyscale and cannot be post-edited. Both classes are still converted by default (the output is usable, just suboptimal), and `--exclude-problem` skips them entirely.
The CLI is
```
`python src/27_greyscale_figures.py [--enhanced] [--dpi DPI] [--skip-maps]``                                   [--exclude-problem] [--dry-run]`
```
with `--dpi` overriding the source DPI, `--skip-maps` excluding the spatial-map outputs that benefit from manual review, and `--dry-run` listing the files that would be converted without writing anything. Script 27 is orchestrated by `run_analysis.py` as PHASE_15 (step 35/35) but is a post-analysis utility rather than an analytical step — it is excluded from the supplement’s 33-step analytical pipeline count and lives here in Appendix A. The script is idempotent: deleting `outputs_bw/` and re-running rebuilds it cleanly.
Script 27’s filename prefix (`27_`) and orchestrator step number (35/35) deliberately do not match — the same convention applied to Script 26 (`26_van_willegen_msl.py` at step 30/35), Script 26b (`26b_van_willegen_msl_projections.py` at step 31/35), and Script 26c (`26c_msl5_report_figures.py` at step 32/35). The filename groups Script 27 alphabetically with the other `2x_` scripts; the orchestrator number reflects its position in the run order after Phase 14. Under the supplement’s analytical-step count (33 steps, Phases 1–14 of `run_analysis.py`), Script 27 has no analytical-step number — it is the post-analysis utility documented in this appendix. Script 26c, the MSL5 report-format figure-rendering companion, is similarly excluded from the analytical count and is documented in §S.18c.
...
The convention throughout the supplement is that there is one place to change a value and one place to look it up. The table reflects that. Entries are organised into thematic groups — model formulation, partition and cluster constants, intervention machinery, scenario engine, ecological thresholds, spatial machinery, MSL aggregation, and provenance and orchestration.
...
| `MIN_OBS = 30` | F.5 / `model_utils.py` | Minimum aligned rows for any per-well SSM fit |
| `LCSC_DATA_LIMIT = 100` | F.5 / `model_utils.py` | Recent-window length for intercept-audit fits |
...
#### Partition and cluster constants
...
| Storage–drainage index τ = Sy/β₃ | S.12 / `18_wtf_05_drainage_timescale.csv` | Deliberately storage-weighted composite; not a residence time. Head-space recession time is t_R = 1/β₃ (see S.12 Site-specific choices; cluster means C1 ≈ 10, C2 ≈ 14, C3 ≈ 20, C4 ≈ 57, C5 ≈ 22 months) |
| Interception correction for forest Sy | S.12 | Cluster vs well-level reconciliation |


#### Forest and forest scenarios
...
| `summer_amplification_factors()`, `scenario_cluster_sy()`, `flux_to_summer_min_mm()`, `scenario_summer_min_bars()` | F.5 / `scraping_common.py` | Flux → summer-minimum scenario conversion (S.6); single source consumed by Scripts 09b (`09b_05`) and 21 (`21_forestry_06`) |
| `pflood_lambda()` | F.5 / `model_utils.py` / S.9 | Iterated closed-form P_flood threshold |
| UKCP18 2050s scaling factors | F.4 / `config.py` | RCP8.5 Wales, 50th percentile, 2050s; `UKCP18_DRY_*` / `UKCP18_WET_*` constants in `config.py` |
| UKCP18 2080s scaling factors | F.4 / S.18b / `26b_van_willegen_msl_projections.py` | Currently hardcoded in Script 26b’s `UKCP18_SCENARIOS` dict; follow-up to centralise as `UKCP18_2080s_*` in `config.py` |
...
| 5-year MSL window | S.18 / `config.MSL_DEFAULT_WINDOW_YEARS = 5` | Van Willegen 2025 Figure 4 (correlation stabilises at 5 years) |
...
| Concept / parameter | Canonical source | Notes |
|---|---|---|
| `paths.py` `OUT_*` / `INT_*` / `DIR_*` | F.5 / `utils/paths.py` | All file paths; no script hardcodes a path |
...
| Greyscale post-processor | Appendix A (this chapter) / `27_greyscale_figures.py` | Phase 15 in `run_analysis.py` (step 35/35); outside the 33-step analytical count |
...
| `paths.DIR_26C`, `paths.OUT_26C_*` | F.5 / `utils/paths.py` / S.18c | All Script 26c outputs (trajectory report figure, ΔMSL5 vs Δsummer-min projection figure, results transcript) |
...
| `paths.OUT_11C_*` | F.5 / `utils/paths.py` / S.9.3 | Script 11c P_flood achievability outputs (sharing `DIR_11B` with Script 11b): map PNG, per-well category CSV, results memo |
...
| `paths.DIR_28`, `paths.OUT_28_*` | F.5 / `utils/paths.py` / S.19.1 | All Script 28 C3 detrend check outputs (per-well CSV, results memo, four-panel figure) |
| `paths.DIR_29`, `paths.OUT_29_*` | F.5 / `utils/paths.py` / S.19.2 | All Script 29 within-C3 variance outputs (per-well panel CSV, univariate R² matrix, drop-one matrix, results memo, six-panel figure) |
...
The Methods Supplement closes here. The chapters S.1–S.19 together document the 33-step Newborough Warren analytical pipeline, the design choices behind each step, the rationale for site-specific parameters, and the verification chain by which pipeline outputs feed the main report. Script 26c (`26c_msl5_report_figures.py`, Phase 13 in `run_analysis.py`) is a display-only figure-rendering companion to Scripts 26 and 26b, outside the 33-step analytical count, covered in §S.18c; Script 27 (`27_greyscale_figures.py`, Phase 15 in `run_analysis.py`) is a post-analysis figure-conversion utility, also outside the 33-step analytical count, covered in Appendix A. Readers needing a specific topic should consult the canonical-sources table in Appendix B; readers needing the canonical implementation of any function or constant should consult the live `main` branch of , which remains the source of truth. The supplement is a guide to what the repository contains and why each choice was made; the repository itself is the deliverable.
