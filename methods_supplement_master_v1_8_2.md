...
*Document version: 1.8.2 (June 2026).*

---

## Pipeline at a glance

**`run_analysis.py` — 44 steps across 17 phases**
41 analytical steps (Phases 1–16) + 3 display/utility steps excluded from the analytical count.
...
| 15 | 36–38 | Differential change, climate envelope (32, 33, 35) | §S.20 |
...
- **Script 29** (`29_c3_within_variance_check.py`) — Within-C3 variance attribution. Step 34/44, Phase 14. Routed from the 2026-05-29 post-review cascade. Documented in S.19.2.
- **Script 30** (`30_c4_constrained_fit.py`) — C4 constrained-β₃ triangulation sensitivity. Step 35/44, Phase 14. Added 2026-06-23. Documented in S.19.3.
- **Script 32** (`32_differential_movement.py`) — Secular differential water-table movement; report Figure 59. Step 36/44, Phase 15. Documented in S.20.1.
- **Script 33** (`33_envelope_amplification.py`) — Climate-swing amplification and dry-year spring floor; report Figure 60. Step 37/44, Phase 15. Documented in S.20.2.
- **Script 35** (`35_per_well_amplification.py`) — Per-well climate-sensitivity coefficient (discrete companion to the Figure 60a surface). Step 38/44, Phase 15. Documented in S.20.3.
- **Script 24b** (`24b_residual_climatology.py`) — Cluster-stratified residual climatology; supplementary diagnostic. Step 39/44, Phase 16. Documented in S.21.1.
- **Script 31** (`31_cluster_validation.py`) — Independent k=5 partition validation; supplementary diagnostic. Step 40/44, Phase 16. Documented in S.21.2.
- **Script 31b** (`31b_separation_vs_recoverability.py`) — Cluster separation versus recoverability; supplementary diagnostic. Step 41/44, Phase 16. Documented in S.21.3.
- **Script 34** (`34_window_sensitivity.py`) — MSL5 two-window sensitivity demonstration. Step 42/44, Phase 16. Documented in S.21.4.
- **Script 09f** (`09f_management_effects.py`) — Management-interventions-versus-coastal-retreat spatial-reach synthesis figure. Step 43/44, Phase 17. Display/utility; excluded from the supplement's 41-step analytical count. Documented in §S.15c.
- **Script 27** (`27_greyscale_figures.py`) — Post-pipeline greyscale figure-rendering utility. Step 44/44, Phase 17. Excluded from the supplement's 41-step analytical count. Documented in Appendix A.
...
The eleven Phase 4 chapters (S.8–S.14) cover the climate-and-spatial scripts that feed §4 of the report. Phase 5 collects six further chapters whose methodological purpose is supplementary: each consumes pipeline intermediates, none feeds downstream pipeline scripts, and the conclusions they produce inform the report's discussion rather than its core results chain. S.15 covers Script 25 (coastal-retreat gradient, step 26/44, Phase 11 in `run_analysis.py`). S.15c covers Script 09f (spatial-reach synthesis figure, step 43/44, Phase 17; display/utility). S.16 covers Scripts 22, 23, and 24 (residual diagnostics, steps 27–29/44, Phase 12 in `run_analysis.py`). S.18 covers Script 26 (van Willegen 5-year MSL aggregation, step 30/44, Phase 13). S.18b covers Script 11 Section 5 (Tool A — spring MSL transfer function, embedded in Script 11 at step 22/44, Phase 1) together with Script 26b (Tool B — UKCP18 RCP8.5 MSL5 projections, step 31/44, Phase 13). S.18c covers Script 26c (MSL5 report-format figures, step 32/44, Phase 13). S.19 covers Scripts 28, 29 and 30 (cluster framework diagnostics, steps 33–35/44, Phase 14). S.20 covers Scripts 32, 33 and 35 (observed differential change and climate-response envelope, steps 36–38/44, Phase 15). S.21 covers Scripts 24b, 31, 31b and 34 (supplementary standalone diagnostics, steps 39–42/44, Phase 16). S.17 is appendices. All scripts in S.15, S.15c, S.16, S.18, S.18b, S.18c, S.19, S.20, and S.21 are part of the pipeline orchestrated by `run_analysis.py`; the "post-pipeline supplementary" framing reflects their role in the report (discussion-feeding rather than results-feeding), not their orchestration status. Scripts 26c (MSL5 report-format figures, Phase 13), 09f (spatial-reach synthesis, Phase 17), and 27 (greyscale figures, Phase 17) are display/utility steps excluded from the 41-step analytical count; Script 26c is documented in S.18c, Script 09f in S.15c, and Script 27 in Appendix A.
...
**Step 43/44, Phase 17. Display/utility; excluded from the 41-step analytical count. Documented here as a companion to §S.15 (Script 25, coastal gradient), whose outputs it reads.**
...
**Step 31 / 35 for Script 26b (Phase 13 — Van Willegen MSL Analyses in `run_analysis.py`); Script 11 Section 5 lives in Phase 1 at step 22/44 inside `11_forecasting_thresholds.py`. Companion to S.18; followed in the supplement by S.18c which documents Script 26c's display-only report-format figures.**
...
| Greyscale post-processor | Appendix A (this chapter) / `27_greyscale_figures.py` | Phase 17 in `run_analysis.py` (step 44/44); outside the 41-step analytical count |
| Spatial-reach synthesis figure | §S.15c / `09f_management_effects.py` | Phase 17 in `run_analysis.py` (step 43/44); display/utility; outside the 41-step analytical count |
