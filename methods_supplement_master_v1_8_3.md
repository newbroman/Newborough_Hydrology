...
*Document version: 1.8.3 (June 2026).*

---

## Pipeline at a glance

**`run_analysis.py` — 45 steps across 17 phases**
42 analytical steps (Phases 1–16) + 3 display/utility steps excluded from the analytical count.

| Phase | Steps | Content | Supplement chapter |
|---|---|---|---|
| 1–2 | 1–12 | Data preparation, clustering, SSM | §S.1–§S.4 |
| 3 | 13–17 | Intervention diagnostics, threshold analysis | §S.5–§S.9 |
| 4 | 16–24 | Scraping BACI, clearfell BACI, spatial SSM | §S.6–§S.7, §S.13 |
| 5–11 | 25–29 | WTF, scenarios, coastal gradient | §S.10–§S.15 |
| 12 | 27–29 | Residual diagnostics (Scripts 22–24) | §S.16 |
| 13 | 30–32 | Van Willegen MSL, EWI, report figures | §S.18, §S.18b, §S.18c |
| 14 | 33–35 | Cluster framework diagnostics (28, 29, 30) | §S.19 |
| 15 | 36–39 | Differential change, climate envelope, absolute trend (32, 33, 35, 36) | §S.20 |
| 16 | 39–42 | Supplementary diagnostics (24b, 31, 31b, 34) | §S.21 |
| 17 | 43–44 | Spatial-reach synthesis (09f); greyscale (27) ‡ | §S.15c; App. A |

‡ Display/utility steps. Also display/utility: Script 26c (step 32, Phase 13, §S.18c). Two post-review scripts inserted into earlier phases: Script 11c (step 13, Phase 3, §S.9.3) and Script 14b (step 16, Phase 4, §S.8.5).
...
The supplement covers the 41-step analytical pipeline (Phases 1–16) in script-by-script chapters. Three display/utility steps sit outside the analytical count: Script 26c (MSL5 report-format figures, step 32, §S.18c), Script 09f (spatial-reach synthesis, step 43, §S.15c), and Script 27 (greyscale, step 44, Appendix A). Each chapter addresses seven concerns: motivation, inputs, methodology, site-specific choices and rationale, outputs, limitations and known caveats, and where the result appears in the report. Some chapters are short (figure-only or post-processing scripts); some are longer (Scripts 01, 02, 03, 10 suite, 17, 21, 25, 26). Total document length is approximately 110 pages.
...
- **Script 11c** (`11c_pflood_achievability.py`) — Per-well P_flood achievability categorical priority map. Operationalises Conclusion 4's λ < 1.5 priority criterion. Step 13/45, Phase 3. Routed from the 2026-05-29 post-review cascade. Documented in S.9.3.
- **Script 14b** (`14b_year_of_crossing.py`) — Bootstrap year-of-crossing for Curreli (2013) ecological thresholds. Step 16/45, Phase 4. Routed from the 2026-05-29 post-review cascade. Documented in S.8.5.
- **Script 25** (`25_coastal_gradient.py`) — Coastal-retreat gradient analysis. Step 26/45, Phase 11. Documented in S.15.
- **Script 26** (`26_van_willegen_msl.py`) — Van Willegen 2025 observational 5-year MSL aggregation, plus (v1.3.2) the equilibrium wetness index and its Ellenberg-F cross-validation. Step 30/45, Phase 13. Documented in S.18.
- **Script 26b** (`26b_van_willegen_msl_projections.py`) — UKCP18 RCP8.5 MSL5 climate projections (Tool B). Step 31/45, Phase 13. Documented in S.18b.
- **Script 26c** (`26c_msl5_report_figures.py`) — MSL5 report-format figures for §4.8.4 and §4.10.1 of the main report. Display-only companion to Scripts 26 and 26b; reads only canonical outputs from Scripts 26, 26b, and 19; no recomputation. Step 32/45, Phase 13. Excluded from the supplement's 41-step analytical count. Documented in S.18c.
- **Script 28** (`28_c3_detrend_check.py`) — C3 detrend check; quantitative validation of the aquifer-architecture framing. Step 33/45, Phase 14. Routed from the 2026-05-29 post-review cascade. Documented in S.19.1.
- **Script 29** (`29_c3_within_variance_check.py`) — Within-C3 variance attribution. Step 34/45, Phase 14. Routed from the 2026-05-29 post-review cascade. Documented in S.19.2.
- **Script 30** (`30_c4_constrained_fit.py`) — C4 constrained-β₃ triangulation sensitivity. Step 35/45, Phase 14. Added 2026-06-23. Documented in S.19.3.
- **Script 32** (`32_differential_movement.py`) — Secular differential water-table movement; report Figure 59. Step 36/45, Phase 15. Documented in S.20.1.
- **Script 33** (`33_envelope_amplification.py`) — Climate-swing amplification and dry-year spring floor; report Figure 60. Step 37/45, Phase 15. Documented in S.20.2.
- **Script 35** (`35_per_well_amplification.py`) — Per-well climate-sensitivity coefficient (discrete companion to the Figure 60a surface). Step 38/45, Phase 15. Documented in S.20.3.
- **Script 36** (`36_absolute_climate_trend.py`) — Absolute climate-removed per-well secular trend map. Step 39/45, Phase 15. Documented in S.20.4.
- **Script 24b** (`24b_residual_climatology.py`) — Cluster-stratified residual climatology; supplementary diagnostic. Step 39/45, Phase 16. Documented in S.21.1.
- **Script 31** (`31_cluster_validation.py`) — Independent k=5 partition validation; supplementary diagnostic. Step 40/45, Phase 16. Documented in S.21.2.
- **Script 31b** (`31b_separation_vs_recoverability.py`) — Cluster separation versus recoverability; supplementary diagnostic. Step 41/45, Phase 16. Documented in S.21.3.
- **Script 34** (`34_window_sensitivity.py`) — MSL5 two-window sensitivity demonstration. Step 42/45, Phase 16. Documented in S.21.4.
- **Script 09f** (`09f_management_effects.py`) — Management-interventions-versus-coastal-retreat spatial-reach synthesis figure. Step 43/45, Phase 17. Display/utility; excluded from the supplement's 41-step analytical count. Documented in §S.15c.
- **Script 27** (`27_greyscale_figures.py`) — Post-pipeline greyscale figure-rendering utility. Step 45/45, Phase 17. Excluded from the supplement's 41-step analytical count. Documented in Appendix A.
...
## S.7  Script 10 suite (a–m) — Clearfell BACI
...
### Sub-script 10m — WMC3 BACI dual-panel display figure (v1.0.0)
...
#### Net water-table state map (`20_net_state_map.png`)
...
#### Modelled 2005→2025 driver-change map (`20_driver_change_2005_2025.png`)
...
| `20_spatial_figures/20_scrape_drawdown_nohead.png` | Dune-scrape drawdown field, all eight cuts, leaky-aquifer superposition with method-of-images coastal boundary correction; H₀ empirically anchored at three monitored cuts (CEH36, CEH21, CEH18) from live Script 09a BACI shifts; shared discrete drawdown scale | §3.4.5 / §5.4.2 (Figure 52) |
...
**Volumetric companion (`21_forestry_06`).** Alongside the volumetric panel, `plot_scenario_comparison()` also emits `21_forestry_06_summer_scenario.csv`: the per-cluster equilibrium volumetric metric `Delta_vol_summer_mm_per_month` for the three forest-management scenarios (clearfell, thinning, broadleaf), taken directly from the monthly scenario values and byte-identical with the forestry rows of `09b_05` by construction. The former summer-minimum amplification conversion (`scraping_common` flux → head ÷ Sy → summer minimum × amplification factor) was removed on 2026-07-02. This gives the §4.9.6 forestry narrative a forestry-module source for the volumetric ecological metric rather than having to reach into the scraping suite for it; the climate-scenario comparison remains with `09b_05`.
...
| `21_forestry_06_summer_scenario.csv` | Per-cluster equilibrium volumetric metric `Delta_vol_summer_mm_per_month` for clearfell, thinning, broadleaf — byte-identical with `09b_05` forestry rows; former summer-minimum amplification conversion removed 2026-07-02 | Main report §4.9.6 |
...
The eleven Phase 4 chapters (S.8–S.14) cover the climate-and-spatial scripts that feed §4 of the report. Phase 5 collects six further chapters whose methodological purpose is supplementary: each consumes pipeline intermediates, none feeds downstream pipeline scripts, and the conclusions they produce inform the report's discussion rather than its core results chain. S.15 covers Script 25 (coastal-retreat gradient, step 26/45, Phase 11 in `run_analysis.py`). S.15c covers Script 09f (spatial-reach synthesis figure, step 43/45, Phase 17; display/utility). S.16 covers Scripts 22, 23, and 24 (residual diagnostics, steps 27–29/45, Phase 12 in `run_analysis.py`). S.18 covers Script 26 (van Willegen 5-year MSL aggregation, step 30/45, Phase 13). S.18b covers Script 11 Section 5 (Tool A — spring MSL transfer function, embedded in Script 11 at step 22/45, Phase 1) together with Script 26b (Tool B — UKCP18 RCP8.5 MSL5 projections, step 31/45, Phase 13). S.18c covers Script 26c (MSL5 report-format figures, step 32/45, Phase 13). S.19 covers Scripts 28, 29 and 30 (cluster framework diagnostics, steps 33–35/45, Phase 14). S.20 covers Scripts 32, 33, 35 and 36 (observed differential change and climate-response envelope, steps 36–39/45, Phase 15). S.21 covers Scripts 24b, 31, 31b and 34 (supplementary standalone diagnostics, steps 40–43/45, Phase 16). S.17 is appendices. All scripts in S.15, S.15c, S.16, S.18, S.18b, S.18c, S.19, S.20, and S.21 are part of the pipeline orchestrated by `run_analysis.py`; the "post-pipeline supplementary" framing reflects their role in the report (discussion-feeding rather than results-feeding), not their orchestration status. Scripts 26c (MSL5 report-format figures, Phase 13), 09f (spatial-reach synthesis, Phase 17), and 27 (greyscale figures, Phase 17) are display/utility steps excluded from the 41-step analytical count; Script 26c is documented in S.18c, Script 09f in S.15c, and Script 27 in Appendix A.
...
**Step 43/45, Phase 17. Display/utility; excluded from the 41-step analytical count. Documented here as a companion to §S.15 (Script 25, coastal gradient), whose outputs it reads.**
...
| Input file | Source script | Contents used |
|---|---|---|
...
**Step 31 / 35 for Script 26b (Phase 13 — Van Willegen MSL Analyses in `run_analysis.py`); Script 11 Section 5 lives in Phase 1 at step 22/45 inside `11_forecasting_thresholds.py`. Companion to S.18; followed in the supplement by S.18c which documents Script 26c's display-only report-format figures.**
...
## S.20  Scripts 32, 33, 35, 36 — Observed differential change and the climate-response envelope

Phase 15 (steps 36–39/45). Four figures that characterize observed network change directly, independently of the single-mechanism driver model of §S.13–S.14: where the spring water table is moving relative to the site (Script 32, Figure 59), how far it swings between climate extremes (Script 33, Figure 60a), how deep it sits in dry years against the ecological thresholds (Script 33, Figure 60b), a per-well frame-independent coefficient extending the amplification metric to short-record wells (Script 35), and an absolute climate-removed secular trend map placing the coastal and forest drying on one figure without mean-referencing or residual-construction artefacts (Script 36). All four read committed pipeline outputs and re-fit nothing in the SSM; their value layers are observational.
...
**Step 39/45, Phase 15. Added 2026-07-05 (v1.0.4). Analytical step; counted in the 42-step analytical total.**
...
**Report location.** Figure number TBC with Martin. Main-report §4.9.8 / §5.7.5, as the absolute companion to Figure 59 (Script 32 differential map). Pairs with the Script 20 modelled 2005→2025 driver-change map (`20_driver_change_2005_2025.png`) — modelled versus observed over the same window.
...
| Greyscale post-processor | Appendix A (this chapter) / `27_greyscale_figures.py` | Phase 17 in `run_analysis.py` (step 45/45); outside the 41-step analytical count |
| Spatial-reach synthesis figure | §S.15c / `09f_management_effects.py` | Phase 17 in `run_analysis.py` (step 43/45); display/utility; outside the 41-step analytical count |
