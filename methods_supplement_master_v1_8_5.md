...
*Document version: 1.8.5 (July 2026).*
...
41 analytical steps across 16 analytical phases, plus 3 display/utility steps (Scripts 26c, 09f, 27) excluded from the analytical count. The remainder are opt-in supplementary diagnostics (Phase 15's tail — Scripts 36, 37, 37b — and all of Phase 16), which run only with `--with-supplementary`.
...
‡ Display/utility steps. Also display/utility: Script 26c (step 32, Phase 13, §S.18c). Two post-review scripts inserted into earlier phases: Script 11c (step 13, Phase 3, §S.9.3) and Script 14b (step 16, Phase 4, §S.8.5). "Opt-in" steps (Phase 15's tail and all of Phase 16) run only with `run_analysis.py --with-supplementary` or the interactive menu's option 1 prompt; they are tier-X diagnostics, not part of the 41-step analytical count, but they are registered pipeline steps (not ad hoc scripts) and do appear in the numbering above and in `pipeline_manifest.json`.
...
- **Script 36** (`36_absolute_climate_trend.py`) — Absolute climate-removed per-well secular trend map. Step 39/48, Phase 15. **Opt-in diagnostic tier** (runs with `--with-supplementary`); not part of the 41-step analytical count. Documented in S.20.4.
- **Script 37** (`37_driver_validation.py`) — Predicted-vs-observed driver-change validation (scatter + residual map). Step 40/48, Phase 15. **Opt-in diagnostic tier.** Documented in S.20.5.
- **Script 37b** (`37b_driver_footing.py`) — Part B: comparative driver footing — forest, scrape, and coastal-retreat effects on common currencies (peak local head change, area-integrated volume, ecological-threshold crossings). Step 41/48, Phase 15. **Opt-in diagnostic tier.** Documented in S.20.6.
...
- **Script 34** (`34_window_sensitivity.py`) — MSL5 two-window sensitivity demonstration. Step 45/48, Phase 16. **Opt-in diagnostic tier.** Documented in S.21.4.
- **Script 38** (`38_coastal_transect.py`) — Coast-to-inland MAM transect; observational delta_0 (coastal-retreat) diagnostic distinguishing a growing (erosion-consistent) from a static (substrate-geometry) coast-inland head gradient. Step 46/48, Phase 16. **Opt-in diagnostic tier.** Wired into `run_analysis.py` 2026-07-08 (previously standalone). Documented in S.21.5.
- **Script 09f** (`09f_management_effects.py`) — Management-interventions-versus-coastal-retreat spatial-reach synthesis figure. Step 47/48, Phase 17. Display/utility; excluded from the supplement's 41-step analytical count. Documented in §S.15c.
- **Script 27** (`27_greyscale_figures.py`) — Post-pipeline greyscale figure-rendering utility. Step 48/48, Phase 17. Excluded from the supplement's 41-step analytical count. Documented in Appendix A.
...
**Step 47/48, Phase 17. Display/utility; excluded from the 41-step analytical count. Documented here as a companion to §S.15 (Script 25, coastal gradient), whose outputs it reads.**
...
**Steps 27–29/48 in the orchestrator (Phase 12 — Residual Diagnostics in `run_analysis.py`); inside the supplement's 41-step analytical count; second chapter under Phase 5 — Post-pipeline supplementary analyses in the supplement.**
...
**Step 39/48, Phase 15. Added 2026-07-05 (v1.0.4). Opt-in diagnostic tier (`exec="optin"` — runs with `run_analysis.py --with-supplementary` or the menu option 1 prompt, per the 2026-07-07 signed-off A/X/A classification); not part of the 41-step analytical count. See the Pipeline-at-a-glance table and `outputs/pipeline_manifest.json`.**
...
**Step 41/48, Phase 15. Opt-in diagnostic tier (`--with-supplementary`); v1.0.1 (2026-07-07). Not part of the 42-step analytical count.**
...
**Step 46/48, Phase 16. Opt-in diagnostic tier (`--with-supplementary`); v1.3.0 (2026-07-08, wired into `run_analysis.py` — previously standalone). Not part of the 42-step analytical count.**
...
| Greyscale post-processor | Appendix A (this chapter) / `27_greyscale_figures.py` | Phase 17 in `run_analysis.py` (step 48/48); outside the 41-step analytical count |
| Spatial-reach synthesis figure | §S.15c / `09f_management_effects.py` | Phase 17 in `run_analysis.py` (step 47/48); display/utility; outside the 41-step analytical count |
