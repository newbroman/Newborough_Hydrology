...
>
> Written **2026-04-17** and held in the Drive project store, not the repository,
> which is why `17_wtf_specific_yield.py:29` cited a filename that did not
> resolve. Recovered under T-10 and kept **verbatim below** — it is a dated
> record and is not edited.
>
> **§2 is the definitive statement of why PET stays gross, and it still holds.**
> So does §3, whose event-pool-recovery mechanism explains a counter-intuitive
> result nothing else in the project derives: the interception correction moves
> the cluster median *up*, not down. Both are now summarised in
> `INTERCEPTION_TREATMENT.md` §4(c).
>
> **Everything numeric below is superseded.** The document predates the k = 5
> repartition, the `pdfs` blacklist, and D-021. Specifically:
>
> | the April text says | as of 2026-08-25 |
> |---|---|
> | the correction applies to the Forest cluster **C4** | it applies to **C4 and C5** (D-022) |
> | C4 uncorrected 0.215 (n = 38), corrected 0.227 (n = 44) | C4 uncorrected 0.312 (n = 51), corrected 0.260 (n = 63) |
> | open-dune range 0.223–0.259 | 0.210–0.327 |
...
>
> **§6 "action required" is discharged.** Script 17 applies the correction today,
> to both forest clusters, in Approaches B and C. Do not act on that section.
>
...
> today it is **below** (0.312 → 0.260). §3's mechanism is not thereby wrong: it
...
> reason report9 calls that cluster only weakly corroborative.
