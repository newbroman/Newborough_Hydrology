...
__version__ = "1.6.2"  # Hollingham (2026) — 2026-06-20 (coverage plates: 2-yr x ticks at 7pt for denser labelling)
# Changelog:
...
def _render_coverage_figure(wells_scope, states):
    ...
    for i, (kind, key, cl, _) in enumerate(rows):
        if kind in ("ref", "ext"):
            ...
            for j, st in enumerate(states[key].values):
                if st in ("measured", "interpolated"):
                    img[i, j, :] = face
                    if st in obs_hatch:
    ...
    legend = [
        ...
        Patch(fc="#777777", hatch="..", ec="#404040",
