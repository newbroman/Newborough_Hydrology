...
#   minus DEM" was the upstand, its RMSE the RMS upstand, and the three "outliers"
#   the two tallest upstands (CEH2 0.71 m, NW2 0.59 m) and L1. The metadata holds no
#   LiDAR sample at a surveyed well that is independent of the survey, so there is
#   nothing to compare. 01_report_numbers.csv now carries only the ground-source
#   counts (dgps / lidar / total) that report8 §3.1.2 states; DGPS_DEM_OUTLIER_M is
#   retired from config.
# 1.18.0 - 2026-09-21. The DEM-vs-DGPS elevation check was emitted (E34) — withdrawn
#   the same day, above.
...
def _report_elevation_check(elev_df, src):
    ...
    1.18.0 emitted here is withdrawn (see the 1.19.0 note): the metadata carries no
    LiDAR sample at a surveyed well that is independent of the survey."""
