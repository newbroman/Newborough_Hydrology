...
def run_stability_diagnostics(wells_ref: pd.DataFrame) -> None:
    ...
    for k in K_RANGE_BOOTSTRAP:
        step(f"Bootstrap stability at k={k} ({N_BOOTSTRAP} resamples)...")
    ...
    step(f"Saved per-well stability: {OUT_02_STABILITY_PER_WELL.name}")
...
def make_cluster_spaghetti_figure() -> None:
    """Companion to the 02_03 cluster-mean hydrograph: one stacked panel per
    cluster, each showing that cluster's reference wells as thin lines with the
    cluster mean overlaid in bold, on a shared axis so the between-cluster
    stratification and the within-cluster spread are both legible. Same quantity
    and axis as 02_03 panel (b) (raw depth-to-water, m)."""
    wells_all = pd.read_csv(INT_WELLS_CLEAN, index_col=0, parse_dates=True)
    wells_all = wells_all.apply(pd.to_numeric, errors="coerce")
    cluster_df_local = pd.read_csv(INT_CLUSTER_STATS)
    ...
    cids = sorted(
        pd.to_numeric(cluster_df_local["Cluster"], errors="coerce")
        .dropna().astype(int).unique()
    )
    n_wells = 0
    # A well is labelled where its mean level sits more than THRESH_M from its
    # cluster mean. This divergence is in absolute level only (each well's local
    # ground/pipe-top elevation); membership is set by temporal pattern, and all
    # labelled wells still correlate r>0.9 with their cluster mean.
    THRESH_M = 0.30      # |mean offset from cluster mean| to label a well (m)
    GAP_M = 0.16         # min vertical spacing between adjacent labels (m)
    GUTTER_DAYS = 760    # right-hand margin reserved for the labels (days)

    fig, axes = plt.subplots(len(cids), 1, figsize=(12, 12),
    ...
    for ax, cid in zip(axes, cids):
        ...
        avail = [c for c in members if c in wells_all.columns]
        if not avail:
            continue
        cm = wells_all[avail].mean(axis=1)
        for wl in avail:
            ax.plot(wells_all.index, wells_all[wl].values,
                    color=CLUSTER_COLOURS[cid], lw=0.6, alpha=0.45, zorder=2)
        ax.plot(wells_all.index, cm.values,
                color=CLUSTER_COLOURS[cid], lw=2.2, alpha=0.98, zorder=5)
        n_wells += len(avail)
        ax.axhline(0, color="black", lw=0.6, ls=":", alpha=0.4)
        ax.set_title(f"{CLUSTER_LABELS[cid]}  (n={len(avail)})", fontsize=10,
                     loc="left", color=CLUSTER_COLOURS[cid], fontweight="bold",
                     pad=2)
        ax.yaxis.set_major_locator(ticker.MultipleLocator(0.5))
        ax.grid(True, axis="y", lw=0.4, ls="--", alpha=0.4)
        for sp in ("top", "right"):
            ax.spines[sp].set_visible(False)
        ax.margins(x=0.005)

        # right-edge labels for wells diverging in level from the cluster mean
        diverg = []
        for wl in avail:
            if abs((wells_all[wl] - cm).mean()) > THRESH_M:
                diverg.append((wells_all[wl].iloc[-36:].mean(), wl))
        diverg.sort()
        last_y = -1e9
        for y, wl in diverg:
            y = y if y > last_y + GAP_M else last_y + GAP_M
            last_y = y
            ax.annotate(wl, xy=(wells_all.index[-1], y),
                        xytext=(6, 0), textcoords="offset points",
                        fontsize=7, va="center", ha="left",
                        color=CLUSTER_COLOURS[cid], annotation_clip=False)

    axes[-1].set_xlim(right=wells_all.index[-1] + pd.Timedelta(days=GUTTER_DAYS))
...
def compute_cluster_amplitude_descriptors(
    ...
    step(f"Computing per-well amplitude stats ({len(wells_ref.columns)} wells)...")
    rows = []
    for well in wells_ref.columns:
        ...
        if cid is None:
            ...
            skipped(f"no cluster assignment for well: {well!r}")
            continue
    ...
    saved(f"{OUT_02_AMP_PER_WELL.name}")
    ...
    saved(f"{OUT_02_AMP_SUMMARY.name}")
...
if __name__ == "__main__":
    banner("02", "k-Means Clustering & Partition", version=__version__)
    make_all_dirs()
    ...
    step(f"Clustering {len(wells_ref.columns)} reference wells...")
