...
def load_amplitude_heterogeneity(cluster_ids: list[int],
    ...
    try:
        ...
        warn(f"Could not read {OUT_02_AMP_PER_WELL.name}: {exc} \u2014 using fallback values.")
        return {cid: _AMPLITUDE_FALLBACK.get(cid, (False, np.nan, np.nan))
                for cid in cluster_ids}
...
def build_upstand_lookup(elev_path) -> dict[str, float]:
    ...
    step(f"Upstand lookup loaded: {len(lookup)} wells.")
    return lookup
...
def make_well_datum_maps(optimal_df: pd.DataFrame,
    ...
    try:
        ...
        warn("map_utils not available — skipping spatial datum maps.")
        return

    if not DATA_DIR.exists():
        warn(f"Data directory {DATA_DIR} not found — skipping spatial datum maps.")
        return
    ...
    if map_df.empty:
        warn("No wells with locations — skipping spatial datum maps.")
        return
...
def export_regional_averages_maod(cluster_df: pd.DataFrame,
    ...
    try:
        ...
        warn(f"maOD cluster averages not written: {exc}")
        return
    ...
    saved(f"{INT_CLUSTER_AVG_MAOD.name}")
...
def export_cluster_peak_months(centroids: dict[int, pd.Series]) -> None:
    ...
    saved(f"{INT_CLUSTER_PEAK_MONTHS.name}")
    ...
    try:
        ...
        note(f"Pipeline params peak_month update skipped: {e}")
...
def main() -> None:
    ...
    try:
        ...
        note(f"Pipeline params β update skipped: {e}")
    ...
    if len(valid_datums) > 0:
        ...
        warn("No reference depth produces all-positive, all-significant β₃ — check datum sensitivity figure.")
    ...
    if n_valid > 0:
        ...
        warn("No wells achieved β₃ > 0 & p < 0.05 at any datum")
    ...
    saved(f"{OUT_03_CLUSTER_SUMMARY.name}")
