...
def main():
    ...
    colours = [CLUSTER_COLOURS.get(int(c), "grey") for c in map_df["Cluster_ID"]]
    ax.scatter(map_df["E"], map_df["N"], c=colours, marker="o", s=150, edgecolor="black", linewidth=1.2, alpha=0.95, zorder=5)
    ...
    cluster_handles = [Line2D([0],[0], marker="o", color="w", label=CLUSTER_LABELS.get(c,f"C{c}"),
    ...
    plt.tight_layout(); plt.savefig(OUT_04_ARCHITECTURE_MAP); plt.close()
