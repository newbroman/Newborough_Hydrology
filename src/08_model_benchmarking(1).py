...
def compute_showdown_metrics(target_well_name, df_clean, df_climate):
    ...
    df = df.dropna(subset=['Delta_h', 'P_lag1', 'PET', 'h_prev'])
    ...
    x_trad = sm.add_constant(pd.DataFrame({'P': df['P_lag1'], 'PET': -df['PET']}), has_constant='add')
    x_lcsc = pd.DataFrame({'P': df['P_lag1'], 'PET': -df['PET'], 'h_prev': -df['h_prev']})
    y_fit = df['Delta_h']

    model_trad = sm.OLS(y_fit, x_trad).fit()
    model_lcsc = sm.OLS(y_fit, x_lcsc).fit()

    p_arr = df['P_lag1'].values
...
def plot_overlay_with_lcsc07_modelb(output_path, payload_08, payload_07):
    ...
    ax_top.plot(payload_08['index'], payload_08['h_lcsc_one'], color='#0072B2', lw=2.2, ls='--', label=f"SSM07 (R\u00b2={payload_08['r2_lcsc_one']:.3f})")
    ...
    ax_bottom.plot(
        ...
        ls='--',
        label=(
            f"SSM07 (NSE={payload_08['nse_lcsc']:.3f}, "
