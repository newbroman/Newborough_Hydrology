...
#         remain within 800 m uphill of CEH36, vs 6/19 = 32% previously).
#         Text annotation on the scenario-comparison figure updated to match.
#         Visible bar-height impact is negligible (C3 scraping bar shrinks
#         from ≈0.22 to ≈0.20 mm/month — both round to 0.2 mm/month at
#         display precision); the label change is the main user-visible
#         effect. No further downstream impact (Script 21 reads from
#         pipeline_scenario_params.csv, not from 09b's outputs).
# 1.3.0 — 2026-05-30 — _summer_scenario() now routes the
                        # amplification factors, per-cluster Sy, and the
                        # flux->summer-min conversion through the shared
                        # scraping_common helpers (v1.4.0). Output 09b_05 is
                        # byte-identical; the conversion is now shared with
                        # Script 21 (21_forestry_06).
# 1.2.0 — 2026-05-08 — hardcoded SCENARIO_VALUES replaced with compute_scenario_bars()
...
def main():
    ...
    for well_name in all_wells:
        ...
        if series is None:
            warn(f"{well_name} not found in well data")
            continue
        ...
        if pre is None or post is None:
            warn(f"{well_name} \u2014 insufficient data in one or both windows")
            continue
...
def _plot_equilibration(wells_clean):
    ...
    if ceh36_col is None or ceh4_col is None:
        warn("CEH36 or CEH4 not found \u2014 skipping trajectory figure")
        return

    ceh36_cc = _make_cc(wells_clean[ceh36_col])
    ceh4_cc  = _make_cc(wells_clean[ceh4_col])
    if ceh36_cc is None or ceh4_cc is None:
        warn("insufficient pre-scraping data \u2014 skipping trajectory figure")
        return
...
FRAC_AFFECTED = {"C1": 0.0, "C2": 0.0, "C3": 0.2857, "C4": 0.7778, "C5": 1.00}
...
def _compute_scraping_bars(centroids_df):
    ...
    if len(c3_row) == 0 or len(c4_row) == 0:
        warn("centroid summaries incomplete \u2014 scraping bars set to zero")
...
def _plot_scenario_comparison(centroids_df, dpi=200):
    ...
    ax.text(0.98, 0.02,
            ...
            "within 800 m uphill of CEH36 (C3: 29%, C4: 78%, C5: 100%)",
...
def _summer_scenario(wells_clean, wells_ext, climate):
    ...
    if OUT_17_SY_TABLE.exists():
        ...
        sy_by_cluster = scenario_cluster_sy(
            sy_df, clusters=tuple(clusters), fallback=SY_FALLBACK)
        if sy_by_cluster:
            print("   Sy from Script 17: " +
