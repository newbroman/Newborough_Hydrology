...
def write_summary(forest, c4, c5, corr_df, reg_df, summary_df,
    ...
    if len(nw10) > 0:
        ...
        lines.append("   Both position (closest to ridge) and canopy type may contribute.")
    ...
    if len(ceh14) > 0:
        ...
        lines.append("   not a genuine outlier (upper end of tight linear relationship).")
    lines.append("")
    ...
    lines.append("   Conclusion: boundary reflects a real topographic/substrate")
    lines.append("   transition (dune ridge → coastal plain), not an arbitrary cut.")
