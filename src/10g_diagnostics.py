...
from utils.paths import (
    make_all_dirs, INT_REGIONAL_AVG, OUT_10G_NW10_TREND, OUT_10G_TRANSECT_FIG,
    OUT_10G_TRANSECT_CSV, OUT_10G_ROLLING_CSV, OUT_10G_REPORT,
)
...
def nw10_broadleaf_trend(wells, rpt):
    ...
    if len(common_yrs) < 5:
        skipped("Insufficient common years — skipping")
        return
    ...
    saved(f"{OUT_10G_NW10_TREND.name}")
...
def clearfell_transect(wells, rpt):
    ...
    if len(transect_available) < 3:
        skipped("Too few transect wells available — skipping")
        return
    ...
    saved(f"{OUT_10G_TRANSECT_CSV.name}")
...
def rolling_coefficients(wells, climate, rpt):
    ...
    valid_impact = [w for w in IMPACT_WELLS
                    if w in wells.columns and w not in EXCLUDED_WELLS]
    if not valid_impact:
        skipped("No impact wells available — skipping")
        return
    ...
    try:
        ...
        skipped("Could not load cluster centroids from Script 03 — skipping")
        return
    ...
    if not roll_impact.empty and not roll_c3.empty:
        ...
        if len(post_roll) >= 6 and len(c3_post) >= 6:
            ...
            skipped("Insufficient post-felling rolling windows for transition assessment")
    else:
    ...
    if not roll_export.empty:
        ...
        saved(f"{OUT_10G_ROLLING_CSV.name}")
    else:
        skipped("No rolling data to export")
