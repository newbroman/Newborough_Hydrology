...
#         synthetic-control and SSM-residual steps (read from 10f outputs), and
#         the FE2 (inside-footprint) / FE1 (~20 m outside) per-well divergences.
#         Visualises the §4.6 robustness convergence: felled / in-footprint
#         estimators cluster small-positive, the out-of-footprint control
#         straddles zero. NEW FIGURE ONLY — no change to any CSV or to the
#         ANCOVA / FE computations. New import: Line2D. Reads 10f_01/10f_02 and
#         skips the synth/residual rows gracefully if those outputs are absent.
...
OUT_FIG_FOREST    = DIR_10 / "10h_11_robustness_forest.png"
...
for fe_name in FE_SYNTH_WELLS:
    ...
    if n_cal < 15:
        warn(f"{fe_name.upper()} has only {n_cal} calibration months — skipping")
        continue
...
for row, fe_name in enumerate(synthetic_wells):
    ...
    ax.set_title('Post-clearfell: actual minus counterfactual', fontsize=10)
    ax.legend(fontsize=8, frameon=False)
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
...
if cusum_key in all_frames and cusum_key in all_results:
    ...
    warn("Variant B Forest not available for CUSUM")
...
rpt.save(OUT_REPORT)
saved(f"{OUT_REPORT.name}")

# ============================================================================
# ROBUSTNESS FOREST PLOT (consolidated clearfell-step estimators)
# ============================================================================
phase(11, "Generating robustness forest plot")
_cf = comp_df[comp_df['Control'] == 'Forest'].set_index('Variant')
_cal = cal_df.set_index('Well')

# Independent estimators from Script 10f (synthetic control, SSM residual).
# 10f runs earlier in run_10_clearfell.py; skip gracefully if not present.
_synth_step = _synth_p = _resid_step = _resid_p = None
try:
    _fs = pd.read_csv(DIR_10 / "10f_02_synthetic_control_results.csv")
    _fs = _fs[_fs['Zone'] == 'Impact'].iloc[0]
    _synth_step, _synth_p = float(_fs['Step_m']), float(_fs['P_value'])
    _fr = pd.read_csv(DIR_10 / "10f_01_ssm_residual_results.csv")
    _fr = _fr[_fr['Zone'] == 'Impact'].iloc[0]
    _resid_step, _resid_p = float(_fr['Step_m']), float(_fr['P_value'])
except (FileNotFoundError, IndexError, KeyError):
    print("    (10f outputs not found — synthetic/residual rows omitted)")

# (label, step, ci_lo, ci_hi, p) — ci None where the method stores no interval
_rows = [
    ("ANCOVA — WMC3 (headline)",  _cf.loc['C (WMC3 only)', 'Clearfell_step_m'],
     _cf.loc['C (WMC3 only)', 'Clearfell_CI_lo_m'],
     _cf.loc['C (WMC3 only)', 'Clearfell_CI_hi_m'],
     _cf.loc['C (WMC3 only)', 'Clearfell_p']),
    ("ANCOVA — WMC3 + FE2",       _cf.loc['B (WMC3+FE2)', 'Clearfell_step_m'],
     _cf.loc['B (WMC3+FE2)', 'Clearfell_CI_lo_m'],
     _cf.loc['B (WMC3+FE2)', 'Clearfell_CI_hi_m'],
     _cf.loc['B (WMC3+FE2)', 'Clearfell_p']),
    ("ANCOVA — WMC3 + FE1 + FE2", _cf.loc['A (WMC3+FE1+FE2)', 'Clearfell_step_m'],
     _cf.loc['A (WMC3+FE1+FE2)', 'Clearfell_CI_lo_m'],
     _cf.loc['A (WMC3+FE1+FE2)', 'Clearfell_CI_hi_m'],
     _cf.loc['A (WMC3+FE1+FE2)', 'Clearfell_p']),
]
_grp = ['ANCOVA', 'ANCOVA', 'ANCOVA']
if _synth_step is not None:
    _rows.append(("Synthetic control", _synth_step, None, None, _synth_p)); _grp.append('Indep')
if _resid_step is not None:
    _rows.append(("SSM forward residual", _resid_step, None, None, _resid_p)); _grp.append('Indep')
_rows.append(("FE2 — inside felled footprint",
              _cal.loc['FE2', 'PostFell_divergence_mm'] / 1000.0, None, None,
              _cal.loc['FE2', 'PostFell_divergence_p'])); _grp.append('FE')
_rows.append(("FE1 — ~20 m outside footprint",
              _cal.loc['FE1', 'PostFell_divergence_mm'] / 1000.0, None, None,
              _cal.loc['FE1', 'PostFell_divergence_p'])); _grp.append('FE')

_headline = _rows[0][1]
_n = len(_rows)
_yp = np.arange(_n)[::-1]
_ACC, _HEAD, _NS = '#2166ac', '#b2182b', '#999999'

fig, ax = plt.subplots(figsize=(7.6, 5.0), dpi=200)
ax.axvline(0, color='0.35', lw=1.0, zorder=1)
ax.axvline(_headline, color=_HEAD, lw=1.0, ls='--', alpha=0.6, zorder=1)
for _y, (_lab, _step, _lo, _hi, _p) in zip(_yp, _rows):
    _sig = _p < 0.05
    _col = _HEAD if 'headline' in _lab else (_ACC if _sig else _NS)
    if _lo is not None and pd.notna(_lo):
        ax.plot([_lo, _hi], [_y, _y], color=_col, lw=1.6, zorder=2)
        ax.plot([_lo, _lo], [_y - 0.12, _y + 0.12], color=_col, lw=1.2)
        ax.plot([_hi, _hi], [_y - 0.12, _y + 0.12], color=_col, lw=1.2)
    ax.scatter(_step, _y, s=70, facecolors=(_col if _sig else 'white'),
               edgecolors=_col, linewidths=1.6, zorder=3)
    ax.annotate(f"{_step:+.3f} m" + ("" if _sig else "  (ns)"), (_step, _y),
                xytext=(6, 8), textcoords='offset points', fontsize=8, color='0.25')
for _i in range(1, _n):
    if _grp[_i] != _grp[_i - 1]:
        ax.axhline(_yp[_i] + 0.5, color='0.88', lw=0.8, zorder=0)
ax.set_yticks(_yp)
ax.set_yticklabels([r[0] for r in _rows], fontsize=9)
ax.set_ylim(_yp.min() - 0.6, _yp.max() + 0.6)
ax.set_xlabel('Clearfell step at the Impact tier (m)', fontsize=10.5)
ax.set_title('Robustness of the clearfell step across independent estimators',
             fontsize=11.5)
ax.grid(axis='x', color='0.93', lw=0.6, zorder=0)
ax.legend(handles=[
    Line2D([], [], marker='o', mfc=_ACC, mec=_ACC, ls='', ms=8, label='significant (p<0.05)'),
