...
def main():
    ...
    overlap = pd.concat([target, donor], axis=1, sort=False, join='inner').dropna()
    ...
    # ------------------------------------------------------------------
    phase(3, f"Hindcasting {TARGET_WELL.upper()} over pre-record window")
    hind_dates = donor.index[donor.index < target.index.min()]
    if len(hind_dates) == 0:
        raise RuntimeError("No pre-target donor coverage — nothing to hindcast.")
