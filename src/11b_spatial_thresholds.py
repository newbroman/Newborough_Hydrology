...
# 1.1.2 — data/geo/ reorg: DEM + site-boundary now via utils.paths constants
#          (DATA_DEM, DATA_KML_SITE_BOUNDARY) from data/geo/. No functional change.
...
def _load_cluster_coefficients() -> dict:
    ...
    for _, r in cc.iterrows():
        ...
        if b3_val < 0:
            warn(f"Cluster {cluster_int}: β₃ = {b3_val:.4f} is negative (expected positive under displacement formulation)")
        coeffs[cluster_int] = {
...
def plot_winter_maxima_map(df: pd.DataFrame, dpi: int = 300) -> None:
    ...
    if not winter_rows:
        warn("No winter maxima data available")
        return
...
def plot_pflood_map(df: pd.DataFrame, dpi: int = 300) -> None:
    ...
    if len(pf_reachable) < 3:
        warn("Too few reachable wells for IDW surface; plotting markers only.")
        sc = None
        gx = gy = surf = None
    else:
...
def plot_flood_frequency_map(df: pd.DataFrame, dpi: int = 300) -> None:
    ...
    if not ff_rows:
        warn("No flood frequency data computed")
        return
...
def export_table10_spreadsheet() -> None:
    ...
    if not OUT_11_TABLE8_THRESHOLDS.exists():
        warn(f"Script 11 output not found at {OUT_11_TABLE8_THRESHOLDS}; cannot generate Table 10. Run Script 11 first.")
        return
    ...
    if missing:
        warn(f"Script 11 output is missing expected columns: {missing}. Run the updated Script 11 to regenerate the full CSV.")
        return
...
def _build_forecaster_data_bundle() -> dict:
    ...
    for _, r in cc.iterrows():
        ...
        if pfr is None:
            warn(f"No P_flood entry for {c}; skipping.")
            continue
        b3_fc = float(r["beta_3_drainage"])
        if b3_fc < 0:
            warn(f"{c}: β₃ = {b3_fc:.4f} is negative (expected positive under displacement formulation)")
        cluster_coeffs[c] = {
    ...
    for block, cs in block_clusters.items():
        ...
        if w.empty or s.empty:
            warn(f"Transfer-function row missing for block {block}; skipping.")
            continue
...
def _build_base_layer() -> dict:
    ...
    if dem_path.exists():
        try:
            ...
            warn(f"Hillshade embed failed: {exc}")
    else:
        warn(f"DEM not found at {dem_path}; forecaster will lack hillshade")
...
def build_forecaster_html() -> None:
    ...
    if not SRC_FORECASTER_TEMPLATE.exists():
        warn(f"Forecaster template not found at {SRC_FORECASTER_TEMPLATE}; skipping forecaster HTML generation.")
        return
    for needed in (OUT_11_TABLE8_THRESHOLDS, INT_CLUSTER_MECHANISTIC,
        ...
        if not needed.exists():
            warn(f"Required input missing: {needed}. Run Scripts 03 and 11 first; skipping forecaster.")
            return
    ...
    if n_wells == 0 or n_clusters == 0 or n_blocks == 0:
        warn(f"Empty data bundle ({n_wells} wells, {n_clusters} clusters, {n_blocks} blocks); skipping forecaster HTML.")
        return
    ...
    saved(f"{OUT_11B_FORECASTER_HTML.name}")
