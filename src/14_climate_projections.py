...
Outputs:
    ...
    outputs/14_climate_projections/14_winter_trend_stats.csv
    outputs/14_climate_projections/14_annual_extremes.csv
    outputs/14_climate_projections/14_winter_exceedance.csv
...
Reviewer-facing method summary:
    ...
    - Winter trend statistics (observed-period OLS slope, R2, p-value) are
      descriptive only — no projection is fitted — but are persisted to
      14_winter_trend_stats.csv so downstream report text cites a pipeline
      output rather than a recomputed value.
"""
...
#         discarded). Closes the provenance gap whereby the report
#         quoted winter trend figures with no _report_numbers-style
#         CSV. New path: OUT_14_WINTER_TREND_CSV.
...
def main() -> None:
    ...
    for c in TRAJECTORY_CLUSTERS:
        ...
        # Winter — observed only, no projection. Descriptive trend statistics
        # are computed and persisted (see 14_winter_trend_stats.csv) so the
        # report can cite them from a pipeline output rather than recomputing.
        if c in winter_max and len(winter_max[c]) >= 5:
            w_years = winter_max[c].index.to_numpy(dtype=float)
            w_vals = winter_max[c].values
            w_slope, _, w_r, w_pval, _ = stats.linregress(w_years, w_vals)
            print(
                f"  {c} winter observed trend (descriptive only): "
                f"{w_slope:+.4f} m yr\u207b\xb9  "
                f"(R\u00b2={w_r**2:.3f}, p={w_pval:.4f}, n={len(w_years)})"
            )
            winter_data[c] = (w_years, w_vals)
            winter_trend_stats[c] = {
                "n_years": len(w_years),
                "slope": w_slope,
                "r2": w_r ** 2,
                "p_value": w_pval,
    ...
    # provenance standard of the summer trend table).
    winter_trend_rows = []
    for c in TRAJECTORY_CLUSTERS:
...
def render_seasonal_scatter(out_html: Path) -> None:
    ...
    try:
        ...
        warn("[14] Warning: well network table not found — skipping scatter plot")
        return

    if "Mean_Summer_Min_m" not in net.columns:
        warn("[14] Warning: Mean_Summer_Min_m column missing from well network table")
        print("       Run updated script 00 first to generate these columns.")
        return
