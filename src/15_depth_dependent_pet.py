...
The standard SSM is:
    Δh_t = β₁·P_t  −  β₂·PET_t  −  β₃·h_{t-1}

The modified model replaces the fixed β₂ with a depth-dependent term:
    Δh_t = β₁·P_t  −  β₂·exp(−λ·d_{t-1})·PET_t  −  β₃·h_{t-1}
...
def iterative_simulate(h0: float, P: np.ndarray, PET: np.ndarray,
    ...
    Parameters
    ----------
    h0          : initial water table (negative convention, m from pipe top)
    P           : precipitation array (m)
    PET         : PET array (m)
    depth_prev  : cluster mean upstand (scalar, m) — used to compute
                  d_t = -h_t + upstand at each step
    b1,b2,b3    : OLS coefficients
    ...
    h = np.full(n, np.nan)
    h_t = h0
    upstand = float(depth_prev)   # reused as scalar upstand here

    for t in range(n):
        d_t = max(-h_t + upstand, 0.0)          # depth below ground (≥ 0)
        decay = np.exp(-lam * d_t)
        delta = b1 * P[t] - b2 * decay * PET[t] - b3 * h_t
        h_t = h_t + delta
        h[t] = h_t
...
def fit_at_lambda(df: pd.DataFrame, lam: float, upstand: float) -> dict:
    """
    Given a cluster centroid DataFrame with columns [h, P_m, PET, h_prev],
    fit the depth-dependent SSM by OLS at a fixed lambda.
    ...
    X = pd.DataFrame({
        "beta_1_recharge":        df["P_m"].values,
        "beta_2_atmospheric_draw": -decay * df["PET"].values,
        "beta_3_internal_brake":  -df["h_prev"].values,
    }, index=df.index)
    ...
    return {
        ...
        "b3":  model.params["beta_3_internal_brake"],
        "r2":  model.rsquared,
    }
...
def load_data():
    ...
    if INT_WELL_ELEV.exists():
        ...
        # Expect columns: Well (or Name), Upstand_m
        name_col = "Well" if "Well" in elev_df.columns else elev_df.columns[0]
...
def build_cluster_centroids(wells_clean, cluster_stats, well_col_lookup, upstand_map):
    ...
    for cid in cluster_ids:
        ...
        for col, us in zip(available_cols, well_upstands):
            # depth in clean file is negative below pipe; add upstand gives
...
def build_regression_df(centroid: pd.Series, climate: pd.DataFrame,
                        data_limit: int = DATA_LIMIT) -> pd.DataFrame:
    """Merge centroid with climate and build Δh, h_prev columns."""
...
def fit_standard_ssm(df: pd.DataFrame) -> dict:
    ...
    # Iterative NSE
    h_sim = iterative_simulate(
        h0=df["h"].iloc[0], P=df["P_m"].values, PET=df["PET"].values,
        depth_prev=0.0,   # upstand=0 → no correction, standard model
...
def plot_fit_comparison(regression_dfs: dict, centroids: dict, climate: pd.DataFrame,
    ...
    for i, cid in enumerate(CLUSTER_ORDER):
        ...
        # Standard SSM simulation
        h_ssm = iterative_simulate(
            h0=df["h"].iloc[0], P=df["P_m"].values, PET=df["PET"].values,
            depth_prev=0.0, b1=ssm["b1"], b2=ssm["b2"], b3=ssm["b3"], lam=0.0,
        )

        # Depth-dependent simulation
        h_ddp = iterative_simulate(
            h0=df["h"].iloc[0], P=df["P_m"].values, PET=df["PET"].values,
            depth_prev=upstand, b1=bp["b1"], b2=bp["b2"], b3=bp["b3"],
            lam=bp["best_lambda"],
        )
