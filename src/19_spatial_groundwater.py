...
#          No functional change.
                         ...
                         #         New top row in the per-cluster scenario
                         #         summary table reports the 5-year mean
                         #         spring water-level shift (mean Δh over
                         #         March-May) per cluster per scenario --
                         #         the van Willegen et al. (2025) metric
                         #         and the published best-performing
                         #         predictor of dune-slack vegetation
                         #         response.  Surfaces vegetation-relevant
                         #         information alongside the existing
                         #         hydrological Δh rows in the SAC's
                         #         primary public-facing tool.
                         #         IMPORTANT formulation choice (recorded
                         #         here because the canonical 26b output
                         #         is the validation target):  the ΔMSL5
                         #         row mirrors Script 26b's pure-climate
                         #         perturbation exactly --
                         #           Δh(m) = β₁·P(m)·(sP(m)-1)
                         #                  − β₂·PET(m)·(sPET(m)-1)
                         #         -- using per-month climatology and no
                         #         forest interception, so it matches
                         #         26b_msl5_ukcp18_projection_summary.csv
                         #         (column msl5_shift_mean_m) to within
                         #         rounding (≤0.5 mm).  The other rows of
                         #         the same table (Δ head, storage shift,
                         #         Pₑₑ, PET draw) DO apply forest
                         #         interception and a per-scenario sB2 for
                         #         C4/C5, so the two formulations sit
                         #         side-by-side and may disagree slightly
                         #         for the forest clusters under climate-
                         #         change presets.  A footnote beneath the
                         #         cluster summary table flags this
                         #         asymmetry explicitly and notes that
                         #         MSL5 is primarily a vegetation
                         #         predictor for C1-C3.  Decision recorded
                         #         per Martin (2026-05-27 chat):
                         #         "Match the canonical (drop interception
                         #         from MSL5 row only) but make it plain
                         #         in the scenario viewer that interception
                         #         is not accounted for. MSL is only
                         #         really relevant for c1-3."
                         #         New constants: SHOULDER_MONTHS=[4,10],
                         #         SPRING_MONTHS=[3,4,5].  Helpers added:
                         #         _expand_seasonal_to_monthly(),
                         #         _cluster_msl5_shift().  CLIMATE.
                         #         monthly_arrays_m.P / .PET added to the
                         #         JS payload (12-element arrays in m).
                         #         19_scenario_summary.csv gains a
                         #         season="msl5" block: 6 rows per scenario
                         #         (5 clusters + SITE), well-count-weighted
                         #         SITE aggregate.  Canonical match
                         #         validated against
                         #         26b_msl5_ukcp18_projection_summary.csv
                         #         for both UKCP18 presets (max abs
                         #         difference 0.000 m).
                         ...
                         #         identically to C4 and C5.  Broadleaf preset
                         #         now uses sB2_w=0.87, sB2_s=1.09 (means of
                         #         Script 21's monthly deciduous profile over
                         #         Nov-Mar / May-Sep).  Clearfell/thinning
                         #         presets apply their BACI multiplier uniformly
                         #         across seasons.  Python _well_dh() and JS
                         #         dhOne() updated to pass seasonal sB2.
                         #         Summary table PET-draw row is now season-
                         #         aware for β₂ scaling.
...
SHOULDER_MONTHS = [4, 10]            # April + October -- transitional months
                                     ...
                                     # the seasonal-window sP/sPET are projected
                                     # onto a 12-month calendar array.  Used by
...
def _init_scenario_params():
    ...
    # broadleaf has deciduous phenology (lower winter, higher summer).
    SCENARIO_PARAMS = {
...
def build_hillshade_base64(site_polygon_xy=None):
    ...
    try:
        ...
        info(f"Hillshade skipped (missing dependency: {exc}).")
        return ""

    if not DATA_DEM.exists():
        info(f"Hillshade skipped (DEM not found at {DATA_DEM}).")
        return ""
    ...
    try:
        ...
        if site_polygon_xy is not None and len(site_polygon_xy) >= 3:
            try:
                ...
                warn(f"Site-mask step skipped: {exc}")
        ...
        warn(f"Hillshade build failed: {exc}")
        return ""
...
def build_dem_grid(site_polygon_xy=None):
    ...
    try:
        ...
        info(f"DEM grid skipped (missing dependency: {exc}).")
        return {}

    if not DATA_DEM.exists():
        info(f"DEM grid skipped (DEM not found at {DATA_DEM}).")
        return {}
    ...
    try:
        ...
        if site_polygon_xy is not None and len(site_polygon_xy) >= 3:
            try:
                ...
                warn(f"DEM grid site-mask skipped: {exc}")
        ...
        warn(f"DEM grid build failed: {exc}")
        return {}
...
def build_well_table(loc, cl, md, elev, maod, clim, sy_df):
    ...
    # 12-element calendar-ordered arrays in metres, used by the v2.8.0 ΔMSL5
    # row (computed in Python for the summary CSV and serialised to JS for
    # the live viewer table).  Index 0 = January, 11 = December.  Mirrors
    # Script 26b's monthly_P_m / monthly_PET_m construction.
    monthly_P_m_arr   = [float(monthly["P_m"].get(m, 0.0))  for m in range(1, 13)]
    monthly_PET_m_arr = [float(monthly["PET"].get(m, 0.0))  for m in range(1, 13)]
    climate_stats = {
...
def serialise_climate(wt, climate_stats):
    ...
    # 12-element calendar-ordered arrays in metres for the v2.8.0 ΔMSL5
    # row.  JS expects index 0 = January.  Kept separate from monthly.*_mm
...
/* Layout grid */
.main{{display:flex;gap:0;padding:12px;align-items:stretch;}}
@media(max-width:640px){{.main{{flex-direction:column;}}.ctrl{{width:100%!important;}}}}
...
.splitter{{width:8px;cursor:col-resize;background:var(--border);border-radius:3px;
           flex-shrink:0;align-self:stretch;transition:background 0.15s;
           margin:0 2px;}}
.splitter:hover,.splitter.active{{background:var(--water);}}
...
/* Right column */
.rp{{min-width:0;flex:1;display:flex;flex-direction:column;gap:10px;margin-left:12px;}}
...
/* Metric cards */
.metrics{{display:grid;grid-template-columns:repeat(auto-fit,minmax(110px,1fr));gap:6px;}}
...
.mapwrap{{position:relative;width:640px;max-width:100%;height:440px;border-radius:3px;
         resize:both;min-width:320px;min-height:220px;overflow:hidden;}}
...
.tip{{position:fixed;pointer-events:none;background:rgba(255,255,255,0.95);
     border:1px solid var(--border);border-radius:4px;padding:6px 10px;
     font-size:12px;color:var(--text);line-height:1.5;
     box-shadow:0 2px 8px rgba(0,0,0,0.15);display:none;z-index:1000;
...
<div class="ctrl" id="ctrlPanel">
  ...
  <div class="ch">Forest &#946;&#8322; scaling (C4 + C5)</div>
  <div class="srow"><label>Winter &#946;&#8322;</label>
    <input type="range" min="0.5" max="2" step="0.01" value="1" id="sB2_w" oninput="onSl()">
    <span class="sv" id="vB2_w">1.00&#215;</span></div>
  <div class="srow"><label>Summer &#946;&#8322;</label>
...
<div class="rp">
  ...
  <div class="panel">
    ...
    <div style="font-size:10px;color:var(--text-light);line-height:1.4;margin-top:5px;">
      &#916;MSL5 is the mean change in monthly water level over the spring
      window (March&#8211;May), the 5-year mean spring water level metric of
      van Willegen et al. (2025) and the best-performing published predictor
      of dune-slack vegetation response. It is computed as a pure-climate
      perturbation (&#946;&#8321;&#183;&#916;P &#8722; &#946;&#8322;&#183;&#916;PET)
      and is independent of the season tab.
      <strong>Canopy interception is not applied to this row</strong>; the
      other rows above (&#916;&#160;head, storage shift, P&#8337;&#8337;,
      PET draw) do apply it for the forest clusters (C4, C5) under their
      seasonal P/PET multipliers, so the two formulations sit side-by-side
      and may disagree slightly for C4 and C5 under climate-change presets.
      MSL5 is in any case a vegetation predictor primarily relevant to the
      open dune-slack clusters C1&#8211;C3.
    </div>
  </div>
...
function checkExtremes(sl){{
  ...
  if((sl.sI_c4<=0.02&&sl.sB2_w<=0.6&&sl.sB2_s<=0.6)||(sl.sI_c5<=0.02&&sl.sB2_w<=0.6&&sl.sB2_s<=0.6)) msgs.push('Zero interception with reduced \u03b2\u2082 is contradictory: removing canopy typically increases atmospheric draw, not decreases it.');
...
function go(){{
  ...
  // Pure-climate perturbation, no canopy interception, no β₂ scaling.
  // Mirrors Script 26b exactly (see _cluster_msl5_shift in the Python
  ...
  function _msl5One(b1,b2){{
    ...
    var WIN=[11,12,1,2,3], SUM=[5,6,7,8,9], SHO=[4,10];
    for(var i=0;i<12;i++){{ sPm[i]=1.0; sPETm[i]=1.0; }}
...
document.addEventListener('DOMContentLoaded',function(){{
  document.getElementById('mapC').addEventListener('mousemove',function(e){{
    if(!MW)return;
    var r=this.getBoundingClientRect(),sx=MW/r.width,sy2=MH/r.height;
    var mx=(e.clientX-r.left)*sx,my=(e.clientY-r.top)*sy2;
    ...
    if(best&&bd<18){{
      ...
      if(best.dg!=null&&hb!=null){{
        ...
        depTxt='<br>Depth: '+depBase.toFixed(2)+' \u2192 '+depSce.toFixed(2)+'\u00a0m';
      }}
      tip.innerHTML='<strong>'+best.n+'</strong> \u00b7 C'+best.cl
        +'<br>Baseline: '+hb.toFixed(2)+'\u00a0m\u00a0AOD'
      ...
      if(tx2+tip.offsetWidth>window.innerWidth-10)tx2=e.clientX-tip.offsetWidth-14;
      if(ty2<5)ty2=5;
      if(ty2+tip.offsetHeight>window.innerHeight-5)ty2=window.innerHeight-tip.offsetHeight-5;
      tip.style.left=tx2+'px';tip.style.top=ty2+'px';
    }}else{{tip.style.display='none';}}
  }});
  document.getElementById('mapC').addEventListener('mouseleave',function(){{
...
function renderTable(){{
  var cls=[1,2,3,4,5],sl=CUR_SL||{{sP_w:1,sP_s:1,sPET_w:1,sPET_s:1,sSyMode:0,sI_c4:FOREST_INTERCEPTION,sI_c5:FOREST_INTERCEPTION,sB2_w:1,sB2_s:1}},cld=CLIMATE[sea],P=cld.P,PET=cld.PET;
  ...
  var sB2_view=(sea==='winter')?sl.sB2_w:(sea==='summer')?sl.sB2_s:0.5*(sl.sB2_w+sl.sB2_s);
  var rows=[
    // v2.8.0 -- vegetation-relevant ΔMSL5 row.  Independent of the season
    // tab (spring is its own fixed window).  Pure-climate perturbation,
    // no canopy interception -- mirrors Script 26b exactly; see footnote
    // beneath the table.  Title-case label gives full disambiguation
    // (avoids confusion with "mean sea level") on first appearance.
    {{l:'\u0394MSL5 (m) — Mar\u2013May, van Willegen 2025',
      v:cls.map(function(c){{
        var v=MSL5[c];
        if(v==null)return'\u2014';
...
function renderMetrics(){{
  ...
  var fDH=fW.length>0?(fW.reduce(function(s,w){{var d=w._dh;return d!=null?s+d:s;}},0)/fW.filter(function(w){{return w._dh!=null;}}).length||0):0;
  var fSy=fW.length?fW.reduce(function(s,w){{return s+syEff(w,sl.sSyMode);}},0)/fW.length:0.12;
  var fShift=fSy*fDH*1000;
...
function applyLayout(){{
  ...
  if(window.innerWidth<640){{ctrl.style.width='100%';sp.style.display='none';}}
  else{{sp.style.display='';}}
}}
function initSplitter(){{
  var sp=document.getElementById('splitter'),ctrl=document.getElementById('ctrlPanel');
  if(!sp||!ctrl)return;
  var dragging=false,startX=0,startW=0;
  sp.addEventListener('mousedown',function(e){{
    dragging=true;startX=e.clientX;startW=ctrl.offsetWidth;
    sp.classList.add('active');document.body.style.cursor='col-resize';
  ...
  document.addEventListener('mouseup',function(){{
    if(dragging){{dragging=false;sp.classList.remove('active');
      document.body.style.cursor='';document.body.style.userSelect='';}}
  }});
  sp.addEventListener('touchstart',function(e){{
    dragging=true;startX=e.touches[0].clientX;startW=ctrl.offsetWidth;
    sp.classList.add('active');
  }},{{passive:true}});
  document.addEventListener('touchmove',function(e){{
    if(!dragging)return;
    var newW=Math.max(160,Math.min(600,startW+(e.touches[0].clientX-startX)));
    ctrl.style.width=newW+'px';
  }});
...
def _expand_seasonal_to_monthly(sP_w, sP_s, sPET_w, sPET_s):
    """
    Convert seasonal P / PET multipliers into 12-element monthly arrays.
    Winter (Nov-Mar) and summer (May-Sep) months take their respective
    seasonal multiplier; April and October are shoulder months taking the
    mean of the two.  Matches Script 26b._monthly_multipliers exactly so
    the viewer's ΔMSL5 row reproduces the canonical
    26b_msl5_ukcp18_projection_summary.csv values.

    Returns (sP_arr, sPET_arr), each a numpy array of length 12 indexed
    ...
    for m in SHOULDER_MONTHS:
        sP[m - 1]   = 0.5 * (sP_w + sP_s)
        sPET[m - 1] = 0.5 * (sPET_w + sPET_s)
    return sP, sPET
...
def _well_dh(row, sl, P0, PET0, h_col, cluster_betas, season):
    ...
    if season == "winter":
        return _dh_one(P0, PET0, sl["sP_w"], sl["sPET_w"], sl["sB2_w"])
    elif season == "summer":
        return _dh_one(P0, PET0, sl["sP_s"], sl["sPET_s"], sl["sB2_s"])
    else:  # annual
        ...
        dh_w = _dh_one(P_w, PET_w, sl["sP_w"], sl["sPET_w"], sl["sB2_w"])
        dh_s = _dh_one(P_s, PET_s, sl["sP_s"], sl["sPET_s"], sl["sB2_s"])
...
def compute_scenario_summary(wt, climate_stats, out_dir):
    ...
    print(f"  Scenario summary CSV: {out_csv.name} "
          ...
          f"{n_seasons_eff} seasons (incl. msl5) x 6 spatial units)")

    # ── v2.8.0 ΔMSL5 cross-script validation ──────────────────────────────────
    # The viewer's ΔMSL5 row aggregates per-well betas to a cluster mean and
    # applies Script 26b's canonical pure-climate perturbation; the same
    # aggregation+formulation is computed independently by 26b v1.1.0
    # (compute_perwell_msl5_summary).  This block loads 26b's per-well CSV
    # and confirms ≤0.5 mm match per (cluster, UKCP18 scenario).  Warns on
    # miss but never crashes the pipeline -- 26b may be absent (first run
    # of a fresh checkout) or out of date.
    try:
        canon_path = OUT_26B_PROJECTION_TABLE_PERWELL
    except NameError:
        canon_path = None
    if canon_path is not None and Path(canon_path).exists():
        try:
            canon = pd.read_csv(canon_path)
            canon_msl5 = canon[canon["cluster_label"].str.startswith("C", na=False)]
            scen_pairs = [("ukcp18_2050s", "2050s"), ("ukcp18_2080s", "2080s")]
            worst_mm = 0.0
            worst_label = ""
            for view_scen, canon_scen in scen_pairs:
                for cid in [1, 2, 3, 4, 5]:
                    view_val = out[(out["scenario"] == view_scen)
                                   & (out["season"] == "msl5")
            ...
            if worst_mm <= tol_mm:
                ...
                warnings.warn(
                    f"ΔMSL5 cross-check vs 26b per-well CSV exceeds tolerance: "
                    f"worst = {worst_mm:.3f} mm at {worst_label} "
                    f"(tolerance {tol_mm} mm).  Likely cause: 03_master_data.csv "
                    f"changed since 26b was last rerun; rerun script 26b."
                )
        except Exception as exc:
            warnings.warn(f"ΔMSL5 cross-check skipped: {exc}")
    else:
