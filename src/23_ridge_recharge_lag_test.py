...
def write_test_summary(ccf_df, fits_df, trend_stats, output_path):
    ...
    lines.append("  Analysis model: Delta_h = alpha + b10*P(t) + b11*P(t-1)"
                 " - b2*PET(t) - b3*h_disp_prev(t)")
    ...
    lines.append("  RESULT")
    lines.append("-" * 78)
    lines.append("  Spearman rank correlation of peak lag vs ridge distance:")
...
def main():
    ...
    step(f"Rainfall AR(1) phi = {phi_P:+.3f}; pre-whitened series built.")
    ...
    saved(f"{INT_23_FITS_TABLE.name}")
