"""
====================================================================================
25_forest_zone_analysis.py — Per-well Forest Zone Spatial Analysis
====================================================================================
Purpose:
...
Outputs:
    INT_25_CORRELATION_TABLE  — Pearson correlations and regression R² values
    INT_25_CLUSTER_SUMMARY    — C4 vs C5 summary statistics and t-test results
    OUT_25_B1_B2_SCATTER      — β₁ vs β₂ scatter coloured by cluster
    OUT_25_B2_ELEV_REGRESSION — β₂ vs elevation regression with R² annotation
    OUT_25_BOUNDARY_MAP       — Spatial map of C4/C5 wells with elevation context
    OUT_25_SUMMARY            — Plain-text interpretive summary
====================================================================================
"""
...
from utils.paths import (
    ...
    INT_LOCATIONS,
    INT_25_CORRELATION_TABLE,
    INT_25_CLUSTER_SUMMARY,
    OUT_25_B1_B2_SCATTER,
    OUT_25_B2_ELEV_REGRESSION,
    OUT_25_BOUNDARY_MAP,
    OUT_25_SUMMARY,
)
...
def plot_b1_b2_scatter(forest, clearfell, c3_boundary):
    ...
    fig.tight_layout()
    fig.savefig(OUT_25_B1_B2_SCATTER, dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"  [saved] {OUT_25_B1_B2_SCATTER.name}")
...
def plot_b2_elevation_regression(forest):
    ...
    ax.grid(True, alpha=0.15)
    fig.tight_layout()
    fig.savefig(OUT_25_B2_ELEV_REGRESSION, dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"  [saved] {OUT_25_B2_ELEV_REGRESSION.name}")
...
def plot_boundary_map(forest, coeff):
    ...
    fig.tight_layout()
    fig.savefig(OUT_25_BOUNDARY_MAP, dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"  [saved] {OUT_25_BOUNDARY_MAP.name}")
...
def write_summary(forest, c4, c5, corr_df, reg_df, summary_df,
    ...
    lines = []
    lines.append("=" * 72)
    lines.append("SCRIPT 25 — Forest Zone Spatial Analysis Summary")
    lines.append("=" * 72)
    lines.append("")
    ...
    text = "\n".join(lines)
    OUT_25_SUMMARY.write_text(text)
    print(f"  [saved] {OUT_25_SUMMARY.name}")
    print(text)
...
def main():
    make_all_dirs()
    print("\n" + "=" * 60)
    print("Script 25 — Forest Zone Spatial Analysis")
    print("=" * 60 + "\n")
    ...
    combined.to_csv(INT_25_CORRELATION_TABLE, index=False)
    print(f"  [saved] {INT_25_CORRELATION_TABLE.name}")

    # Q2/Q4: Cluster summary
    summary_df = compute_cluster_summary(c4, c5)
    summary_df.to_csv(INT_25_CLUSTER_SUMMARY, index=False)
    print(f"  [saved] {INT_25_CLUSTER_SUMMARY.name}")
