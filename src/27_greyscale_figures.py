...
=======================
Renamed in 2026-05-20 commit from 26_greyscale_figures.py → 27_greyscale_figures.py
to free script number 26 for the new van Willegen MSL aggregation step
(Phase 13). This utility is now Phase 14 / step 29 in the pipeline.
...
This script is idempotent. To regenerate after data changes:
    ...
    2. Re-run this script: python src/27_greyscale_figures.py

The outputs_bw/ directory can be safely deleted and rebuilt at any time.

Author: Hollingham 2026
Pipeline step: 29 (Phase 14, post-processing)
"""
...
def convert_figure(
    ...
    try:
        ...
        error(f"Cannot open {src_path}: {e}")
        return False
...
def main():
    ...
    if not COLOUR_DIR.exists():
        error(f"Source directory not found: {COLOUR_DIR}")
        sys.exit(1)

    figures = collect_figures(COLOUR_DIR)
    if not figures:
        warn("No image files found in outputs/")
        sys.exit(0)
    ...
    for fig_path in figures:
        ...
        if is_problem and args.exclude_problem:
            skipped += 1
            skipped(f"{rel_path}  (known problem)")
