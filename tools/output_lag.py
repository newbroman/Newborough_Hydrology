...
THE MANIFEST IS THE RECORD OF A RUN
  ...
  2026-08-28 the whole 50-step pipeline executed and **twelve files changed**:
  four map PNGs by a couple of hundred bytes of metadata, Script 15's five
  artefacts from the κ column rename, the scraping simulator, and the manifest.
  `audit_doc_numbers` put it plainly — *0 numeric cells changed*. Everything
  else regenerated byte-identical, so git recorded no commit for it, so the
